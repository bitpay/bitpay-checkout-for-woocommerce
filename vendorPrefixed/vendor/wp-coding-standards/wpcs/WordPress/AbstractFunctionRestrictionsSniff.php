...
abstract class AbstractFunctionRestrictionsSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\true === $this->is_targetted_token($stackPtr)) {
    ...
    public function is_targetted_token($stackPtr)
    {
        ...
        if (ContextHelper::has_object_operator_before($this->phpcsFile, $stackPtr) === \true) {
            return \false;
        }
        if (ContextHelper::is_token_namespaced($this->phpcsFile, $stackPtr) === \true) {
            return \false;
        }
        ...
        $prev = $this->phpcsFile->findPrevious($search, $stackPtr - 1, null, \true);
        // Skip sniffing on function, OO definitions or for function aliases in use statements.
        $invalid_tokens = Tokens::$ooScopeTokens;
        $invalid_tokens += array(\T_FUNCTION => \T_FUNCTION, \T_NEW => \T_NEW, \T_AS => \T_AS);
        if (isset($invalid_tokens[$this->tokens[$prev]['code']])) {
            return \false;
        }
        // Check if this could even be a function call.
        $next = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if (\false === $next) {
            return \false;
        }
        // Check for `use function ... (as|;)`.
        if (\T_STRING === $this->tokens[$prev]['code'] && 'function' === $this->tokens[$prev]['content'] && (\T_AS === $this->tokens[$next]['code'] || \T_SEMICOLON === $this->tokens[$next]['code'])) {
            return \true;
        }
    ...
    {
        MessageHelper::addMessage($this->phpcsFile, $this->groups[$group_name]['message'], $stackPtr, 'error' === $this->groups[$group_name]['type'], MessageHelper::stringToErrorcode($group_name . '_' . $matched_content), array($matched_content));
    }
    /**
    ...
    {
        $function_name = \str_replace(array('.*', '*'), '@@', $function_name);
        // Replace wildcards with placeholder.
