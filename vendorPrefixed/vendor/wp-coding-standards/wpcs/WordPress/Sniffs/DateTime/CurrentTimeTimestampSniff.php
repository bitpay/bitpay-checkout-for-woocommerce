 ...
 */
namespace BitPayVendor\WordPressCS\WordPress\Sniffs\DateTime;
...
final class CurrentTimeTimestampSniff extends AbstractFunctionParameterSniff
{
     ...
     */
    protected $target_functions = array('current_time' => \true);
    /**
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        for ($i = $type_param['start']; $i <= $type_param['end']; $i++) {
            ...
            if (isset(Tokens::$textStringTokens[$this->tokens[$i]['code']])) {
                $content_first = \trim(TextStrings::stripQuotes($this->tokens[$i]['content']));
         ...
         */
        if (\false === $gmt_true) {
            $this->phpcsFile->addWarning('Calling current_time() with a $type of "timestamp" or "U" is strongly discouraged as it will not return a Unix (UTC) timestamp. Please consider using a non-timestamp format or otherwise refactoring this code.', $stackPtr, 'Requested');
            return;
        }
        /*
