...
final class RestrictedFunctionsSniff extends AbstractFunctionRestrictionsSniff
{
    ...
    public function getGroups()
    {
        return array(
             ...
             */
            'timezone_change' => array('type' => 'error', 'message' => 'Using %s() and similar isn\'t allowed, instead use WP internal timezone support.', 'functions' => array('date_default_timezone_set')),
            /*
             * Use gmdate(), not date().
             ...
             */
            'date' => array('type' => 'error', 'message' => '%s() is affected by runtime timezone changes which can cause date/time to be incorrectly displayed. Use gmdate() instead.', 'functions' => array('date')),
        );
    }
}
