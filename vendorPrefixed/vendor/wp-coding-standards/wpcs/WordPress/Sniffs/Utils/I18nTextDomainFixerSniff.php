 ...
 */
namespace BitPayVendor\WordPressCS\WordPress\Sniffs\Utils;
...
final class I18nTextDomainFixerSniff extends AbstractFunctionParameterSniff
{
     ...
     */
    private $theme_headers = array('Theme Name' => \true, 'Theme URI' => \false, 'Author' => \true, 'Author URI' => \false, 'Description' => \true, 'Version' => \true, 'Requires at least' => \true, 'Tested up to' => \true, 'Requires PHP' => \true, 'License' => \true, 'License URI' => \true, 'Text Domain' => \true, 'Tags' => \false, 'Domain Path' => \false);
    /**
     ...
     *            required (true) or optional (false) header.
     */
    private $plugin_headers = array('Plugin Name' => \true, 'Plugin URI' => \false, 'Description' => \false, 'Version' => \false, 'Requires at least' => \false, 'Requires PHP' => \false, 'Author' => \false, 'Author URI' => \false, 'License' => \false, 'License URI' => \false, 'Text Domain' => \false, 'Domain Path' => \false, 'Network' => \false, 'Update URI' => \false);
    /**
    ...
    {
        $headers = \array_map('preg_quote', \array_keys($this->theme_headers), \array_fill(0, \count($this->theme_headers), '`'));
        $this->theme_header_regex = \sprintf($this->header_regex_template, \implode('|', $headers));
        $headers = \array_map('preg_quote', \array_keys($this->plugin_headers), \array_fill(0, \count($this->plugin_headers), '`'));
    ...
    public function process_token($stackPtr)
    {
        ...
        if (!\is_string($this->new_text_domain) || '' === $this->new_text_domain) {
        ...
        if (isset($this->old_text_domain)) {
            ...
            if (!\is_array($this->old_text_domain) || array() === $this->old_text_domain) {
                return $this->phpcsFile->numTokens + 1;
            }
        }
        ...
        if ($this->new_text_domain !== $this->validated_textdomain) {
            ...
            if ('default' === $this->new_text_domain) {
                $this->phpcsFile->addWarning('The "default" text domain is reserved for WordPress core use and should not be used by plugins or themes', 0, 'ReservedNewDomain', array($this->new_text_domain));
                return $this->phpcsFile->numTokens + 1;
            }
            if (\preg_match('`^[a-z0-9-]+$`', $this->new_text_domain) !== 1) {
                $this->phpcsFile->addWarning('The text domain should be a simple lowercase text string with words separated by dashes. "%s" appears invalid', 0, 'InvalidNewDomain', array($this->new_text_domain));
                return $this->phpcsFile->numTokens + 1;
            }
        ...
        if (\T_DOC_COMMENT_OPEN_TAG === $this->tokens[$stackPtr]['code'] || \T_COMMENT === $this->tokens[$stackPtr]['code']) {
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if (\false === $found_param && 1 !== $target_param_specs['position']) {
            ...
            if (\false === $has_named_params && isset($parameters[$target_param_specs['position'] - 1])) {
                ...
                if (\true === $fix) {
                    ...
                    if (\T_WHITESPACE === $this->tokens[$start_previous]['code'] && $this->tokens[$start_previous]['content'] === $this->phpcsFile->eolChar) {
        ...
        if (!\in_array($old_domain, $this->old_text_domain, \true)) {
            // Not a text domain targetted for replacement, ignore.
            return;
        }
        $fix = $this->phpcsFile->addFixableError('Mismatched text domain. Expected \'%s\' but found \'%s\'', $domain_token, 'TextDomainMismatch', array($this->new_text_domain, $old_domain));
        if (\true === $fix) {
    ...
    public function process_no_parameters($stackPtr, $group_name, $matched_content)
    {
        ...
        $opener = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if (\T_OPEN_PARENTHESIS !== $this->tokens[$opener]['code'] || isset($this->tokens[$opener]['parenthesis_closer']) === \false) {
        ...
        if (\true === $fix) {
            ...
            if ($this->tokens[$opener]['line'] !== $this->tokens[$closer]['line']) {
                ...
                $addBefore = $closer - 1;
                if (\T_WHITESPACE === $this->tokens[$closer - 1]['code'] && $this->tokens[$closer - 1]['line'] === $this->tokens[$closer]['line']) {
                    if (isset($this->tokens[$closer - 1]['orig_content'])) {
                        $replacement = $this->tokens[$closer - 1]['orig_content'] . "\t" . $replacement;
                    } else {
                        $replacement = $this->tokens[$closer - 1]['content'] . \str_repeat(' ', $this->tab_width) . $replacement;
                    }
                    --$addBefore;
                } else {
    ...
    {
        if (\true === $this->header_found && !\defined('PHP_CODESNIFFER_IN_TESTS')) {
            return;
        }
        ...
        $comment_details = array('required_header_found' => \false, 'headers_found' => 0, 'text_domain_ptr' => \false, 'text_domain_found' => '', 'last_header_ptr' => \false, 'last_header_matches' => array());
        if (\T_COMMENT === $this->tokens[$stackPtr]['code']) {
            ...
            $current = $stackPtr;
            do {
                if (\false === $comment_details['text_domain_ptr'] || \false === $comment_details['required_header_found'] || $comment_details['headers_found'] < 3) {
                    $comment_details = $this->examine_comment_line($current, $regex, $headers, $comment_details);
                }
                if (\true === $block_comment && \substr($this->tokens[$current]['content'], -2) === '*/') {
                    ++$current;
                    break;
                }
                ++$current;
            ...
            while (($current = $this->phpcsFile->findNext(\T_DOC_COMMENT_STRING, $current + 1, $closer)) !== \false) {
                $comment_details = $this->examine_comment_line($current, $regex, $headers, $comment_details);
                if (\false !== $comment_details['text_domain_ptr'] && \true === $comment_details['required_header_found'] && $comment_details['headers_found'] >= 3) {
        ...
        if (\true === $comment_details['required_header_found'] && $comment_details['headers_found'] >= 3) {
            $this->header_found = \true;
            ...
            if (\false !== $text_domain_ptr) {
                if ($this->new_text_domain !== $text_domain_found && \in_array($text_domain_found, $this->old_text_domain, \true)) {
                    $fix = $this->phpcsFile->addFixableError('Mismatched text domain in %s header. Expected \'%s\' but found \'%s\'', $text_domain_ptr, 'TextDomainHeaderMismatch', array($type, $this->new_text_domain, $text_domain_found));
                    if (\true === $fix) {
                ...
                $fix = $this->phpcsFile->addFixableError('Missing "Text Domain" in %s header', $last_header_ptr, 'MissingTextDomainHeader', array($type));
                if (\true === $fix) {
                    ...
                    if (\T_DOC_COMMENT_OPEN_TAG === $this->tokens[$stackPtr]['code']) {
                        ...
                        $replacement = $this->phpcsFile->eolChar . $this->phpcsFile->getTokensAsString($i, $last_header_ptr - $i, \true) . $replacement;
                    }
    ...
    protected function examine_comment_line($stackPtr, $regex, $headers, $comment_details)
    {
        if (\preg_match($regex, $this->tokens[$stackPtr]['content'], $matches) === 1) {
            ...
            if (\true === $headers[$matches[1]]) {
                $comment_details['required_header_found'] = \true;
            }
            if ('Text Domain' === $matches[1]) {
                $comment_details['text_domain_ptr'] = $stackPtr;
                $comment_details['text_domain_found'] = \trim($matches[2]);
            }
