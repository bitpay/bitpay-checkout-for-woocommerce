...
final class ValidPostTypeSlugSniff extends AbstractFunctionParameterSniff
{
     ...
     */
    protected $target_functions = array('register_post_type' => \true);
    /**
     ...
     * @var array
     */
    protected $reserved_names = array('post' => \true, 'page' => \true, 'attachment' => \true, 'revision' => \true, 'nav_menu_item' => \true, 'custom_css' => \true, 'customize_changeset' => \true, 'oembed_cache' => \true, 'user_request' => \true, 'wp_block' => \true, 'action' => \true, 'author' => \true, 'order' => \true, 'theme' => \true);
    /**
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if (\false === $post_type_param || '' === $post_type_param['clean']) {
            ...
            $this->phpcsFile->addError('register_post_type() called without a post type slug. The slug must be a non-empty string.', \false === $post_type_param ? $stackPtr : $post_type_param['start'], 'Empty');
            return;
        }
        ...
        $has_invalid_tokens = $this->phpcsFile->findNext($this->valid_tokens, $post_type_param['start'], $post_type_param['end'] + 1, \true);
        if (\false !== $has_invalid_tokens || \false === $string_pos) {
            ...
            $this->phpcsFile->addWarning('The post type slug is not a string literal. It is not possible to automatically determine the validity of this slug. Found: %s.', $stackPtr, 'NotStringLiteral', array($post_type_param['clean']), 3);
            return;
        }
        ...
        if ('T_DOUBLE_QUOTED_STRING' === $this->tokens[$string_pos]['type'] || 'T_HEREDOC' === $this->tokens[$string_pos]['type'] && \strpos($this->tokens[$string_pos]['content'], '$') !== \false) {
            $this->phpcsFile->addWarning('The post type slug may, or may not, get too long with dynamic contents and could contain invalid characters. Found: "%s".', $string_pos, 'PartiallyDynamic', $data);
        ...
        if (\preg_match(self::VALID_POST_TYPE_CHARACTERS, $post_type) === 0) {
            ...
            $this->phpcsFile->addError('register_post_type() called with invalid post type "%s". Post type contains invalid characters. Only lowercase alphanumeric characters, dashes, and underscores are allowed.', $string_pos, 'InvalidCharacters', $data);
        }
        if (isset($this->reserved_names[$post_type])) {
            // Error for using reserved slug names.
            $this->phpcsFile->addError('register_post_type() called with reserved post type "%s". Reserved post types should not be used as they interfere with the functioning of WordPress itself.', $string_pos, 'Reserved', $data);
        } elseif (\stripos($post_type, 'wp_') === 0) {
            // Error for using reserved slug prefix.
            $this->phpcsFile->addError('The post type passed to register_post_type() uses a prefix reserved for WordPress itself. Found: "%s".', $string_pos, 'ReservedPrefix', $data);
        }
        // Error for slugs that are too long.
        if (\strlen($post_type) > self::POST_TYPE_MAX_LENGTH) {
            $this->phpcsFile->addError('A post type slug must not exceed %d characters. Found: "%s" (%d characters).', $string_pos, 'TooLong', array(self::POST_TYPE_MAX_LENGTH, $post_type, \strlen($post_type)));
        }
    }
}
