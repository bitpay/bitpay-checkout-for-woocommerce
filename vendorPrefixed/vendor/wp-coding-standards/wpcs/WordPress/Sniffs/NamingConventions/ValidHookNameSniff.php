    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        for ($i = $hook_name_param['start']; $i <= $hook_name_param['end']; $i++) {
            ...
            if (\T_VARIABLE === $this->tokens[$i]['code']) {
                do {
                    $open_bracket = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
                    if (\false === $open_bracket || \T_OPEN_SQUARE_BRACKET !== $this->tokens[$open_bracket]['code'] || !isset($this->tokens[$open_bracket]['bracket_closer'])) {
                        $last_non_empty = $i;
                        continue 2;
                    }
            ...
            if (\T_OPEN_PARENTHESIS === $this->tokens[$i]['code'] && (\T_STRING === $this->tokens[$last_non_empty]['code'] || \T_VARIABLE === $this->tokens[$last_non_empty]['code']) && isset($this->tokens[$i]['parenthesis_closer'])) {
        ...
        $first_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $hook_name_param['start'], $hook_name_param['end'] + 1, \true);
        $data = array(\trim(\implode('', $expected)), \trim(\implode('', $content)));
        if ($case_errors > 0) {
