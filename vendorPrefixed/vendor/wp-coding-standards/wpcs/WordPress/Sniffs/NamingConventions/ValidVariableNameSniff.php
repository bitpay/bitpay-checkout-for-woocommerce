...
final class ValidVariableNameSniff extends PHPCS_AbstractVariableSniff
{
     ...
     * @var array
     */
    protected $wordpress_mixed_case_vars = array('EZSQL_ERROR' => \true, 'GETID3_ERRORARRAY' => \true, 'is_IE' => \true, 'is_IIS' => \true, 'is_macIE' => \true, 'is_NS4' => \true, 'is_winIE' => \true, 'PHP_SELF' => \true, 'post_ID' => \true, 'tag_ID' => \true, 'user_ID' => \true);
    /**
     ...
     * @var array
     */
    protected $allowed_mixed_case_member_var_names = array('cat_ID' => \true, 'comment_ID' => \true, 'comment_author_IP' => \true, 'comment_post_ID' => \true, 'ID' => \true, 'post_ID' => \true);
    /**
    ...
    protected function processVariable(File $phpcsFile, $stackPtr)
    {
        ...
        $obj_operator = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if (\T_OBJECT_OPERATOR === $tokens[$obj_operator]['code'] || \T_NULLSAFE_OBJECT_OPERATOR === $tokens[$obj_operator]['code']) {
            // Check to see if we are using a variable from an object.
            $var = $phpcsFile->findNext(Tokens::$emptyTokens, $obj_operator + 1, null, \true);
            if (\T_STRING === $tokens[$var]['code']) {
                $bracket = $phpcsFile->findNext(Tokens::$emptyTokens, $var + 1, null, \true);
                if (\T_OPEN_PARENTHESIS !== $tokens[$bracket]['code']) {
                    ...
                    if ($suggested_name !== $obj_var_name) {
                        ...
                        $data = array($obj_var_name, $suggested_name);
        ...
        $in_class = \false;
        if (ContextHelper::has_object_operator_before($phpcsFile, $stackPtr) === \true) {
        ...
        if ($suggested_name !== $var_name) {
            ...
            if (isset($error, $error_name)) {
                $data = array($var_name, $suggested_name);
    ...
    protected function merge_allow_lists()
    {
        if ($this->allowed_custom_properties !== $this->addedCustomProperties['properties']) {
            ...
            $this->allowed_mixed_case_member_var_names = RulesetPropertyHelper::merge_custom_array($customProperties, $this->allowed_mixed_case_member_var_names);
