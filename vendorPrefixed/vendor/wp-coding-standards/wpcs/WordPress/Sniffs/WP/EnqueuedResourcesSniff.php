...
final class EnqueuedResourcesSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\preg_match_all('# rel=\\\\?[\'"]?stylesheet\\\\?[\'"]?#', $content, $matches, \PREG_OFFSET_CAPTURE) > 0) {
            foreach ($matches[0] as $match) {
                $this->phpcsFile->addError('Stylesheets must be registered/enqueued via wp_enqueue_style()', $this->find_token_in_multiline_string($stackPtr, $content, $match[1]), 'NonEnqueuedStylesheet');
            }
        }
        if (\preg_match_all('#<script[^>]*(?<=src=)#', $content, $matches, \PREG_OFFSET_CAPTURE) > 0) {
            foreach ($matches[0] as $match) {
                $this->phpcsFile->addError('Scripts must be registered/enqueued via wp_enqueue_script()', $this->find_token_in_multiline_string($stackPtr, $content, $match[1]), 'NonEnqueuedScript');
            }
        }
        return $end_ptr + 1;
    }
    ...
    private function find_token_in_multiline_string($stackPtr, $content, $match_offset)
    {
        ...
        if (isset(Tokens::$heredocTokens[$this->tokens[$stackPtr]['code']]) === \true) {
            ++$newline_count;
        }
