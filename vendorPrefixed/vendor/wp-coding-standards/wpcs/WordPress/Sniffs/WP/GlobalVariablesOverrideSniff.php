...
final class GlobalVariablesOverrideSniff extends Sniff
{
     ...
     * @var array
     */
    protected $override_allowed = array('content_width' => \true, 'wp_cockneyreplace' => \true);
    /**
    ...
    {
        $targets = array(\T_GLOBAL, \T_VARIABLE);
    ...
    public function process_token($stackPtr)
    {
        ...
        if (isset(Tokens::$ooScopeTokens[$token['code']])) {
            if (\true === $this->is_test_class($this->phpcsFile, $stackPtr) && $token['scope_condition'] === $stackPtr && isset($token['scope_closer'])) {
        ...
        if (isset(Collections::listOpenTokensBC()[$token['code']]) && \false === $in_function_scope && \false === $this->treat_files_as_scoped) {
            return $this->process_list_assignment($stackPtr);
        } elseif (\T_VARIABLE === $token['code'] && ('$GLOBALS' === $token['content'] || \false === $in_function_scope && \false === $this->treat_files_as_scoped)) {
            return $this->process_variable_assignment($stackPtr);
        } elseif (\T_GLOBAL === $token['code'] && (\true === $in_function_scope || \true === $this->treat_files_as_scoped)) {
    ...
    {
        $token = $this->tokens[$stackPtr];
        $var_name = \substr($token['content'], 1);
        // Strip the dollar sign.
        $data = array();
        // Determine the variable name for `$GLOBALS['array_key']`.
        if ('GLOBALS' === $var_name) {
            $bracketPtr = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
            ...
            for ($ptr = $start; $ptr < $this->tokens[$bracketPtr]['bracket_closer']; $ptr++) {
                 ...
                 */
                if (\T_VARIABLE === $this->tokens[$ptr]['code'] || \T_STRING === $this->tokens[$ptr]['code'] || \T_DOUBLE_QUOTED_STRING === $this->tokens[$ptr]['code']) {
                    return;
                }
         ...
         */
        if (isset($this->override_allowed[$var_name]) === \true) {
            return;
        }
        /*
    ...
    protected function process_global_statement($stackPtr, $in_function_scope)
    {
        ...
        while (isset($this->tokens[$ptr]) && $ptr < $end_of_statement) {
            if (\T_VARIABLE === $this->tokens[$ptr]['code']) {
                ...
                if (WPGlobalVariablesHelper::is_wp_global($var_name) && isset($this->override_allowed[$var_name]) === \false) {
                    $search[$this->tokens[$ptr]['content']] = \true;
                }
            }
            ++$ptr;
        }
        ...
        for ($ptr = $start; $ptr < $end; $ptr++) {
            ...
            if (ContextHelper::has_object_operator_before($this->phpcsFile, $ptr) === \true) {
                continue;
            }
            if (\true === VariableHelper::is_assignment($this->phpcsFile, $ptr)) {
                $this->add_error($ptr);
                continue;
            }
    ...
    protected function add_error($stackPtr, $data = array())
    {
        ...
        $this->phpcsFile->addError('Overriding WordPress globals is prohibited. Found assignment to %s', $stackPtr, 'Prohibited', $data);
    }
}
