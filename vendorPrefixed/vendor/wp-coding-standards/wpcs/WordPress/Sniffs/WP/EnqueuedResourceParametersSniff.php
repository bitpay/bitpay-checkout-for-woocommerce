...
final class EnqueuedResourceParametersSniff extends AbstractFunctionParameterSniff
{
     ...
     */
    protected $target_functions = array('wp_register_script' => \true, 'wp_enqueue_script' => \true, 'wp_register_style' => \true, 'wp_enqueue_style' => \true);
    /**
     ...
     * @var array
     */
    private $false_tokens = array(\T_FALSE => \T_FALSE);
    /**
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if (\false !== $version_param) {
            $error_ptr = $this->phpcsFile->findNext(Tokens::$emptyTokens, $version_param['start'], $version_param['end'] + 1, \true);
            if (\false === $error_ptr) {
        ...
        if (\false === $version_param || 'null' === $version_param['clean']) {
            ...
            $this->phpcsFile->addWarning('Resource version not set in call to %s(). This means new versions of the %s may not always be loaded due to browser caching.', $error_ptr, 'MissingVersion', array($matched_content, $type));
            // The version argument should have a non-false value.
        } elseif ($this->is_falsy($version_param['start'], $version_param['end'])) {
            $this->phpcsFile->addError('Version parameter is not explicitly set or has been set to an equivalent of "false" for %s; ' . 'This means that the WordPress core version will be used which is not recommended for plugin or theme development.', $error_ptr, 'NoExplicitVersion', array($matched_content));
        }
        /*
         * In footer Check
         *
        ...
        if (\false === $infooter_param) {
            ...
            $this->phpcsFile->addWarning('In footer ($in_footer) is not set explicitly %s; ' . 'It is recommended to load scripts in the footer. Please set this value to `true` to load it in the footer, or explicitly `false` if it should be loaded in the header.', $stackPtr, 'NotInFooter', array($matched_content));
        }
    }
    /**
    ...
    protected function is_falsy($start, $end)
    {
        ...
        $has_non_false = $this->phpcsFile->findNext($this->false_tokens, $start, $end + 1, \true);
