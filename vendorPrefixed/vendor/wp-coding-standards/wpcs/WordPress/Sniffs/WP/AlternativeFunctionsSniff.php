...
final class AlternativeFunctionsSniff extends AbstractFunctionRestrictionsSniff
{
     ...
     * @var array
     */
    private $allowed_local_streams = array('php://input' => \true, 'php://output' => \true, 'php://stdin' => \true, 'php://stdout' => \true, 'php://stderr' => \true);
    /**
     ...
     * @var array
     */
    private $allowed_local_stream_partials = array('php://temp/', 'php://fd/');
    /**
     ...
     * @var array
     */
    private $allowed_local_stream_constants = array('STDIN' => \true, 'STDOUT' => \true, 'STDERR' => \true);
    /**
    ...
    public function process_matched_token($stackPtr, $group_name, $matched_content)
    {
        ...
        switch ($matched_content) {
            ...
            case 'parse_url':
                ...
                if (\false !== $has_component && $this->wp_version_compare($this->minimum_wp_version, '4.7.0', '<')) {
                    return;
                }
                unset($has_component);
                break;
            case 'file_get_contents':
                ...
                if (\strpos($filename_param['clean'], 'http:') !== \false || \strpos($filename_param['clean'], 'https:') !== \false) {
                    // Definitely a URL, throw notice.
                    break;
                }
                $contains_wp_path_constant = \preg_match('`\\b(?:ABSPATH|WP_(?:CONTENT|PLUGIN)_DIR|WPMU_PLUGIN_DIR|TEMPLATEPATH|STYLESHEETPATH|(?:MU)?PLUGINDIR)\\b`', $filename_param['clean']);
                if (1 === $contains_wp_path_constant) {
                    // Using any of the constants matched in this regex is an indicator of a local file.
                    return;
                }
                $contains_wp_path_function_call = \preg_match('`(?:get_home_path|plugin_dir_path|get_(?:stylesheet|template)_directory|wp_upload_dir)\\s*\\(`i', $filename_param['clean']);
                if (1 === $contains_wp_path_function_call) {
                    // Using any of the functions matched in the regex is an indicator of a local file.
                    return;
                }
                if ($this->is_local_data_stream($filename_param['clean']) === \true) {
                    // Local data stream.
                    return;
                }
    ...
    protected function is_local_data_stream($clean_param_value)
    {
        ...
        if (isset($this->allowed_local_streams[$stripped]) || isset($this->allowed_local_stream_constants[$clean_param_value])) {
            return \true;
        }
