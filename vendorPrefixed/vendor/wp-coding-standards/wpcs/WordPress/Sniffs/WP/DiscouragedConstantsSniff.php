...
final class DiscouragedConstantsSniff extends AbstractFunctionParameterSniff
{
     ...
     */
    protected $target_functions = array('define' => array('position' => 1, 'name' => 'constant_name'));
    /**
    ...
    public function process_arbitrary_tstring($stackPtr)
    {
        ...
        $this->phpcsFile->addWarning('Found usage of constant "%s". Use %s instead.', $stackPtr, MessageHelper::stringToErrorcode($content . 'UsageFound'), array($content, $this->discouraged_constants[$content]));
    }
    /**
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if (isset($this->discouraged_constants[$clean_content])) {
            $first_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $found_param['start'], $found_param['end'] + 1, \true);
            $this->phpcsFile->addWarning('Found declaration of constant "%s". Use %s instead.', $first_non_empty, MessageHelper::stringToErrorcode($clean_content . 'DeclarationFound'), array($clean_content, $this->discouraged_constants[$clean_content]));
        }
    }
}
