...
final class CapitalPDangitSniff extends Sniff
{
     ...
     * @var array
     */
    private $comment_text_tokens = array(\T_DOC_COMMENT => \T_DOC_COMMENT, \T_DOC_COMMENT_STRING => \T_DOC_COMMENT_STRING, \T_COMMENT => \T_COMMENT);
    /**
    ...
    public function process_token($stackPtr)
    {
         ...
         */
        if ((\T_ARRAY === $this->tokens[$stackPtr]['code'] || \T_LIST === $this->tokens[$stackPtr]['code']) && isset($this->tokens[$stackPtr]['parenthesis_closer'])) {
            return $this->tokens[$stackPtr]['parenthesis_closer'];
        }
        if ((\T_OPEN_SHORT_ARRAY === $this->tokens[$stackPtr]['code'] || \T_OPEN_SQUARE_BRACKET === $this->tokens[$stackPtr]['code']) && isset($this->tokens[$stackPtr]['bracket_closer'])) {
        ...
        if (\T_NAMESPACE === $this->tokens[$stackPtr]['code']) {
            ...
            foreach ($levels as $level) {
                if (\preg_match_all(self::WP_CLASSNAME_REGEX, $level, $matches, \PREG_PATTERN_ORDER) > 0) {
                    ...
                    if (!empty($misspelled)) {
                        $this->phpcsFile->addWarning('Please spell "WordPress" correctly. Found: "%s" as part of the namespace name.', $stackPtr, 'MisspelledNamespaceName', array(\implode(', ', $misspelled)));
                    }
                }
            }
            return;
        ...
        if (isset(Tokens::$ooScopeTokens[$this->tokens[$stackPtr]['code']])) {
            ...
            if (\preg_match_all(self::WP_CLASSNAME_REGEX, $classname, $matches, \PREG_PATTERN_ORDER) > 0) {
                $misspelled = $this->retrieve_misspellings($matches[1]);
                if (!empty($misspelled)) {
                    $this->phpcsFile->addWarning('Please spell "WordPress" correctly. Found: "%s" as part of the class/interface/trait/enum name.', $stackPtr, 'MisspelledClassName', array(\implode(', ', $misspelled)));
                }
            }
            return;
        }
        ...
        if (\T_DOC_COMMENT_STRING === $this->tokens[$stackPtr]['code'] || \T_DOC_COMMENT === $this->tokens[$stackPtr]['code']) {
            $comment_tag = $this->phpcsFile->findPrevious(array(\T_DOC_COMMENT_TAG, \T_DOC_COMMENT_OPEN_TAG), $stackPtr - 1);
            if (\false !== $comment_tag && \T_DOC_COMMENT_TAG === $this->tokens[$comment_tag]['code'] && '@link' === $this->tokens[$comment_tag]['content']) {
                // @link tag, so ignore.
                return;
            }
        }
        // Ignore constant declarations via define().
        if (ContextHelper::is_in_function_call($this->phpcsFile, $stackPtr, array('define' => \true), \true, \true)) {
            return;
        }
        // Ignore constant declarations using the const keyword.
        $stop_points = array(\T_CONST, \T_SEMICOLON, \T_OPEN_TAG, \T_CLOSE_TAG, \T_OPEN_CURLY_BRACKET);
        ...
        if (\preg_match_all(self::WP_REGEX, $content, $matches, \PREG_PATTERN_ORDER | \PREG_OFFSET_CAPTURE) > 0) {
            ...
            $fix = $this->phpcsFile->addFixableWarning('Please spell "WordPress" correctly. Found %s misspelling(s): %s', $stackPtr, $code, array(\count($misspelled), \implode(', ', $misspelled)));
            if (\true === $fix) {
