...
final class DeprecatedClassesSniff extends AbstractClassRestrictionsSniff
{
    ...
    private $deprecated_classes = array(
        // WP 3.1.0.
        'WP_User_Search' => array('alt' => 'WP_User_Query', 'version' => '3.1.0'),
        // WP 4.9.0.
        'Customize_New_Menu_Section' => array('version' => '4.9.0'),
        'WP_Customize_New_Menu_Control' => array('version' => '4.9.0'),
        // WP 5.3.0.
        'Services_JSON' => array('alt' => 'The PHP native JSON extension', 'version' => '5.3.0'),
    );
    /**
    ...
    public function getGroups()
    {
        ...
        return array('deprecated_classes' => array('classes' => \array_keys($this->deprecated_classes)));
    }
    /**
    ...
    public function process_matched_token($stackPtr, $group_name, $matched_content)
    {
        ...
        $data = array(\ltrim($matched_content, '\\'), $this->deprecated_classes[$class_name]['version']);
        if (!empty($this->deprecated_classes[$class_name]['alt'])) {
            $message .= ' Use %s instead.';
            $data[] = $this->deprecated_classes[$class_name]['alt'];
        }
        MessageHelper::addMessage($this->phpcsFile, $message, $stackPtr, $this->wp_version_compare($this->deprecated_classes[$class_name]['version'], $this->minimum_wp_version, '<'), MessageHelper::stringToErrorcode($class_name . 'Found'), $data);
    }
}
