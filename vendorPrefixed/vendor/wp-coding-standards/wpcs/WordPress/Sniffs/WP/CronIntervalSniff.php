...
final class CronIntervalSniff extends Sniff
{
     ...
     * @var array
     */
    protected $wp_time_constants = array('MINUTE_IN_SECONDS' => 60, 'HOUR_IN_SECONDS' => 3600, 'DAY_IN_SECONDS' => 86400, 'WEEK_IN_SECONDS' => 604800, 'MONTH_IN_SECONDS' => 2592000, 'YEAR_IN_SECONDS' => 31536000);
    /**
     * Function within which the hook should be found.
     *
     * @var array
     */
    protected $valid_functions = array('add_filter' => \true);
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        $callbackArrayPtr = $this->phpcsFile->findNext(Tokens::$emptyTokens, $callback['start'], $callback['end'] + 1, \true);
        // If callback is array, get second element.
        if (\false !== $callbackArrayPtr && (\T_ARRAY === $this->tokens[$callbackArrayPtr]['code'] || isset(Collections::shortArrayListOpenTokensBC()[$this->tokens[$callbackArrayPtr]['code']]) && Arrays::isShortArray($this->phpcsFile, $callbackArrayPtr) === \true)) {
        ...
        if (\T_CLOSURE === $this->tokens[$callbackFunctionPtr]['code'] || \T_FN === $this->tokens[$callbackFunctionPtr]['code']) {
            ...
            $before = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $callbackFunctionPtr - 1, null, \true);
            $after = $this->phpcsFile->findNext(Tokens::$emptyTokens, $callbackFunctionPtr + 1, null, \true);
            if (\false !== $before && \T_OPEN_PARENTHESIS === $this->tokens[$before]['code'] && (\false !== $after && \T_CLOSE_PARENTHESIS === $this->tokens[$after]['code'])) {
                // Ok, now see if we can find the function name.
                $beforeOpen = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $before - 1, null, \true);
        ...
        for ($i = $opening; $i <= $closing; $i++) {
            if (isset(Tokens::$stringTokens[$this->tokens[$i]['code']]) === \true) {
                if ('interval' === TextStrings::stripQuotes($this->tokens[$i]['content'])) {
                    $operator = $this->phpcsFile->findNext(\T_DOUBLE_ARROW, $i, null, \false, null, \true);
                    if (\false === $operator) {
                        $this->confused($stackPtr);
                        return;
                    }
                    $valueStart = $this->phpcsFile->findNext(Tokens::$emptyTokens, $operator + 1, null, \true, null, \true);
                    ...
                    for ($j = $valueStart; $j <= $valueEnd; $j++) {
                        ...
                        if (\T_LNUMBER === $this->tokens[$j]['code'] || \T_DNUMBER === $this->tokens[$j]['code']) {
        ...
        if (isset($interval) && $interval < $this->min_interval) {
            ...
            $this->phpcsFile->addWarning('Scheduling crons at %s sec ( less than %s minutes ) is discouraged.', $stackPtr, 'CronSchedulesInterval', array($interval, $minutes));
            return;
        }
    }
    /**
    ...
    {
        $this->phpcsFile->addWarning('Detected changing of cron_schedules, but could not detect the interval value.', $stackPtr, 'ChangeDetected');
    }
}
