...
final class ControlStructureSpacingSniff extends Sniff
{
     ...
     * @var boolean
     */
    public $blank_line_after_check = \true;
    /**
     ...
     * @var array
     */
    private $ignore_extra_space_after_open_paren = array(\T_FUNCTION => \true, \T_CLOSURE => \true, \T_DO => \true, \T_ELSE => \true, \T_TRY => \true);
    /**
    ...
    {
        return array(\T_IF, \T_WHILE, \T_FOREACH, \T_FOR, \T_SWITCH, \T_DO, \T_ELSE, \T_ELSEIF, \T_FUNCTION, \T_CLOSURE, \T_USE, \T_TRY, \T_CATCH);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\T_USE === $this->tokens[$stackPtr]['code'] && UseStatements::isClosureUse($this->phpcsFile, $stackPtr) === \false) {
            return;
        }
        if (isset($this->tokens[$stackPtr + 1]) && \T_WHITESPACE !== $this->tokens[$stackPtr + 1]['code'] && !(\T_ELSE === $this->tokens[$stackPtr]['code'] && \T_COLON === $this->tokens[$stackPtr + 1]['code']) && !(\T_CLOSURE === $this->tokens[$stackPtr]['code'] && 0 >= $this->spaces_before_closure_open_paren)) {
        ...
        $parenthesisOpener = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        ...
        if (\T_FUNCTION === $this->tokens[$stackPtr]['code']) {
            if (\T_STRING === $this->tokens[$parenthesisOpener]['code']) {
                ...
                $parenthesisOpener = $this->phpcsFile->findNext(Tokens::$emptyTokens, $parenthesisOpener + 1, null, \true);
                $function_name_ptr = $parenthesisOpener;
            }
            if (isset($function_name_ptr)) {
                ...
                if ($function_name_ptr + 1 !== $parenthesisOpener) {
                    ...
                    $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'SpaceBeforeFunctionOpenParenthesis', array($this->tokens[$function_name_ptr + 1]['content']));
                    if (\true === $fix) {
            ...
            if (isset($this->tokens[$parenthesisOpener]['parenthesis_closer'])) {
                $usePtr = $this->phpcsFile->findNext(Tokens::$emptyTokens, $this->tokens[$parenthesisOpener]['parenthesis_closer'] + 1, null, \true, null, \true);
        ...
        if (\T_COLON !== $this->tokens[$parenthesisOpener]['code'] && \T_FUNCTION !== $this->tokens[$stackPtr]['code']) {
            if (\T_CLOSURE === $this->tokens[$stackPtr]['code'] && 0 === $this->spaces_before_closure_open_paren) {
            ...
            } elseif ((\T_CLOSURE !== $this->tokens[$stackPtr]['code'] || 1 === $this->spaces_before_closure_open_paren) && $stackPtr + 1 === $parenthesisOpener) {
        ...
        if (\T_WHITESPACE === $this->tokens[$stackPtr + 1]['code'] && ' ' !== $this->tokens[$stackPtr + 1]['content']) {
            ...
            $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'ExtraSpaceBeforeOpenParenthesis', array($this->tokens[$stackPtr + 1]['content']));
            if (\true === $fix) {
                $this->phpcsFile->fixer->replaceToken($stackPtr + 1, ' ');
            }
        }
        if (\T_CLOSE_PARENTHESIS !== $this->tokens[$parenthesisOpener + 1]['code']) {
            if (\T_WHITESPACE !== $this->tokens[$parenthesisOpener + 1]['code']) {
            ...
            } elseif (' ' !== $this->tokens[$parenthesisOpener + 1]['content'] && "\n" !== $this->tokens[$parenthesisOpener + 1]['content'] && "\r\n" !== $this->tokens[$parenthesisOpener + 1]['content'] && !isset($this->ignore_extra_space_after_open_paren[$this->tokens[$stackPtr]['code']])) {
                ...
                $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'ExtraSpaceAfterOpenParenthesis', array($this->tokens[$parenthesisOpener + 1]['content']));
                if (\true === $fix) {
        ...
        if (isset($this->tokens[$parenthesisOpener]['parenthesis_closer'])) {
            ...
            if (\T_CLOSE_PARENTHESIS !== $this->tokens[$parenthesisOpener + 1]['code']) {
                ...
                if (\T_WHITESPACE !== $this->tokens[$parenthesisCloser - 1]['code']) {
                    ...
                    $prevNonEmpty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $parenthesisCloser - 1, null, \true);
                    if ($this->tokens[$parenthesisCloser]['line'] === $this->tokens[$prevNonEmpty + 1]['line']) {
                        ...
                        $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'ExtraSpaceBeforeCloseParenthesis', array($this->tokens[$parenthesisCloser - 1]['content']));
                        if (\true === $fix) {
                            $this->phpcsFile->fixer->replaceToken($parenthesisCloser - 1, ' ');
                        }
                    }
                }
                if (\T_WHITESPACE !== $this->tokens[$parenthesisCloser + 1]['code'] && !(\T_COLON === $this->tokens[$parenthesisCloser + 1]['code'] && (isset($this->tokens[$parenthesisCloser]['parenthesis_owner']) === \false || \in_array($this->tokens[$this->tokens[$parenthesisCloser]['parenthesis_owner']]['code'], array(\T_FUNCTION, \T_CLOSURE), \true))) && (isset($scopeOpener) && \T_COLON !== $this->tokens[$scopeOpener]['code'])) {
            ...
            if (\T_FUNCTION !== $this->tokens[$stackPtr]['code'] && \T_CLOSURE !== $this->tokens[$stackPtr]['code'] && isset($this->tokens[$parenthesisOpener]['parenthesis_owner']) && (isset($scopeOpener) && $this->tokens[$parenthesisCloser]['line'] !== $this->tokens[$scopeOpener]['line'])) {
                ...
                return;
            } elseif (\T_WHITESPACE === $this->tokens[$parenthesisCloser + 1]['code'] && ' ' !== $this->tokens[$parenthesisCloser + 1]['content']) {
                ...
                $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'ExtraSpaceAfterCloseParenthesis', array($this->tokens[$parenthesisCloser + 1]['content']));
                if (\true === $fix) {
                    $this->phpcsFile->fixer->replaceToken($parenthesisCloser + 1, ' ');
                }
            }
        ...
        if (\false !== $this->blank_line_check && isset($scopeOpener)) {
            $firstContent = $this->phpcsFile->findNext(\T_WHITESPACE, $scopeOpener + 1, null, \true);
            // We ignore spacing for some structures that tend to have their own rules.
            $ignore = array(\T_FUNCTION => \true, \T_CLOSURE => \true, \T_DOC_COMMENT_OPEN_TAG => \true, \T_CLOSE_TAG => \true, \T_COMMENT => \true);
            $ignore += Tokens::$ooScopeTokens;
            if (!isset($ignore[$this->tokens[$firstContent]['code']]) && $this->tokens[$firstContent]['line'] > $this->tokens[$scopeOpener]['line'] + 1) {
            ...
            if ($firstContent !== $scopeCloser) {
                $lastContent = $this->phpcsFile->findPrevious(\T_WHITESPACE, $scopeCloser - 1, null, \true);
                $lastNonEmptyContent = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $scopeCloser - 1, null, \true);
                ...
                if (!isset($ignore[$this->tokens[$checkToken]['code']]) && $this->tokens[$lastContent]['line'] <= $this->tokens[$scopeCloser]['line'] - 2) {
                    for ($i = $scopeCloser - 1; $i > $lastContent; $i--) {
                        if ($this->tokens[$i]['line'] < $this->tokens[$scopeCloser]['line'] && \T_OPEN_TAG !== $this->tokens[$firstContent]['code']) {
                            ...
                            if (\true === $fix) {
                                 ...
                                 * conflict.
                                 */
                                if (\T_COMMENT !== $this->tokens[$lastContent]['code'] && !isset(Tokens::$phpcsCommentTokens[$this->tokens[$lastContent]['code']])) {
        ...
        if (!isset($scopeCloser) || \true !== $this->blank_line_after_check) {
            return;
        }
        // {@internal This is just for the blank line check. Only whitespace should be considered,
        // not "other" empty tokens.}}
        $trailingContent = $this->phpcsFile->findNext(\T_WHITESPACE, $scopeCloser + 1, null, \true);
        if (\false === $trailingContent) {
            return;
        }
        if (\T_COMMENT === $this->tokens[$trailingContent]['code'] || isset(Tokens::$phpcsCommentTokens[$this->tokens[$trailingContent]['code']])) {
            // Special exception for code where the comment about
            // an ELSE or ELSEIF is written between the control structures.
            $nextCode = $this->phpcsFile->findNext(Tokens::$emptyTokens, $scopeCloser + 1, null, \true);
            ...
            if ($this->tokens[$trailingContent]['line'] === $this->tokens[$scopeCloser]['line']) {
                if (\preg_match('`^//[ ]?end`i', $this->tokens[$trailingContent]['content'], $matches) > 0) {
                    ...
                    $trailingContent = $this->phpcsFile->findNext(\T_WHITESPACE, $trailingContent + 1, null, \true);
                }
            }
        }
        ...
        if (isset($this->tokens[$trailingContent]['scope_condition']) && \T_CLOSE_CURLY_BRACKET === $this->tokens[$trailingContent]['code']) {
            // Another control structure's closing brace.
            $owner = $this->tokens[$trailingContent]['scope_condition'];
            if (\in_array($this->tokens[$owner]['code'], array(\T_FUNCTION, \T_CLOSURE, \T_CLASS, \T_ANON_CLASS, \T_INTERFACE, \T_TRAIT), \true)) {
            ...
            if ($this->tokens[$scopeCloser]['line'] + 1 !== $this->tokens[$trailingContent]['line']) {
                ...
                if (\true === $fix) {
                    ...
                    if (\T_COMMENT !== $this->tokens[$scopeCloser]['code'] && isset(Tokens::$phpcsCommentTokens[$this->tokens[$scopeCloser]['code']]) === \false) {
