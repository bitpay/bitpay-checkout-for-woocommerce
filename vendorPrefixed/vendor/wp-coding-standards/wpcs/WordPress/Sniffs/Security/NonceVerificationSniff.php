...
class NonceVerificationSniff extends Sniff
{
     ...
     * @var array
     */
    protected $superglobals = array('$_POST' => \true, '$_FILES' => \true, '$_GET' => \false, '$_REQUEST' => \false);
    /**
     ...
     * @var array
     */
    private $nonceVerificationFunctions = array('wp_verify_nonce' => \true, 'check_admin_referer' => \true, 'check_ajax_referer' => \true);
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        $cache_keys = array('file' => $this->phpcsFile->getFilename(), 'start' => 0, 'end' => $stackPtr);
        ...
        MessageHelper::addMessage($this->phpcsFile, 'Processing form data without nonce verification.', $stackPtr, $this->superglobals[$this->tokens[$stackPtr]['content']], $error_code);
    }
    /**
    ...
    protected function needs_nonce_check($stackPtr, array $cache_keys)
    {
        ...
        $needs_nonce = 'before';
        if (ContextHelper::is_in_isset_or_empty($this->phpcsFile, $stackPtr) || ContextHelper::is_in_type_test($this->phpcsFile, $stackPtr) || VariableHelper::is_comparison($this->phpcsFile, $stackPtr) || VariableHelper::is_assignment($this->phpcsFile, $stackPtr, \true) || ContextHelper::is_in_array_comparison($this->phpcsFile, $stackPtr) || ContextHelper::is_in_function_call($this->phpcsFile, $stackPtr, UnslashingFunctionsHelper::get_functions()) !== \false || $this->is_only_sanitized($stackPtr)) {
            $needs_nonce = 'after';
        }
        return $needs_nonce;
    }
    ...
    private function has_nonce_check($stackPtr, array $cache_keys, $allow_nonce_after = \false)
    {
        ...
        if (\false !== $current_cache['nonce']) {
            ...
            return \true === $allow_nonce_after || $current_cache['nonce'] < $stackPtr;
        ...
        for ($i = $search_start; $i < $end; $i++) {
            ...
            if (isset(Collections::closedScopes()[$this->tokens[$i]['code']]) || \T_FN === $this->tokens[$i]['code']) {
            ...
            if (isset($this->nonceVerificationFunctions[$this->tokens[$i]['content']])) {
                 ...
                 */
                if (ContextHelper::has_object_operator_before($this->phpcsFile, $i) === \true) {
                    continue;
                }
                if (ContextHelper::is_token_namespaced($this->phpcsFile, $i) === \true) {
                    continue;
                }
    ...
    {
        if (\is_array($this->cached_results) && $filename === $this->cached_results['file'] && isset($this->cached_results['cache'][$start])) {
            return $this->cached_results['cache'][$start];
        }
        return array('end' => 0, 'nonce' => \false);
    }
    /**
    ...
    {
        if (\is_array($this->cached_results) === \false || $filename !== $this->cached_results['file']) {
            $this->cached_results = array('file' => $filename, 'cache' => array($start => array('end' => $end, 'nonce' => $nonce)));
            return;
        }
        ...
        if (isset($this->cached_results['cache'][$start]) === \false) {
            $this->cached_results['cache'][$start] = array('end' => $end, 'nonce' => $nonce);
            return;
        }
    ...
    protected function mergeFunctionLists()
    {
        if ($this->customNonceVerificationFunctions !== $this->addedCustomNonceFunctions) {
            $this->nonceVerificationFunctions = RulesetPropertyHelper::merge_custom_array($this->customNonceVerificationFunctions, $this->nonceVerificationFunctions);
