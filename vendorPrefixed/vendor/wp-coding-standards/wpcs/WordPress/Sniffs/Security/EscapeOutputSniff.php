...
use BitPayVendor\WordPressCS\WordPress\Helpers\ConstantsHelper;
use BitPayVendor\WordPressCS\WordPress\Helpers\EscapingFunctionsTrait;
use BitPayVendor\WordPressCS\WordPress\Helpers\FormattingFunctionsHelper;
use BitPayVendor\WordPressCS\WordPress\Helpers\PrintingFunctionsTrait;
...
class EscapeOutputSniff extends Sniff
{
     ...
     * @var array
     */
    protected $unsafePrintingFunctions = array('_e' => 'esc_html_e() or esc_attr_e()', '_ex' => 'echo esc_html_x() or echo esc_attr_x()');
    /**
    ...
    private $safe_php_constants = array(
        ...
        'PHP_VERSION' => \true,
        // Integer.
        'PHP_MAJOR_VERSION' => \true,
        // Integer.
        'PHP_MINOR_VERSION' => \true,
        // Integer.
        'PHP_RELEASE_VERSION' => \true,
        // Integer.
        'PHP_VERSION_ID' => \true,
        // Integer.
        'PHP_EXTRA_VERSION' => \true,
        // String.
        'PHP_DEBUG' => \true,
    );
    /**
     ...
     * @var array
     */
    private $safe_components = array('T_CONSTANT_ENCAPSED_STRING' => \true, 'T_LNUMBER' => \true, 'T_MINUS' => \true, 'T_PLUS' => \true, 'T_MULTIPLY' => \true, 'T_DIVIDE' => \true, 'T_MODULUS' => \true, 'T_TRUE' => \true, 'T_FALSE' => \true, 'T_NULL' => \true, 'T_DNUMBER' => \true, 'T_START_NOWDOC' => \true, 'T_NOWDOC' => \true, 'T_END_NOWDOC' => \true);
    /**
    ...
    {
        return array(\T_ECHO, \T_PRINT, \T_EXIT, \T_STRING, \T_OPEN_TAG_WITH_ECHO);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        $open_paren = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        ...
        if (\T_STRING === $this->tokens[$stackPtr]['code']) {
            ...
            if (\in_array($function, array('trigger_error', 'user_error'), \true)) {
        ...
        if (isset($this->unsafePrintingFunctions[$function])) {
            $error = $this->phpcsFile->addError("All output should be run through an escaping function (like %s), found '%s'.", $stackPtr, 'UnsafePrintingFunction', array($this->unsafePrintingFunctions[$function], $function));
        ...
        if (!isset($end_of_statement)) {
            ...
            $last_token = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $end_of_statement - 1, null, \true);
        ...
        for ($i = $stackPtr; $i < $end_of_statement; $i++) {
            ...
            if (\T_OPEN_SHORT_ARRAY === $this->tokens[$i]['code'] || \T_CLOSE_SHORT_ARRAY === $this->tokens[$i]['code']) {
                continue;
            }
            if (\in_array($this->tokens[$i]['code'], array(\T_DOUBLE_ARROW, \T_CLOSE_PARENTHESIS), \true)) {
                continue;
            }
            ...
            if (\T_STRING === $this->tokens[$i]['code'] && isset($this->safe_php_constants[$this->tokens[$i]['content']]) && ConstantsHelper::is_use_of_global_constant($this->phpcsFile, $i)) {
                continue;
            }
            ...
            if (\T_STRING === $this->tokens[$i]['code']) {
                ...
                $function_opener = $this->phpcsFile->findNext(\T_OPEN_PARENTHESIS, $i + 1, null, \false, null, \true);
                ...
                if (\false !== $function_opener) {
                    if (ArrayWalkingFunctionsHelper::is_array_walking_function($functionName)) {
                        ...
                        if (!empty($callback)) {
                             ...
                             */
                            $mapped_function = $this->phpcsFile->findNext(Tokens::$emptyTokens, $callback['start'], $callback['end'] + 1, \true);
                ...
                if ($is_formatting_function || $this->is_escaping_function($functionName) || $this->is_auto_escaped_function($functionName)) {
                    continue;
                }
            ...
            $this->phpcsFile->addError("All output should be run through an escaping function (see the Security sections in the WordPress Developer Handbooks), found '%s'.", $ptr, 'OutputNotEscaped', array($content));
        }
