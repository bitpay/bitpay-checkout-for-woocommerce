...
use BitPayVendor\WordPressCS\WordPress\Helpers\ValidationHelper;
...
class ValidatedSanitizedInputSniff extends Sniff
{
    ...
    {
        return array(\T_VARIABLE, \T_DOUBLE_QUOTED_STRING, \T_HEREDOC);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\T_DOUBLE_QUOTED_STRING === $this->tokens[$stackPtr]['code'] || \T_HEREDOC === $this->tokens[$stackPtr]['code']) {
            ...
            $interpolated_variables = \array_map(function ($embed) {
        ...
        if (!\in_array($this->tokens[$stackPtr]['content'], $superglobals, \true)) {
            return;
        }
        ...
        if (!$this->is_sanitized($stackPtr, \true)) {
