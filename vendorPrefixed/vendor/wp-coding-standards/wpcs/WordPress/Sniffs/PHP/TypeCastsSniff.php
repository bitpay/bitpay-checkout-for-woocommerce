...
final class TypeCastsSniff extends Sniff
{
    ...
    {
        return array(\T_DOUBLE_CAST, \T_UNSET_CAST, \T_BINARY_CAST);
    }
    /**
        ...
        switch ($token_code) {
            case \T_DOUBLE_CAST:
                if ('(float)' !== $typecast_lc) {
                    $fix = $this->phpcsFile->addFixableError('Normalized type keywords must be used; expected "(float)" but found "%s"', $stackPtr, 'DoubleRealFound', array($typecast));
                    if (\true === $fix) {
                ...
                break;
            case \T_UNSET_CAST:
                $this->phpcsFile->addError('Using the "(unset)" cast is forbidden as the type cast is removed in PHP 8.0. Use the "unset()" language construct instead.', $stackPtr, 'UnsetFound');
                break;
            case \T_BINARY_CAST:
                $this->phpcsFile->addWarning('Using binary casting is strongly discouraged. Found: "%s"', $stackPtr, 'BinaryFound', array($typecast));
                break;
        }
    }
}
