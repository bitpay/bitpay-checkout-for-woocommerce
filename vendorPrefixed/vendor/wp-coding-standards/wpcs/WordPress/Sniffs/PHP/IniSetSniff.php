...
final class IniSetSniff extends AbstractFunctionParameterSniff
{
     ...
     *     );
     */
    protected $target_functions = array('ini_set' => \true, 'ini_alter' => \true);
    /**
     ...
     *     );
     */
    protected $safe_options = array('auto_detect_line_endings' => array(), 'highlight.bg' => array(), 'highlight.comment' => array(), 'highlight.default' => array(), 'highlight.html' => array(), 'highlight.keyword' => array(), 'highlight.string' => array(), 'short_open_tag' => array('valid_values' => array('true', '1', 'on')));
    /**
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if (isset($this->safe_options[$option_name])) {
            ...
            if (empty($safe_option['valid_values']) || \in_array(\strtolower($option_value), $safe_option['valid_values'], \true)) {
                return;
            }
        }
        if (isset($this->disallowed_options[$option_name])) {
            $disallowed_option = $this->disallowed_options[$option_name];
            if (empty($disallowed_option['invalid_values']) || \in_array(\strtolower($option_value), $disallowed_option['invalid_values'], \true)) {
                $this->phpcsFile->addError('Found: %s(%s, %s). %s', $stackPtr, MessageHelper::stringToErrorcode($option_name . '_Disallowed'), array($matched_content, $option_param['clean'], $value_param['clean'], $disallowed_option['message']));
                return;
            }
        }
        $this->phpcsFile->addWarning('Changing configuration values at runtime is strongly discouraged. Found: %s(%s, %s)', $stackPtr, 'Risky', array($matched_content, $option_param['clean'], $value_param['clean']));
    }
}
