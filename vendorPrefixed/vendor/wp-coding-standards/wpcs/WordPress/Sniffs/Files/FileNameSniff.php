...
final class FileNameSniff extends Sniff
{
     ...
     * @var bool
     */
    public $strict_class_file_names = \true;
    /**
     ...
     * @var array
     */
    private $hyphenation_exceptions = array('class.wp-dependencies.php' => \true, 'class.wp-scripts.php' => \true, 'class.wp-styles.php' => \true, 'functions.wp-scripts.php' => \true, 'functions.wp-styles.php' => \true);
    /**
     ...
     * @var array
     */
    private $unittest_hyphenation_exceptions = array('class.wp-dependencies.inc' => \true, 'class.wp-scripts.inc' => \true, 'class.wp-styles.inc' => \true, 'functions.wp-scripts.inc' => \true, 'functions.wp-styles.inc' => \true);
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        while ($i = $this->phpcsFile->findNext(\T_PHPCS_DISABLE, $i + 1)) {
            if (empty($this->tokens[$i]['sniffCodes']) || isset($this->tokens[$i]['sniffCodes']['WordPress']) || isset($this->tokens[$i]['sniffCodes']['WordPress.Files']) || isset($this->tokens[$i]['sniffCodes']['WordPress.Files.FileName'])) {
                do {
                    $i = $this->phpcsFile->findNext(\T_PHPCS_ENABLE, $i + 1);
                } while (\false !== $i && !empty($this->tokens[$i]['sniffCodes']) && !isset($this->tokens[$i]['sniffCodes']['WordPress']) && !isset($this->tokens[$i]['sniffCodes']['WordPress.Files']) && !isset($this->tokens[$i]['sniffCodes']['WordPress.Files.FileName']));
                if (\false === $i) {
        ...
        if (\true === $this->strict_class_file_names && \false !== $class_ptr) {
            $this->check_filename_has_class_prefix($class_ptr, $file_name);
        }
        if (\false !== \strpos($file, \DIRECTORY_SEPARATOR . 'wp-includes' . \DIRECTORY_SEPARATOR) && \false === $class_ptr) {
    ...
    protected function check_filename_is_hyphenated($file_name)
    {
        ...
        if (\true === $this->is_theme && 1 === \preg_match(self::THEME_EXCEPTIONS_REGEX, $file_name)) {
            return;
        }
        $this->phpcsFile->addError('Filenames should be all lowercase with hyphens as word separators. Expected %s, but found %s.', 0, 'NotHyphenatedLowercase', array($expected, $file_name));
    }
    /**
    ...
    protected function check_filename_has_class_prefix($class_ptr, $file_name)
    {
        ...
        $this->phpcsFile->addError('Class file names should be based on the class name with "class-" prepended. Expected %s, but found %s.', 0, 'InvalidClassFileName', array($expected, $file_name));
    }
    /**
    ...
    protected function check_filename_for_template_suffix($stackPtr, $file_name)
    {
        ...
        if ('Template' === \trim($this->tokens[$subpackage]['content']) && $this->tokens[$subpackage_tag]['line'] === $this->tokens[$subpackage]['line'] && (!\defined('\\PHP_CODESNIFFER_IN_TESTS') && '-template.php' !== $fileName_end || \defined('\\PHP_CODESNIFFER_IN_TESTS') && '-template.inc' !== $fileName_end)) {
            $this->phpcsFile->addError('Files containing template tags should have "-template" appended to the end of the file name. Expected %s, but found %s.', 0, 'InvalidTemplateTagFileName', array(\substr($file_name, 0, -4) . '-template.php', $file_name));
        }
    }
}
