...
use BitPayVendor\WordPressCS\WordPress\Helpers\WPDBTrait;
...
final class PreparedSQLPlaceholdersSniff extends Sniff
{
     ...
     * @var array
     */
    protected $target_methods = array('prepare' => \true);
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        $valid_in_clauses = array('uses_in' => 0, 'implode_fill' => 0, 'adjustment_count' => 0);
        for ($i = $query['start']; $i <= $query['end']; $i++) {
            ...
            if (!isset(Tokens::$textStringTokens[$this->tokens[$i]['code']])) {
                ...
                if (\T_STRING === $this->tokens[$i]['code']) {
                    if ('sprintf' === \strtolower($this->tokens[$i]['content'])) {
                        ...
                        $prev = $this->phpcsFile->findPrevious(Tokens::$textStringTokens, $i - 1, $query['start']);
                        ...
                        if (\preg_match('`\\s+IN\\s*\\(\\s*(' . $regex_quote . ')?$`i', $prev_content, $match) > 0) {
                            if (isset($match[1]) && $regex_quote !== $this->regex_quote) {
                                $this->phpcsFile->addError('Dynamic placeholder generation should not have surrounding quotes.', $i, 'QuotedDynamicPlaceholderGeneration');
                            }
                            if ($this->analyse_implode($i) === \true) {
                                ++$valid_in_clauses['uses_in'];
                                ++$valid_in_clauses['implode_fill'];
                                $next = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
            ...
            if (\T_DOUBLE_QUOTED_STRING === $this->tokens[$i]['code'] || \T_HEREDOC === $this->tokens[$i]['code']) {
                ...
                if ($stripped_content !== $content) {
                    $vars_without_wpdb = \array_filter(TextStrings::getEmbeds($content), function ($symbol) {
                    ...
                    foreach ($walk as $match_key) {
                        foreach ($matches[$match_key] as $index => $match) {
                            ...
                            if (\strpos($match, '%') === \false && \strpos($match, '_') === \false) {
                                $this->phpcsFile->addWarning('Unless you are using SQL wildcards, using LIKE is inefficient. Use a straight compare instead. Found: %s.', $i, 'LikeWithoutWildcards', $data);
                            } else {
                                $sql_wildcard_found = \true;
                                if (\strpos($match, '%s') === \false) {
                                    $this->phpcsFile->addError('SQL wildcards for a LIKE query should be passed in through a replacement parameter. Found: %s.', $i, 'LikeWildcardsInQuery', $data);
                                } else {
                                    $this->phpcsFile->addError('SQL wildcards for a LIKE query should be passed in through a replacement parameter and the variable part of the replacement should be escaped using "esc_like()". Found: %s.', $i, 'LikeWildcardsInQueryWithPlaceholder', $data);
                                }
                            }
                            /*
            ...
            if (\preg_match_all(self::UNSUPPORTED_PLACEHOLDER_REGEX, $content, $matches) > 0) {
                if (!empty($matches[0])) {
                    foreach ($matches[0] as $match) {
                        if ('%' === $match) {
                            $this->phpcsFile->addError('Found unescaped literal "%%" character.', $i, 'UnescapedLiteral', array($match));
                        } else {
                            $this->phpcsFile->addError('Unsupported placeholder used in $wpdb->prepare(). Found: "%s".', $i, 'UnsupportedPlaceholder', array($match));
                        }
                    }
                }
                unset($match, $matches);
            ...
            if (\preg_match_all($regex, $content, $matches) > 0) {
                if (!empty($matches[0])) {
                    foreach ($matches[0] as $match) {
                        $this->phpcsFile->addError('Simple placeholders should not be quoted in the query string in $wpdb->prepare(). Found: %s.', $i, 'QuotedSimplePlaceholder', array($match));
                    }
                }
                unset($match, $matches);
            }
                ...
                if (!empty($matches[0])) {
                    foreach ($matches[0] as $match) {
                        if (\preg_match('`%[dfFs]`', $match) !== 1) {
                            $this->phpcsFile->addWarning('Complex placeholders used for values in the query string in $wpdb->prepare() will NOT be quoted automagically. Found: %s.', $i, 'UnquotedComplexPlaceholder', array($match));
                        }
                    }
                }
                unset($match, $matches);
        ...
        if (0 === $total_placeholders) {
            if (1 === $total_parameters) {
                if (\false === $variable_found && \false === $sql_wildcard_found) {
                     ...
                     */
                    $this->phpcsFile->addWarning('It is not necessary to prepare a query which doesn\'t use variable replacement.', $i, 'UnnecessaryPrepare');
                }
            } elseif (0 === $valid_in_clauses['uses_in']) {
                $this->phpcsFile->addWarning('Replacement variables found, but no valid placeholders found in the query.', $i, 'UnfinishedPrepare');
            }
            return;
        }
        if (1 === $total_parameters) {
            $this->phpcsFile->addError('Placeholders found in the query passed to $wpdb->prepare(), but no replacements found. Expected %d replacement(s) parameters.', $stackPtr, 'MissingReplacements', array($total_placeholders));
            return;
        }
        ...
        if (isset($parameters[2]) && 2 === $total_parameters) {
            $next = $this->phpcsFile->findNext(Tokens::$emptyTokens, $parameters[2]['start'], $parameters[2]['end'] + 1, \true);
            if (\false !== $next && (\T_ARRAY === $this->tokens[$next]['code'] || \T_OPEN_SHORT_ARRAY === $this->tokens[$next]['code'])) {
        ...
        if ($valid_in_clauses['uses_in'] > 0 && $valid_in_clauses['uses_in'] === $valid_in_clauses['implode_fill'] && 1 === $total_replacements) {
            return;
        }
        /*
        ...
        if ($total_replacements !== $total_placeholders) {
            $this->phpcsFile->addWarning('Incorrect number of replacements passed to $wpdb->prepare(). Found %d replacement parameters, expected %d.', $stackPtr, 'ReplacementsWrongNumber', array($total_replacements, $total_placeholders));
        }
    }
    /**
    ...
    protected function analyse_sprintf($sprintf_params)
    {
        ...
        foreach ($sprintf_params as $sprintf_param) {
            ...
            $implode = $this->phpcsFile->findNext(Tokens::$emptyTokens, $sprintf_param['start'], $sprintf_param['end'], \true);
            if (\T_STRING === $this->tokens[$implode]['code'] && 'implode' === \strtolower($this->tokens[$implode]['content'])) {
                if ($this->analyse_implode($implode) === \true) {
                    ++$found;
                }
            }
        }
    ...
    protected function analyse_implode($implode_token)
    {
        ...
        $array_fill = $this->phpcsFile->findNext(Tokens::$emptyTokens, $implode_params[2]['start'], $implode_params[2]['end'], \true);
        if (\T_STRING !== $this->tokens[$array_fill]['code'] || 'array_fill' !== \strtolower($this->tokens[$array_fill]['content'])) {
            return \false;
        }
