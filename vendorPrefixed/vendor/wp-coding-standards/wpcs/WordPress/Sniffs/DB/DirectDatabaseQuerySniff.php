...
final class DirectDatabaseQuerySniff extends Sniff
{
     ...
     * @var array
     */
    protected $addedCustomFunctions = array('cacheget' => array(), 'cacheset' => array(), 'cachedelete' => array());
    /**
     ...
     * @var array
     */
    protected $cacheGetFunctions = array('wp_cache_get' => \true);
    /**
     ...
     * @var array
     */
    protected $cacheSetFunctions = array('wp_cache_set' => \true, 'wp_cache_add' => \true);
    /**
     ...
     * @var array
     */
    protected $cacheDeleteFunctions = array('wp_cache_delete' => \true, 'clean_attachment_cache' => \true, 'clean_blog_cache' => \true, 'clean_bookmark_cache' => \true, 'clean_category_cache' => \true, 'clean_comment_cache' => \true, 'clean_network_cache' => \true, 'clean_object_term_cache' => \true, 'clean_page_cache' => \true, 'clean_post_cache' => \true, 'clean_term_cache' => \true, 'clean_user_cache' => \true);
    /**
     ...
     * @var array[]
     */
    protected $methods = array('cachable' => array('delete' => \true, 'get_var' => \true, 'get_col' => \true, 'get_row' => \true, 'get_results' => \true, 'query' => \true, 'replace' => \true, 'update' => \true), 'noncachable' => array('insert' => \true));
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        $is_object_call = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if (\false === $is_object_call || \T_OBJECT_OPERATOR !== $this->tokens[$is_object_call]['code'] && \T_NULLSAFE_OBJECT_OPERATOR !== $this->tokens[$is_object_call]['code']) {
            // This is not a call to the wpdb object.
            return;
        }
        $methodPtr = $this->phpcsFile->findNext(Tokens::$emptyTokens, $is_object_call + 1, null, \true);
            ...
            for ($i = $scopeStart + 1; $i < $scopeEnd; $i++) {
                if (\T_STRING === $this->tokens[$i]['code']) {
                    if (isset($this->cacheDeleteFunctions[$this->tokens[$i]['content']])) {
                        if (\in_array($method, array('query', 'update', 'replace', 'delete'), \true)) {
                            $cached = \true;
                            break;
                        }
    ...
    protected function mergeFunctionLists()
    {
        ...
        if ($this->customCacheGetFunctions !== $this->addedCustomFunctions['cacheget']) {
            $this->cacheGetFunctions = RulesetPropertyHelper::merge_custom_array($this->customCacheGetFunctions, $this->cacheGetFunctions);
        ...
        if ($this->customCacheSetFunctions !== $this->addedCustomFunctions['cacheset']) {
            $this->cacheSetFunctions = RulesetPropertyHelper::merge_custom_array($this->customCacheSetFunctions, $this->cacheSetFunctions);
        ...
        if ($this->customCacheDeleteFunctions !== $this->addedCustomFunctions['cachedelete']) {
            $this->cacheDeleteFunctions = RulesetPropertyHelper::merge_custom_array($this->customCacheDeleteFunctions, $this->cacheDeleteFunctions);
