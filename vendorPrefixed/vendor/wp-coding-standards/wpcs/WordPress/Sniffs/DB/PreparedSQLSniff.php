...
final class PreparedSQLSniff extends Sniff
{
     ...
     */
    protected $methods = array('get_var' => \true, 'get_col' => \true, 'get_row' => \true, 'get_results' => \true, 'prepare' => \true, 'query' => \true);
    /**
     ...
     * @var array<string, bool>
     */
    private $SQLEscapingFunctions = array('absint' => \true, 'esc_sql' => \true, 'floatval' => \true, 'intval' => \true, 'like_escape' => \true);
    /**
     ...
     * @var array<string, bool>
     */
    private $SQLAutoEscapedFunctions = array('count' => \true);
    /**
     ...
     * @var array
     */
    private $ignored_tokens = array(\T_STRING_CONCAT => \true, \T_CONSTANT_ENCAPSED_STRING => \true, \T_COMMA => \true, \T_LNUMBER => \true, \T_DNUMBER => \true, \T_NS_SEPARATOR => \true);
    /**
     * A loop pointer.
     *
    ...
    public function process_token($stackPtr)
    {
        ...
        for ($this->i; $this->i < $this->end; $this->i++) {
            ...
            if (\T_DOUBLE_QUOTED_STRING === $this->tokens[$this->i]['code'] || \T_HEREDOC === $this->tokens[$this->i]['code']) {
                $bad_variables = \array_filter(TextStrings::getEmbeds($this->tokens[$this->i]['content']), function ($symbol) {
                    return \preg_match('`^\\{?\\$\\{?wpdb\\??->`', $symbol) !== 1;
                });
                foreach ($bad_variables as $bad_variable) {
                    $this->phpcsFile->addError('Use placeholders and $wpdb->prepare(); found interpolated variable %s at %s', $this->i, 'InterpolatedNotPrepared', array($bad_variable, $this->tokens[$this->i]['content']));
                }
                continue;
            }
            ...
            if (\T_STRING === $this->tokens[$this->i]['code']) {
                if (isset($this->SQLEscapingFunctions[$this->tokens[$this->i]['content']]) || isset($this->SQLAutoEscapedFunctions[$this->tokens[$this->i]['content']])) {
                    // Find the opening parenthesis.
                    $opening_paren = $this->phpcsFile->findNext(Tokens::$emptyTokens, $this->i + 1, null, \true);
                    if (\false !== $opening_paren && \T_OPEN_PARENTHESIS === $this->tokens[$opening_paren]['code'] && isset($this->tokens[$opening_paren]['parenthesis_closer'])) {
            ...
            $this->phpcsFile->addError('Use placeholders and $wpdb->prepare(); found %s', $this->i, 'NotPrepared', array($this->tokens[$this->i]['content']));
        }
        return $this->end;
    }
}
