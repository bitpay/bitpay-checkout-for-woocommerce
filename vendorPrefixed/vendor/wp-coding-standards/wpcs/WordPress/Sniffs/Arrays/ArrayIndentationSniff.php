...
final class ArrayIndentationSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        if (isset(Collections::shortArrayListOpenTokensBC()[$this->tokens[$stackPtr]['code']]) && Arrays::isShortArray($this->phpcsFile, $stackPtr) === \false) {
        ...
        if ($closer_line_spaces !== $opener_line_spaces) {
             ...
             */
            if (0 === $closer_line_spaces || \T_WHITESPACE === $this->tokens[$closer - 1]['code'] && 1 === $this->tokens[$closer - 1]['column']) {
                $this->add_array_alignment_error($closer, $error, $error_code, $opener_line_spaces, $closer_line_spaces, $this->get_indentation_string($opener_line_spaces));
            } else {
                /*
                 ...
                 */
                $this->phpcsFile->addError($error, $closer, $error_code, array($opener_line_spaces, $closer_line_spaces));
            }
        ...
        foreach ($array_items as $item) {
            ...
            $first_content = $this->phpcsFile->findNext(array(\T_WHITESPACE, \T_DOC_COMMENT_WHITESPACE), $item['start'], $end_of_this_item, \true);
            // Deal with trailing comments.
            if (\false !== $first_content && \T_COMMENT === $this->tokens[$first_content]['code'] && $this->tokens[$first_content]['line'] === $this->tokens[$end_of_previous_item]['line']) {
                $first_content = $this->phpcsFile->findNext(array(\T_WHITESPACE, \T_DOC_COMMENT_WHITESPACE, \T_COMMENT), $first_content + 1, $end_of_this_item, \true);
            }
            ...
            if ($found_spaces !== $expected_spaces) {
                $this->add_array_alignment_error($first_content, 'Array item not aligned correctly; expected %s spaces but found %s', 'ItemNotAligned', $expected_spaces, $found_spaces, $expected_indent);
            }
            ...
            for ($ptr = $first_content + 1; $ptr <= $item['end']; $ptr++) {
                if ($this->tokens[$first_content]['line'] !== $this->tokens[$ptr]['line'] && 1 === $this->tokens[$ptr]['column'] && \false === $this->ignore_token($ptr)) {
                    break;
                }
            }
            $first_content_on_line2 = $this->phpcsFile->findNext(array(\T_WHITESPACE, \T_DOC_COMMENT_WHITESPACE), $ptr, $end_of_this_item, \true);
            if (\false === $first_content_on_line2) {
                 ...
                 */
                if ($this->tokens[$item['end']]['line'] !== $this->tokens[$end_of_this_item]['line'] && \T_COMMA === $this->tokens[$end_of_this_item]['code'] && $this->tokens[$end_of_this_item]['column'] - 1 !== $expected_spaces) {
                    $this->add_array_alignment_error($end_of_this_item, 'Comma after multi-line array item not aligned correctly; expected %s spaces, but found %s', 'MultiLineArrayItemCommaNotAligned', $expected_spaces, $this->tokens[$end_of_this_item]['column'] - 1, $expected_indent);
                }
                $end_of_previous_item = $end_of_this_item;
                continue;
            }
            ...
            if ($found_spaces_on_line2 !== $expected_spaces_on_line2) {
                $fix = $this->phpcsFile->addFixableError('Multi-line array item not aligned correctly; expected %s spaces, but found %s', $first_content_on_line2, 'MultiLineArrayItemNotAligned', array($expected_spaces_on_line2, $found_spaces_on_line2));
                if (\true === $fix) {
                    $expected_indent_on_line2 = $this->get_indentation_string($expected_spaces_on_line2);
                    $this->phpcsFile->fixer->beginChangeset();
                    // Fix second line for the array item.
                    if (1 === $this->tokens[$first_content_on_line2]['column'] && \T_COMMENT === $this->tokens[$first_content_on_line2]['code']) {
                    ...
                    for ($i = $first_content_on_line2 + 1; $i <= $item['end']; $i++) {
                        ...
                        $first_content_on_line = $this->phpcsFile->findNext(array(\T_WHITESPACE, \T_DOC_COMMENT_WHITESPACE), $i, $end_of_this_item, \true);
                        if (\false === $first_content_on_line) {
                            break;
                        }
                        // Ignore lines with heredoc and nowdoc tokens and subsequent lines in multi-line strings.
                        if (\true === $this->ignore_token($first_content_on_line)) {
                        ...
                        $expected_spaces_on_line = \max($expected_spaces_on_line, 0);
                        // Can't be below 0.
                        ...
                        if ($found_spaces_on_line !== $expected_spaces_on_line) {
                            if (1 === $this->tokens[$first_content_on_line]['column'] && \T_COMMENT === $this->tokens[$first_content_on_line]['code']) {
                     ...
                     */
                    if ($this->tokens[$item['end']]['line'] !== $this->tokens[$end_of_this_item]['line'] && \T_COMMA === $this->tokens[$end_of_this_item]['code'] && $this->tokens[$end_of_this_item]['column'] - 1 !== $expected_spaces) {
                        $this->add_array_alignment_error($end_of_this_item, 'Comma after array item not aligned correctly; expected %s spaces, but found %s', 'MultiLineArrayItemCommaNotAligned', $expected_spaces, $this->tokens[$end_of_this_item]['column'] - 1, $expected_indent);
                    }
    ...
    protected function get_indentation_size($ptr)
    {
        ...
        $whitespace = '';
        if (\T_WHITESPACE === $this->tokens[$ptr]['code'] || \T_DOC_COMMENT_WHITESPACE === $this->tokens[$ptr]['code']) {
