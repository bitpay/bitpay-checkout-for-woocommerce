...
final class CommaAfterArrayItemSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        foreach ($array_items as $item_index => $item) {
             ...
             */
            if (\true === $single_line && $item_index === $array_item_count) {
                $this->phpcsFile->recordMetric($stackPtr, 'Single line array - comma after last item', \true === $is_comma ? 'yes' : 'no');
                if (\true === $is_comma) {
                    $fix = $this->phpcsFile->addFixableError('Comma not allowed after last value in single-line array declaration', $maybe_comma, 'CommaAfterLast');
                    if (\true === $fix) {
            ...
            $last_content = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $item['end'], $item['start'], \true);
            ...
            if (\false === $is_comma && \false === $single_line) {
                $fix = $this->phpcsFile->addFixableError('Each array item in a multi-line array declaration must end in a comma', $last_content, 'NoComma');
                if (\true === $fix) {
                    $this->phpcsFile->fixer->addContent($last_content, ',');
                }
            }
            if (\false === $single_line && $item_index === $array_item_count) {
                $this->phpcsFile->recordMetric($stackPtr, 'Multi-line array - comma after last item', \true === $is_comma ? 'yes' : 'no');
            }
            if (\false === $is_comma) {
             ...
             */
            if ($last_content !== $item['end'] && (\true === $single_line || !isset(Tokens::$heredocTokens[$this->tokens[$last_content]['code']]))) {
                $newlines = 0;
                $spaces = 0;
                for ($i = $item['end']; $i > $last_content; $i--) {
                    ...
                    } elseif (\T_COMMENT === $this->tokens[$i]['code'] || isset(Tokens::$phpcsCommentTokens[$this->tokens[$i]['code']])) {
                        break;
                    }
                }
                $space_phrases = array();
                ...
                $fix = $this->phpcsFile->addFixableError('Expected 0 spaces between "%s" and comma; %s found', $maybe_comma, 'SpaceBeforeComma', array($this->tokens[$last_content]['content'], \implode(' and ', $space_phrases)));
                if (\true === $fix) {
            ...
            if (\T_WHITESPACE === $next_token['code']) {
                ...
                $next_non_whitespace = $this->phpcsFile->findNext(\T_WHITESPACE, $maybe_comma + 1, $closer, \true);
                if (\false === $next_non_whitespace || \false === $single_line && $this->tokens[$next_non_whitespace]['line'] === $this->tokens[$maybe_comma]['line'] && (\T_COMMENT === $this->tokens[$next_non_whitespace]['code'] || isset(Tokens::$phpcsCommentTokens[$this->tokens[$next_non_whitespace]['code']]))) {
                    continue;
                }
                ...
                $fix = $this->phpcsFile->addFixableError('Expected 1 space between comma and "%s"; %s found', $maybe_comma, 'SpaceAfterComma', array($this->tokens[$next_non_whitespace]['content'], $space_length));
                if (\true === $fix) {
                ...
                $fix = $this->phpcsFile->addFixableError('Expected 1 space between comma and "%s"; 0 found', $maybe_comma, 'NoSpaceAfterComma', array($next_token['content']));
                if (\true === $fix) {
