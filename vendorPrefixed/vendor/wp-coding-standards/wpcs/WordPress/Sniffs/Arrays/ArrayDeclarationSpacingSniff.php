...
final class ArrayDeclarationSpacingSniff extends Sniff
{
     ...
     */
    public $allow_single_item_single_line_associative_arrays = \true;
    /**
    ...
    protected function process_single_line_array($stackPtr, $opener, $closer)
    {
        ...
        if (\false === $this->allow_single_item_single_line_associative_arrays && empty($array_items) || \true === $this->allow_single_item_single_line_associative_arrays && \count($array_items) === 1) {
            return;
        }
        /*
        ...
        if (\true === $this->allow_single_item_single_line_associative_arrays) {
         ...
         */
        SpacesFixer::checkAndFix($this->phpcsFile, $closer, $this->phpcsFile->findPrevious(\T_WHITESPACE, $closer - 1, null, \true), 'newline', $error, 'AssociativeArrayFound', 'error');
    }
    /**
    ...
    protected function process_multi_line_array($stackPtr, $opener)
    {
        ...
        foreach ($array_items as $item) {
            ...
            if ($this->tokens[$first_content]['line'] === $this->tokens[$end_of_last_item]['line'] && (\T_COMMENT === $this->tokens[$first_content]['code'] || isset(Tokens::$phpcsCommentTokens[$this->tokens[$first_content]['code']]))) {
                ...
                if (\substr(\ltrim($this->tokens[$end_of_comment]['content']), 0, 2) === '/*') {
                    while ((\T_COMMENT === $this->tokens[$end_of_comment]['code'] || isset(Tokens::$phpcsCommentTokens[$this->tokens[$end_of_comment]['code']])) && \substr(\rtrim($this->tokens[$end_of_comment]['content']), -2) !== '*/' && $end_of_comment + 1 < $end_of_this_item) {
                        ++$end_of_comment;
                    }
                ...
                $next = $this->phpcsFile->findNext(array(\T_WHITESPACE, \T_DOC_COMMENT_WHITESPACE), $end_of_comment + 1, $end_of_this_item, \true);
                if (\false === $next) {
            ...
            if ($this->tokens[$end_of_last_item]['line'] === $this->tokens[$first_content]['line']) {
                SpacesFixer::checkAndFix($this->phpcsFile, $first_content, $end_of_last_item, 'newline', 'Each item in a multi-line array must be on %s. Found: %s', 'ArrayItemNoNewLine', 'error');
            }
            $end_of_last_item = $end_of_this_item;
        }
    }
