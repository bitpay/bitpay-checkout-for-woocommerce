...
final class MultipleStatementAlignmentSniff extends Sniff
{
    ...
    protected function process_single_line_array($stackPtr, $items, $opener, $closer)
    {
         ...
         */
        $next_arrow = $this->phpcsFile->findNext(\T_DOUBLE_ARROW, $opener + 1, $closer);
        while (\false !== $next_arrow) {
            if (\T_WHITESPACE === $this->tokens[$next_arrow - 1]['code']) {
                ...
                if (1 !== $space_length) {
                    ...
                    $data = array($this->tokens[$next_arrow - 2]['content'], $space_length);
            ...
            $next_arrow = $this->phpcsFile->findNext(\T_DOUBLE_ARROW, $next_arrow + 1, $closer);
        }
    ...
    protected function process_multi_line_array($stackPtr, $items, $opener, $closer)
    {
         ...
         */
        $index_end_cols = array();
        // Keep track of the end column position of index keys.
        $double_arrow_cols = array();
        // Keep track of arrow column position and count.
        $multi_line_count = 0;
        ...
        foreach ($items as $key => $item) {
            ...
            $last_index_token = $this->phpcsFile->findPrevious(\T_WHITESPACE, $double_arrow - 1, $item['start'], \true);
            if (\true === $this->ignoreNewlines && $this->tokens[$last_index_token]['line'] !== $this->tokens[$double_arrow]['line']) {
        ...
        if (\false === $this->exact && !empty($double_arrow_cols)) {
            ...
            if ($count > 1 || 1 === $count && \count($items) === 1) {
                ...
                $filtered_double_arrow_cols = \array_keys($double_arrow_cols, $count, \true);
         ...
         */
        foreach ($items as $item) {
            if ($this->tokens[$item['operatorPtr']]['column'] === $expected_col && $this->tokens[$item['operatorPtr']]['line'] === $this->tokens[$item['last_index_token']]['line']) {
             ...
             */
            if ($item['last_index_col'] + 2 > $this->maxColumn || \false === $alignMultilineItems && \false === $item['single_line']) {
                if ($item['last_index_col'] + 2 === $this->tokens[$item['operatorPtr']]['column'] && $this->tokens[$item['operatorPtr']]['line'] === $this->tokens[$item['last_index_token']]['line']) {
                ...
                $fix = $this->phpcsFile->addFixableWarning('Expected 1 space between "%s" and double arrow; %s found.', $item['operatorPtr'], $error_code, array($this->tokens[$item['last_index_token']]['content'], $before));
                if (\true === $fix) {
            ...
            $fix = $this->phpcsFile->addFixableWarning('Array double arrow not aligned correctly; expected %s space(s) between "%s" and double arrow, but found %s.', $item['operatorPtr'], 'DoubleArrowNotAligned', array($expected_whitespace, $this->tokens[$item['last_index_token']]['content'], $before));
            if (\true === $fix) {
                if (0 === $before || 'newline' === $before) {
                    ...
                    // Add the correct whitespace.
                    $this->phpcsFile->fixer->addContent($item['last_index_token'], \str_repeat(' ', $expected_whitespace));
                    $this->phpcsFile->fixer->endChangeset();
                    ...
                    $this->phpcsFile->fixer->addContent($item['operatorPtr'] - 1, \str_repeat(' ', $expected_whitespace - $before));
                } else {
                    // Too much whitespace found.
                    $this->phpcsFile->fixer->replaceToken($item['operatorPtr'] - 1, \str_repeat(' ', $expected_whitespace));
                }
            }
        }
    }
    ...
    protected function validate_align_multiline_items()
    {
        ...
        if ('always' === $alignMultilineItems || 'never' === $alignMultilineItems) {
            ...
            if (\preg_match('`^([=<>!]{1,2})(100|[0-9]{1,2})$`', $alignMultilineItems, $matches) > 0) {
                ...
                if (\in_array($operator, array('<', '<=', '>', '>=', '==', '=', '!=', '<>'), \true) === \true && ($number >= 0 && $number <= 100)) {
        ...
        $this->phpcsFile->addError('Invalid property value passed: "%s". The value for the "alignMultilineItems" property for the "WordPress.Arrays.MultipleStatementAlignment" sniff should be either "always", "never" or an comparison operator + a number between 0 and 100.', 0, 'InvalidPropertyPassed', array($this->alignMultilineItems));
