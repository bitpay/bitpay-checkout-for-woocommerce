...
final class VariableHelper
{
     ...
     */
    public static function get_array_access_keys(File $phpcsFile, $stackPtr, $all = \true)
    {
        $tokens = $phpcsFile->getTokens();
        $keys = array();
        if (isset($tokens[$stackPtr]) === \false || \T_VARIABLE !== $tokens[$stackPtr]['code']) {
            return $keys;
        }
        $current = $stackPtr;
        do {
            // Find the next non-empty token.
            $open_bracket = $phpcsFile->findNext(Tokens::$emptyTokens, $current + 1, null, \true);
            // If it isn't a bracket, this isn't an array-access.
            if (\false === $open_bracket || \T_OPEN_SQUARE_BRACKET !== $tokens[$open_bracket]['code'] || !isset($tokens[$open_bracket]['bracket_closer'])) {
                break;
            }
            $key = GetTokensAsString::compact($phpcsFile, $open_bracket + 1, $tokens[$open_bracket]['bracket_closer'] - 1, \true);
            $keys[] = \trim($key);
            $current = $tokens[$open_bracket]['bracket_closer'];
        } while (isset($tokens[$current]) && \true === $all);
        return $keys;
    }
    /**
     ...
     */
    public static function is_comparison(File $phpcsFile, $stackPtr, $include_coalesce = \true)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        $previous_token = $phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if (isset($comparisonTokens[$tokens[$previous_token]['code']])) {
            return \true;
        }
        // Maybe the comparison operator is after this.
        $next_token = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        // This might be an opening square bracket in the case of arrays ($var['a']).
        while (\false !== $next_token && \T_OPEN_SQUARE_BRACKET === $tokens[$next_token]['code'] && isset($tokens[$next_token]['bracket_closer'])) {
            $next_token = $phpcsFile->findNext(Tokens::$emptyTokens, $tokens[$next_token]['bracket_closer'] + 1, null, \true);
        }
     ...
     */
    public static function is_assignment(File $phpcsFile, $stackPtr, $include_coalesce = \true)
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false) {
            return \false;
        }
        static $valid = array(\T_VARIABLE => \true, \T_CLOSE_SQUARE_BRACKET => \true);
        // Must be a variable, constant or closing square bracket (see below).
        if (!isset($valid[$tokens[$stackPtr]['code']])) {
            return \false;
        }
        $next_non_empty = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        // No token found.
        ...
        if (\T_OPEN_SQUARE_BRACKET === $tokens[$next_non_empty]['code'] && isset($tokens[$next_non_empty]['bracket_closer'])) {
