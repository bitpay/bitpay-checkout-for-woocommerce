...
trait WPDBTrait
{
    ...
    protected function is_wpdb_method_call(File $phpcsFile, $stackPtr, $target_methods)
    {
        ...
        // Check for wpdb.
        if (\T_VARIABLE === $tokens[$stackPtr]['code'] && '$wpdb' !== $tokens[$stackPtr]['content'] || \T_STRING === $tokens[$stackPtr]['code'] && 'wpdb' !== \strtolower($tokens[$stackPtr]['content'])) {
            return \false;
        }
        // Check that this is a method call.
        $is_object_call = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if (\false === $is_object_call || isset(Collections::objectOperators()[$tokens[$is_object_call]['code']]) === \false) {
            return \false;
        }
        $methodPtr = $phpcsFile->findNext(Tokens::$emptyTokens, $is_object_call + 1, null, \true, null, \true);
        if (\false === $methodPtr) {
            return \false;
        }
        ...
        $opening_paren = $phpcsFile->findNext(Tokens::$emptyTokens, $methodPtr + 1, null, \true, null, \true);
        ...
        if (\T_OPEN_PARENTHESIS !== $tokens[$opening_paren]['code'] || !isset($tokens[$opening_paren]['parenthesis_closer'])) {
            return \false;
        }
