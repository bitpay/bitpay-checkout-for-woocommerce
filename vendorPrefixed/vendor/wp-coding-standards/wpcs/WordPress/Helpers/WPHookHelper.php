...
final class WPHookHelper
{
     ...
     */
    private static $hookInvokeFunctions = array('do_action' => array('position' => 1, 'name' => 'hook_name'), 'do_action_ref_array' => array('position' => 1, 'name' => 'hook_name'), 'do_action_deprecated' => array('position' => 1, 'name' => 'hook_name'), 'apply_filters' => array('position' => 1, 'name' => 'hook_name'), 'apply_filters_ref_array' => array('position' => 1, 'name' => 'hook_name'), 'apply_filters_deprecated' => array('position' => 1, 'name' => 'hook_name'));
    /**
     ...
     */
    public static function get_functions($include_deprecated = \true)
    {
        $hooks = \array_fill_keys(\array_keys(self::$hookInvokeFunctions), \true);
        if (\false === $include_deprecated) {
            unset($hooks['do_action_deprecated'], $hooks['apply_filters_deprecated']);
        }
        return $hooks;
    }
    /**
    ...
    public static function get_hook_name_param($function_name, array $parameters)
    {
        ...
        return PassedParameters::getParameterFromStack($parameters, self::$hookInvokeFunctions[$function_lc]['position'], self::$hookInvokeFunctions[$function_lc]['name']);
    }
}
