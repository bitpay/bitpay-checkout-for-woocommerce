...
namespace BitPayVendor\GuzzleHttp\Handler;

use BitPayVendor\GuzzleHttp\Exception\ConnectException;
use BitPayVendor\GuzzleHttp\Exception\RequestException;
use BitPayVendor\GuzzleHttp\Promise as P;
use BitPayVendor\GuzzleHttp\Promise\FulfilledPromise;
use BitPayVendor\GuzzleHttp\Promise\PromiseInterface;
use BitPayVendor\GuzzleHttp\Psr7\LazyOpenStream;
use BitPayVendor\GuzzleHttp\TransferStats;
use BitPayVendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BitPayVendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BitPayVendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
