...
final class BCFile
{
    ...
    public static function getDeclarationName(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $stackPtr + 1; $i < $phpcsFile->numTokens; $i++) {
            if ($tokens[$i]['code'] === \T_STRING || $tokens[$i]['code'] === T_SELF || $tokens[$i]['code'] === T_PARENT || $tokens[$i]['code'] === \T_STATIC) {
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (isset(Collections::functionDeclarationTokens()[$tokens[$stackPtr]['code']]) === \false && $tokens[$stackPtr]['code'] !== \T_USE) {
        ...
        if ($tokens[$stackPtr]['code'] === \T_USE) {
            ...
            if ($opener === \false || isset($tokens[$opener]['parenthesis_owner']) === \true && $tokens[$opener]['parenthesis_owner'] !== $stackPtr) {
        ...
        for ($i = $paramStart; $i <= $closer; $i++) {
            ...
            switch ($tokens[$i]['code']) {
                ...
                case T_COMMA:
                    ...
                    $vars[$paramCount]['content'] = \trim($phpcsFile->getTokensAsString($paramStart, $i - $paramStart));
                    if ($defaultStart !== null) {
                        $vars[$paramCount]['default'] = \trim($phpcsFile->getTokensAsString($defaultStart, $i - $defaultStart));
     ...
     */
    public static function getCondition(File $phpcsFile, $stackPtr, $type, $first = \true)
    {
