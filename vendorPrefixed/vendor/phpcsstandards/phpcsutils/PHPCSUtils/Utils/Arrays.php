...
final class Arrays
{
    ...
    public static function getOpenClose(File $phpcsFile, $stackPtr, $isShortArray = null)
    {
        ...
        if (isset($tokens[$stackPtr]) === \false || isset(Collections::arrayOpenTokensBC()[$tokens[$stackPtr]['code']]) === \false) {
            return \false;
        }
        switch ($tokens[$stackPtr]['code']) {
            ...
            case \T_OPEN_SQUARE_BRACKET:
                if ($isShortArray === \true || self::isShortArray($phpcsFile, $stackPtr) === \true) {
                    $opener = $stackPtr;
    ...
    public static function getDoubleArrowPtr(File $phpcsFile, $start, $end)
    {
        ...
        if (isset($tokens[$start], $tokens[$end]) === \false || $start > $end) {
            throw new RuntimeException('Invalid start and/or end position passed to getDoubleArrowPtr().' . ' Received: $start ' . $start . ', $end ' . $end);
        }
        $cacheId = "{$start}-{$end}";
        if (Cache::isCached($phpcsFile, __METHOD__, $cacheId) === \true) {
        ...
        ++$end;
        do {
            $doubleArrow = $phpcsFile->findNext($targets, $doubleArrow + 1, $end);
            ...
            if ((isset(Collections::closedScopes()[$tokens[$doubleArrow]['code']]) === \true || $tokens[$doubleArrow]['code'] === \T_FN || $tokens[$doubleArrow]['code'] === \T_MATCH) && isset($tokens[$doubleArrow]['scope_closer']) === \true) {
                $doubleArrow = $tokens[$doubleArrow]['scope_closer'];
                continue;
            }
            // Skip over attributes which may contain arrays as a passed parameters.
            if ($tokens[$doubleArrow]['code'] === \T_ATTRIBUTE && isset($tokens[$doubleArrow]['attribute_closer'])) {
                $doubleArrow = $tokens[$doubleArrow]['attribute_closer'];
                continue;
            }
            // Skip over potentially keyed long lists.
            if ($tokens[$doubleArrow]['code'] === \T_LIST && isset($tokens[$doubleArrow]['parenthesis_closer'])) {
