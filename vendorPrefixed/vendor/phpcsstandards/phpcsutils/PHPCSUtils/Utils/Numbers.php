...
final class Numbers
{
    ...
    public static function getCompleteNumber(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]) === \false || $tokens[$stackPtr]['code'] !== \T_LNUMBER && $tokens[$stackPtr]['code'] !== \T_DNUMBER) {
            throw new RuntimeException('Token type "' . $tokens[$stackPtr]['type'] . '" is not T_LNUMBER or T_DNUMBER');
        }
        $content = $tokens[$stackPtr]['content'];
        return ['orig_content' => $content, 'content' => \str_replace('_', '', $content), 'code' => $tokens[$stackPtr]['code'], 'type' => $tokens[$stackPtr]['type'], 'decimal' => self::getDecimalValue($content), 'last_token' => $stackPtr];
    }
    /**
    ...
    public static function getDecimalValue($textString)
    {
        ...
        if (self::isDecimalInt($textString) === \true) {
            return $textString;
        }
        if (self::isHexidecimalInt($textString) === \true) {
            return (string) \hexdec($textString);
        }
        if (self::isBinaryInt($textString) === \true) {
            return (string) \bindec($textString);
        }
        if (self::isOctalInt($textString) === \true) {
            return (string) \octdec($textString);
        }
        if (self::isFloat($textString) === \true) {
            return $textString;
        }
        return \false;
    }
