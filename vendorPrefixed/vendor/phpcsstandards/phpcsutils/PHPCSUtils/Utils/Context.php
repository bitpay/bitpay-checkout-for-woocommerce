...
final class Context
{
    ...
    public static function inAttribute(File $phpcsFile, $stackPtr)
    {
        ...
        return $stackPtr !== $tokens[$stackPtr]['attribute_opener'] && $stackPtr !== $tokens[$stackPtr]['attribute_closer'];
    }
    /**
    ...
    public static function inForeachCondition(File $phpcsFile, $stackPtr)
    {
        ...
        $asPtr = $phpcsFile->findNext(\T_AS, $tokens[$foreach]['parenthesis_opener'] + 1, $tokens[$foreach]['parenthesis_closer']);
        if ($asPtr === \false) {
    ...
    public static function inForCondition(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $opener + 1; $i < $closer; $i++) {
            ...
            if ($tokens[$i]['level'] !== $level || \count($tokens[$i]['nested_parenthesis']) !== $parens) {
