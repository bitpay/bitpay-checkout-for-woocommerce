...
final class Namespaces
{
    ...
    public static function getType(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($findAfter) === \false) {
             ...
             */
            $findAfter = Tokens::$assignmentTokens + Tokens::$comparisonTokens + Tokens::$operators + Tokens::$castTokens + Tokens::$blockOpeners + Collections::incrementDecrementOperators() + Collections::objectOperators() + Collections::shortArrayListOpenTokensBC();
        ...
        if (empty($tokens[$stackPtr]['conditions']) === \false || empty($tokens[$stackPtr]['nested_parenthesis']) === \false) {
            /*
        ...
        if ($start === $stackPtr && ($tokens[$next]['code'] === \T_STRING || $tokens[$next]['code'] === \T_NAME_QUALIFIED || $tokens[$next]['code'] === \T_OPEN_CURLY_BRACKET)) {
            return 'declaration';
        }
        if (($tokens[$next]['code'] === \T_NS_SEPARATOR || $tokens[$next]['code'] === \T_NAME_FULLY_QUALIFIED) && ($start !== $stackPtr || $phpcsFile->findNext($findAfter, $stackPtr + 1, null, \false, null, \true) !== \false)) {
            return 'operator';
        }
        return '';
    }
     ...
     */
    public static function getDeclaredName(File $phpcsFile, $stackPtr, $clean = \true)
    {
        try {
        ...
        $next = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, $endOfStatement + 1, \true);
    ...
    public static function findNamespacePtr(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_NAMESPACE && self::isDeclaration($phpcsFile, $stackPtr) === \true) {
            return \false;
        }
        ...
        $find = [\T_NAMESPACE, \T_CLOSE_CURLY_BRACKET, \T_CLOSE_PARENTHESIS, \T_CLOSE_SHORT_ARRAY, \T_CLOSE_SQUARE_BRACKET, \T_DOC_COMMENT_CLOSE_TAG, \T_ATTRIBUTE_END];
        $returnValue = \false;
        do {
            ...
            if ($tokens[$prev]['code'] === \T_CLOSE_CURLY_BRACKET) {
                ...
                if (isset($tokens[$prev]['scope_condition']) === \true && $tokens[$tokens[$prev]['scope_condition']]['code'] === \T_NAMESPACE) {
                    break;
                }
                // Skip over other scoped structures for efficiency.
                if (isset($tokens[$prev]['scope_condition']) === \true) {
                    $prev = $tokens[$prev]['scope_condition'];
                } elseif (isset($tokens[$prev]['scope_opener']) === \true) {
            ...
            if (isset($tokens[$prev]['bracket_opener']) === \true) {
            ...
            if (isset($tokens[$prev]['parenthesis_owner']) === \true) {
                ...
                continue;
            } elseif (isset($tokens[$prev]['parenthesis_opener']) === \true) {
            ...
            if ($tokens[$prev]['code'] === \T_NAMESPACE && self::isDeclaration($phpcsFile, $prev) === \true) {
