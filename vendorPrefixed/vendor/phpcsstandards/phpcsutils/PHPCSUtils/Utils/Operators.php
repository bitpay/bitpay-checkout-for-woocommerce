...
final class Operators
{
     ...
     */
    private static $extraUnaryIndicators = [\T_STRING_CONCAT => \true, \T_RETURN => \true, \T_EXIT => \true, \T_CONTINUE => \true, \T_BREAK => \true, \T_ECHO => \true, \T_PRINT => \true, \T_YIELD => \true, \T_COMMA => \true, \T_OPEN_PARENTHESIS => \true, \T_OPEN_SQUARE_BRACKET => \true, \T_OPEN_SHORT_ARRAY => \true, \T_OPEN_CURLY_BRACKET => \true, \T_COLON => \true, \T_INLINE_THEN => \true, \T_INLINE_ELSE => \true, \T_CASE => \true, \T_FN_ARROW => \true, \T_MATCH_ARROW => \true];
    /**
    ...
    public static function isReference(File $phpcsFile, $stackPtr)
    {
        ...
        $tokenBefore = $phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if (isset(Collections::functionDeclarationTokens()[$tokens[$tokenBefore]['code']]) === \true) {
        ...
        $tokenAfter = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        ...
        if ($lastOpener !== \false) {
            ...
            if (isset(Collections::functionDeclarationTokens()[$tokens[$lastOwner]['code']]) === \true || $tokens[$lastOwner]['code'] === \T_USE) {
         ...
         */
        if ($tokens[$tokenBefore]['code'] === \T_OPEN_PARENTHESIS || $tokens[$tokenBefore]['code'] === \T_COMMA || $tokens[$tokenBefore]['code'] === \T_OPEN_SHORT_ARRAY) {
            if ($tokens[$tokenAfter]['code'] === \T_VARIABLE) {
                ...
                $skip[] = \T_DOUBLE_COLON;
                $nextSignificantAfter = $phpcsFile->findNext($skip, $stackPtr + 1, null, \true);
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || $tokens[$stackPtr]['code'] !== \T_PLUS && $tokens[$stackPtr]['code'] !== \T_MINUS) {
            return \false;
        }
        ...
        if (isset(Tokens::$operators[$tokens[$next]['code']]) === \true) {
         ...
         */
        if (isset(Tokens::$operators[$tokens[$prev]['code']]) === \true || isset(Tokens::$comparisonTokens[$tokens[$prev]['code']]) === \true || isset(Tokens::$booleanOperators[$tokens[$prev]['code']]) === \true || isset(Tokens::$assignmentTokens[$tokens[$prev]['code']]) === \true || isset(Tokens::$castTokens[$tokens[$prev]['code']]) === \true || isset(self::$extraUnaryIndicators[$tokens[$prev]['code']]) === \true) {
            return \true;
        }
        return \false;
    }
