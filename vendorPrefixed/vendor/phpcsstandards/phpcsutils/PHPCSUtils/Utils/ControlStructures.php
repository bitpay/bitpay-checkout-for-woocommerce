...
final class ControlStructures
{
     ...
     */
    public static function hasBody(File $phpcsFile, $stackPtr, $allowEmpty = \true)
    {
        $tokens = $phpcsFile->getTokens();
        // Check for the existence of the token.
        if (isset($tokens[$stackPtr]) === \false || isset(Collections::controlStructureTokens()[$tokens[$stackPtr]['code']]) === \false) {
            return \false;
        }
        // Handle `else if`.
        ...
        if (isset($tokens[$stackPtr]['scope_opener']) === \true) {
            ...
            $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $start, $end, \true);
        ...
        $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $searchStart, null, \true);
        if ($nextNonEmpty === \false || $tokens[$nextNonEmpty]['code'] === \T_SEMICOLON || $tokens[$nextNonEmpty]['code'] === \T_CLOSE_TAG) {
        ...
        if ($tokens[$nextNonEmpty]['code'] === \T_OPEN_CURLY_BRACKET) {
            ...
            $nextNext = $phpcsFile->findNext(Tokens::$emptyTokens, $nextNonEmpty + 1, null, \true);
            if ($nextNext === \false) {
                return \false;
            }
            return \true;
    ...
    public static function isElseIf(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_ELSE && isset($tokens[$stackPtr]['scope_opener']) === \true) {
            return \false;
        }
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || $tokens[$stackPtr]['code'] !== \T_CATCH) {
        ...
        for ($i = $opener + 1; $i <= $closer; $i++) {
            ...
            if (isset(Collections::namespacedNameTokens()[$tokens[$i]['code']]) === \false) {
                ...
                if ($foundName !== '') {
                    $exceptions[] = ['type' => $foundName, 'type_token' => $firstToken, 'type_end_token' => $lastToken];
                }
