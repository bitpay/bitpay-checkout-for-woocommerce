...
final class Variables
{
    ...
    public static function getMemberProperties(File $phpcsFile, $stackPtr)
    {
        ...
        $isStatic = \false;
        $isReadonly = \false;
        $startOfStatement = $phpcsFile->findPrevious([\T_SEMICOLON, \T_OPEN_CURLY_BRACKET, \T_CLOSE_CURLY_BRACKET, \T_ATTRIBUTE_END], $stackPtr - 1);
        ...
        if ($i < $stackPtr) {
            ...
            for ($i; $i < $stackPtr; $i++) {
                ...
                if (isset($propertyTypeTokens[$tokens[$i]['code']]) === \true) {
                    $typeEndToken = $i;
                    if ($typeToken === \false) {
                        $typeToken = $i;
                    }
        ...
        $returnValue = ['scope' => $scope, 'scope_specified' => $scopeSpecified, 'is_static' => $isStatic, 'is_readonly' => $isReadonly, 'type' => $type, 'type_token' => $typeToken, 'type_end_token' => $typeEndToken, 'nullable_type' => $nullableType];
    ...
    public static function isPHPReservedVarName($name)
    {
        ...
        return isset(self::$phpReservedVars[$name]) === \true;
    }
    /**
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || $tokens[$stackPtr]['code'] !== \T_VARIABLE && $tokens[$stackPtr]['code'] !== \T_CONSTANT_ENCAPSED_STRING) {
            return \false;
        }
        ...
        if ($tokens[$stackPtr]['code'] === \T_CONSTANT_ENCAPSED_STRING) {
            ...
            if ($prev === \false || $tokens[$prev]['code'] !== \T_OPEN_SQUARE_BRACKET || ($next === \false || $tokens[$next]['code'] !== \T_CLOSE_SQUARE_BRACKET)) {
                // Not a single string array index key.
                return \false;
            }
            $pprev = $phpcsFile->findPrevious(Tokens::$emptyTokens, $prev - 1, null, \true);
            if ($pprev === \false || $tokens[$pprev]['code'] !== \T_VARIABLE || $tokens[$pprev]['content'] !== '$GLOBALS') {
