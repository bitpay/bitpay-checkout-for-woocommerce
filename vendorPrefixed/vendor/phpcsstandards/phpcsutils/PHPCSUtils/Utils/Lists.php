...
final class Lists
{
     ...
     * @var array
     */
    private static $listItemDefaults = ['raw' => '', 'assignment' => '', 'is_empty' => \false, 'is_nested_list' => \false, 'variable' => \false, 'assignment_token' => \false, 'assignment_end_token' => \false, 'assign_by_reference' => \false, 'reference_token' => \false];
    /**
    ...
    public static function getOpenClose(File $phpcsFile, $stackPtr, $isShortList = null)
    {
        ...
        if (isset($tokens[$stackPtr]) === \false || isset(Collections::listOpenTokensBC()[$tokens[$stackPtr]['code']]) === \false) {
            return \false;
        }
        switch ($tokens[$stackPtr]['code']) {
            ...
            case \T_OPEN_SQUARE_BRACKET:
                if ($isShortList === \true || self::isShortList($phpcsFile, $stackPtr) === \true) {
                    $opener = $stackPtr;
    ...
    public static function getAssignments(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $opener + 1; $i <= $closer; $i++) {
            ...
            switch ($tokens[$i]['code']) {
                case \T_DOUBLE_ARROW:
                    $keys['key'] = GetTokensAsString::compact($phpcsFile, $start, $lastNonEmpty, \true);
                    ...
                    $lastNonEmpty = null;
                    $list = null;
                    // Prevent confusion when short array was used as the key.
                    $reference = null;
                    break;
                case \T_COMMA:
                case $tokens[$closer]['code']:
                    ...
                    if ($start === null) {
                        $current['is_empty'] = \true;
                    } else {
                        $current['assignment'] = \trim(GetTokensAsString::compact($phpcsFile, $start, $lastNonEmpty, \true));
                        ...
                        if (isset($reference)) {
                            $current['assign_by_reference'] = \true;
                ...
                case \T_OPEN_SHORT_ARRAY:
                     ...
                     */
                    $nestedOpenClose = self::getOpenClose($phpcsFile, $i, \true);
                    $list = $i;
                ...
                default:
                    ...
                    if (isset($tokens[$i]['bracket_opener'], $tokens[$i]['bracket_closer']) && $i === $tokens[$i]['bracket_opener']) {
                        $i = $tokens[$i]['bracket_closer'];
                    } elseif ($tokens[$i]['code'] === \T_OPEN_PARENTHESIS && isset($tokens[$i]['parenthesis_closer'])) {
                        $i = $tokens[$i]['parenthesis_closer'];
                    } elseif (isset($tokens[$i]['scope_condition'], $tokens[$i]['scope_closer']) && $tokens[$i]['scope_condition'] === $i) {
                        $i = $tokens[$i]['scope_closer'];
                    } elseif ($tokens[$i]['code'] === \T_ATTRIBUTE && isset($tokens[$i]['attribute_closer'])) {
