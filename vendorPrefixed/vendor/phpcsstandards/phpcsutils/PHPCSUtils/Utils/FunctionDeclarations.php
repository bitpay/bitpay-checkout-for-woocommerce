...
final class FunctionDeclarations
{
    ...
    public static function getProperties(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]) === \false || isset(Collections::functionDeclarationTokens()[$tokens[$stackPtr]['code']]) === \false) {
            throw new RuntimeException('$stackPtr must be of type T_FUNCTION or T_CLOSURE or an arrow function');
        }
        if (Cache::isCached($phpcsFile, __METHOD__, $stackPtr) === \true) {
        ...
        if (isset($parenthesisCloser) === \true) {
            $scopeOpener = null;
            ...
            for ($i = $parenthesisCloser; $i < $phpcsFile->numTokens; $i++) {
                ...
                if (isset($returnTypeTokens[$tokens[$i]['code']]) === \true) {
        ...
        $returnValue = ['scope' => $scope, 'scope_specified' => $scopeSpecified, 'return_type' => $returnType, 'return_type_token' => $returnTypeToken, 'return_type_end_token' => $returnTypeEndToken, 'nullable_return_type' => $nullableReturnType, 'is_abstract' => $isAbstract, 'is_final' => $isFinal, 'is_static' => $isStatic, 'has_body' => $hasBody];
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || isset(Collections::functionDeclarationTokens()[$tokens[$stackPtr]['code']]) === \false && $tokens[$stackPtr]['code'] !== \T_USE) {
        ...
        if ($tokens[$stackPtr]['code'] === \T_USE) {
            ...
            if ($opener === \false || $tokens[$opener]['code'] !== \T_OPEN_PARENTHESIS || UseStatements::isClosureUse($phpcsFile, $stackPtr) === \false) {
        ...
        for ($i = $paramStart; $i <= $closer; $i++) {
            if (isset($parameterTypeTokens[$tokens[$i]['code']]) === \true || $tokens[$i]['code'] === \T_STATIC) {
            ...
            switch ($tokens[$i]['code']) {
                ...
                case \T_COMMA:
                    ...
                    $vars[$paramCount]['content'] = \trim(GetTokensAsString::normal($phpcsFile, $paramStart, $i - 1));
                    if ($defaultStart !== null) {
                        $vars[$paramCount]['default'] = \trim(GetTokensAsString::normal($phpcsFile, $defaultStart, $i - 1));
                    ...
                    break;
                case \T_EQUAL:
                    $defaultStart = $phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
                    $equalToken = $i;
    ...
    public static function isMagicFunction(File $phpcsFile, $stackPtr)
    {
        ...
        if (Scopes::isOOMethod($phpcsFile, $stackPtr) === \true) {
            return \false;
        }
    ...
    public static function isMagicFunctionName($name)
    {
        ...
        return isset(self::$magicFunctions[$name]) === \true;
    }
    /**
    ...
    {
        $name = \strtolower($name);
        return isset(self::$magicMethods[$name]) === \true;
    }
    /**
    ...
    {
        $name = \strtolower($name);
        return isset(self::$methodsDoubleUnderscore[$name]) === \true;
    }
    /**
    ...
    {
        if (self::isMagicMethod($phpcsFile, $stackPtr) === \true) {
            return \true;
        }
        if (self::isPHPDoubleUnderscoreMethod($phpcsFile, $stackPtr) === \true) {
            return \true;
        }
        return \false;
    }
    ...
    {
        $name = \strtolower($name);
        return isset(self::$magicMethods[$name]) === \true || isset(self::$methodsDoubleUnderscore[$name]) === \true;
    }
}
