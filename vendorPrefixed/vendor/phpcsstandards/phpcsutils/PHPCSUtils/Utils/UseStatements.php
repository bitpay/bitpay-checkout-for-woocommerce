...
final class UseStatements
{
    ...
    public static function getType(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['parenthesis_owner']) && $tokens[$stackPtr]['parenthesis_owner'] === $stackPtr) {
            return 'closure';
        }
        $prev = $phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if ($prev !== \false && $tokens[$prev]['code'] === \T_CLOSE_PARENTHESIS && Parentheses::isOwnerIn($phpcsFile, $prev, \T_CLOSURE) === \true) {
            return 'closure';
        }
        ...
        if (isset($traitScopes[$tokens[$lastCondition]['code']]) === \true) {
            return 'trait';
        }
        return '';
    }
    ...
    public static function splitImportUseStatement(File $phpcsFile, $stackPtr)
    {
        ...
        $statements = ['name' => [], 'function' => [], 'const' => []];
        ...
        for ($i = $stackPtr + 1; $i < $endOfStatement; $i++) {
            ...
            switch ($tokenCode) {
                ...
                case \T_NAME_FULLY_QUALIFIED:
                    // This would be a parse error, but handle it anyway.
                    /*
                     * PHPCS 4.x.
                     *
