...
final class ObjectDeclarations
{
    ...
    public static function getName(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]) === \false || ($tokens[$stackPtr]['code'] === \T_ANON_CLASS || $tokens[$stackPtr]['code'] === \T_CLOSURE)) {
            return null;
        }
        ...
        if ($tokenCode === \T_FUNCTION && \strtolower($tokens[$stackPtr]['content']) !== 'function') {
        ...
        if ($tokenCode === \T_FUNCTION && isset($tokens[$stackPtr]['parenthesis_opener']) === \true) {
            $stopPoint = $tokens[$stackPtr]['parenthesis_opener'];
        } elseif (isset($tokens[$stackPtr]['scope_opener']) === \true) {
        ...
        $exclude[] = \T_COLON;
        // Backed enums.
        $nameStart = $phpcsFile->findNext($exclude, $stackPtr + 1, $stopPoint, \true);
        if ($nameStart === \false) {
    ...
    public static function getClassProperties(File $phpcsFile, $stackPtr)
    {
        ...
        $properties = ['is_abstract' => \false, 'abstract_token' => \false, 'is_final' => \false, 'final_token' => \false, 'is_readonly' => \false, 'readonly_token' => \false];
        for ($i = $stackPtr - 1; $i > 0; $i--) {
            ...
            switch ($tokens[$i]['code']) {
                case \T_ABSTRACT:
                    $properties['is_abstract'] = \true;
                    $properties['abstract_token'] = $i;
                    break;
                case \T_FINAL:
                    $properties['is_final'] = \true;
                    $properties['final_token'] = $i;
                    break;
                case \T_READONLY:
                    $properties['is_readonly'] = \true;
    ...
    {
        return self::findNames($phpcsFile, $stackPtr, \T_EXTENDS, [\T_INTERFACE => \T_INTERFACE]);
    }
    /**
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || isset($allowedFor[$tokens[$stackPtr]['code']]) === \false || isset($tokens[$stackPtr]['scope_opener']) === \false) {
            return \false;
        }
        ...
        do {
            $start = $end + 1;
            $end = $phpcsFile->findNext($find, $start, $scopeOpener + 1, \true);
