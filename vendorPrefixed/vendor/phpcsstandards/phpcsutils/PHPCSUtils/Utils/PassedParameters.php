...
final class PassedParameters
{
     ...
     */
    private static $callParsingStopPoints = [\T_COMMA => \T_COMMA, \T_OPEN_SHORT_ARRAY => \T_OPEN_SHORT_ARRAY, \T_OPEN_SQUARE_BRACKET => \T_OPEN_SQUARE_BRACKET, \T_OPEN_PARENTHESIS => \T_OPEN_PARENTHESIS, \T_DOC_COMMENT_OPEN_TAG => \T_DOC_COMMENT_OPEN_TAG, \T_ATTRIBUTE => \T_ATTRIBUTE];
    /**
    ...
    public static function hasParameters(File $phpcsFile, $stackPtr, $isShortArray = null)
    {
        ...
        if (isset($tokens[$stackPtr]) === \false || isset(Collections::parameterPassingTokens()[$tokens[$stackPtr]['code']]) === \false) {
            throw new RuntimeException('The hasParameters() method expects a function call, array, isset or unset token to be passed.');
        }
        ...
        if (isset(Collections::ooHierarchyKeywords()[$tokens[$stackPtr]['code']]) === \true) {
            $prev = $phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
            if ($tokens[$prev]['code'] !== \T_NEW && ($next !== \false && $tokens[$next]['code'] !== \T_OPEN_PARENTHESIS)) {
                throw new RuntimeException('The hasParameters() method expects a function call, array, isset or unset token to be passed.');
            }
        }
        if (isset(Collections::shortArrayListOpenTokensBC()[$tokens[$stackPtr]['code']]) === \true && $isShortArray !== \true && Arrays::isShortArray($phpcsFile, $stackPtr) === \false) {
            throw new RuntimeException('The hasParameters() method expects a function call, array, isset or unset token to be passed.');
        }
        if ($next === \false) {
            return \false;
        ...
        if (isset(Collections::shortArrayListOpenTokensBC()[$tokens[$stackPtr]['code']]) === \true) {
        ...
        $ignore[\T_ELLIPSIS] = \T_ELLIPSIS;
        // Prevent PHP 8.1 first class callables from being seen as function calls.
        $closeParenthesis = $tokens[$next]['parenthesis_closer'];
        $nextNextNonEmpty = $phpcsFile->findNext($ignore, $next + 1, $closeParenthesis + 1, \true);
    ...
    public static function getParameters(File $phpcsFile, $stackPtr, $limit = 0, $isShortArray = null)
    {
        ...
        if (Cache::isCached($phpcsFile, __METHOD__, "{$stackPtr}-{$effectiveLimit}") === \true) {
            return Cache::get($phpcsFile, __METHOD__, "{$stackPtr}-{$effectiveLimit}");
        }
        if ($effectiveLimit !== 0 && Cache::isCached($phpcsFile, __METHOD__, "{$stackPtr}-0") === \true) {
            return \array_slice(Cache::get($phpcsFile, __METHOD__, "{$stackPtr}-0"), 0, $effectiveLimit, \true);
        }
        ...
        if (isset(Collections::shortArrayListOpenTokensBC()[$tokens[$stackPtr]['code']]) === \true) {
            ...
            $opener = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
            $closer = $tokens[$opener]['parenthesis_closer'];
        }
        $mayHaveNames = isset(Collections::functionCallTokens()[$tokens[$stackPtr]['code']]) === \true;
        $parameters = [];
        $nextComma = $opener;
        $paramStart = $opener + 1;
        $cnt = 1;
        ...
        while (($nextComma = $phpcsFile->findNext($stopPoints, $nextComma + 1, $closer + 1)) !== \false) {
            ...
            if (isset($tokens[$nextComma]['bracket_opener'], $tokens[$nextComma]['bracket_closer']) && $nextComma === $tokens[$nextComma]['bracket_opener']) {
                $nextComma = $tokens[$nextComma]['bracket_closer'];
                continue;
            }
            // Skip past nested arrays, function calls and arbitrary groupings.
            if ($tokens[$nextComma]['code'] === \T_OPEN_PARENTHESIS && isset($tokens[$nextComma]['parenthesis_closer'])) {
                $nextComma = $tokens[$nextComma]['parenthesis_closer'];
                continue;
            }
            // Skip past closures, anonymous classes and anything else scope related.
            if (isset($tokens[$nextComma]['scope_condition'], $tokens[$nextComma]['scope_closer']) && $tokens[$nextComma]['scope_condition'] === $nextComma) {
                $nextComma = $tokens[$nextComma]['scope_closer'];
                continue;
            }
            // Skip over potentially large docblocks.
            if ($tokens[$nextComma]['code'] === \T_DOC_COMMENT_OPEN_TAG && isset($tokens[$nextComma]['comment_closer'])) {
                $nextComma = $tokens[$nextComma]['comment_closer'];
                continue;
            }
            // Skip over attributes.
            if ($tokens[$nextComma]['code'] === \T_ATTRIBUTE && isset($tokens[$nextComma]['attribute_closer'])) {
                $nextComma = $tokens[$nextComma]['attribute_closer'];
                continue;
            }
            if ($tokens[$nextComma]['code'] !== \T_COMMA && $tokens[$nextComma]['code'] !== $tokens[$closer]['code']) {
                // Just in case.
                continue;
                // @codeCoverageIgnore
            }
            ...
            $key = $cnt;
            if ($mayHaveNames === \true) {
                $firstNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $paramStart, $paramEnd + 1, \true);
                if ($firstNonEmpty !== $paramEnd) {
                    $secondNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $firstNonEmpty + 1, $paramEnd + 1, \true);
                    if ($tokens[$secondNonEmpty]['code'] === \T_COLON && $tokens[$firstNonEmpty]['code'] === \T_PARAM_NAME) {
            ...
            $hasNextParam = $phpcsFile->findNext(Tokens::$emptyTokens, $nextComma + 1, $closer, \true);
    ...
    public static function getParameter(File $phpcsFile, $stackPtr, $paramOffset, $paramNames = [])
    {
        ...
        if (isset(Collections::functionCallTokens()[$tokens[$stackPtr]['code']]) === \false) {
            if (isset($parameters[$paramOffset]) === \true) {
    ...
    public static function getParameterFromStack(array $parameters, $paramOffset, $paramNames)
    {
        ...
        if (empty($paramNames) === \false) {
            ...
            foreach ($paramNames as $name) {
                ...
                if (isset($parameters[$name]) === \true) {
                    return $parameters[$name];
                }
            }
        }
        // Next check for positional parameters.
        if (isset($parameters[$paramOffset]) === \true && isset($parameters[$paramOffset]['name']) === \false) {
            return $parameters[$paramOffset];
        }
        if (empty($paramNames) === \true) {
            throw new RuntimeException('To allow for support for PHP 8 named parameters, the $paramNames parameter must be passed.');
        }
        return \false;
    }
}
