...
final class Cache
{
    ...
    public static function isCached(File $phpcsFile, $key, $id)
    {
        ...
        $loop = $phpcsFile->fixer->enabled === \true ? $phpcsFile->fixer->loops : 0;
        return isset(self::$cache[$loop][$fileName][$key]) && \array_key_exists($id, self::$cache[$loop][$fileName][$key]);
    }
    /**
    ...
    public static function get(File $phpcsFile, $key, $id)
    {
        ...
        $fileName = $phpcsFile->getFilename();
        $loop = $phpcsFile->fixer->enabled === \true ? $phpcsFile->fixer->loops : 0;
        if (isset(self::$cache[$loop][$fileName][$key]) && \array_key_exists($id, self::$cache[$loop][$fileName][$key])) {
    ...
    public static function getForFile(File $phpcsFile, $key)
    {
        ...
        $fileName = $phpcsFile->getFilename();
        $loop = $phpcsFile->fixer->enabled === \true ? $phpcsFile->fixer->loops : 0;
        if (isset(self::$cache[$loop][$fileName]) && \array_key_exists($key, self::$cache[$loop][$fileName])) {
    ...
    public static function set(File $phpcsFile, $key, $id, $value)
    {
         ...
         */
        if (isset(self::$cache[$loop]) === \false && empty(self::$cache) === \false) {
            self::clear();
        }
