...
final class IsShortArrayOrList
{
    ...
    public function __construct(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]) === \false || isset($openBrackets[$tokens[$stackPtr]['code']]) === \false) {
            throw new RuntimeException('The IsShortArrayOrList class expects to be passed a T_OPEN_SHORT_ARRAY or T_OPEN_SQUARE_BRACKET token.');
        }
        ...
        $this->beforeOpener = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $this->opener - 1, null, \true);
        $this->afterCloser = $this->phpcsFile->findNext(Tokens::$emptyTokens, $this->closer + 1, null, \true);
    ...
    public function solve()
    {
        ...
        if (Context::inAttribute($this->phpcsFile, $this->opener) === \true) {
            return self::SHORT_ARRAY;
        }
        ...
        if (isset($this->openBrackets[$this->tokens[$this->beforeOpener]['code']]) === \true || $this->tokens[$this->beforeOpener]['code'] === \T_COMMA || $this->tokens[$this->beforeOpener]['code'] === \T_DOUBLE_ARROW) {
            $hasRiskyTokenBeforeOpener = \true;
        }
        $hasRiskyTokenAfterCloser = \false;
        if ($this->tokens[$this->afterCloser]['code'] === \T_COMMA || $this->tokens[$this->afterCloser]['code'] === \T_CLOSE_SHORT_ARRAY || $this->tokens[$this->afterCloser]['code'] === \T_CLOSE_SQUARE_BRACKET) {
            $hasRiskyTokenAfterCloser = \true;
        }
         ...
         */
        if (isset($this->openBrackets[$this->tokens[$this->beforeOpener]['code']]) === \true) {
        ...
        if ($this->tokens[$this->afterCloser]['code'] === \T_COMMA) {
            ...
            $nextEffectiveAfterCloser = $this->phpcsFile->findNext(Tokens::$emptyTokens, $this->afterCloser + 1, null, \true);
        }
        if ($this->tokens[$nextEffectiveAfterCloser]['code'] === \T_CLOSE_SHORT_ARRAY || $this->tokens[$nextEffectiveAfterCloser]['code'] === \T_CLOSE_SQUARE_BRACKET) {
    ...
    private function isShortArrayBracket()
    {
        if ($this->tokens[$this->opener]['code'] === \T_OPEN_SQUARE_BRACKET) {
            if (\version_compare($this->phpcsVersion, '3.7.2', '>=') === \true) {
             ...
             */
            if ($this->tokens[$this->beforeOpener]['code'] === \T_CLOSE_PARENTHESIS && isset($this->tokens[$this->beforeOpener]['parenthesis_owner']) === \true && isset(Tokens::$scopeOpeners[$this->tokens[$this->tokens[$this->beforeOpener]['parenthesis_owner']]['code']]) === \true) {
                return \true;
            }
    ...
    private function walkInside($opener, $recursions = 0)
    {
        ...
        $items = PassedParameters::getParameters($this->phpcsFile, $opener, self::ITEM_LIMIT, \true);
        if ($items === []) {
            /*
        ...
        foreach ($items as $item) {
            ...
            if ($arrow === \false) {
                $firstNonEmptyInValue = $this->phpcsFile->findNext($skip, $item['start'], $item['end'] + 1, \true);
            } else {
                $firstNonEmptyInValue = $this->phpcsFile->findNext($skip, $arrow + 1, $item['end'] + 1, \true);
            }
            if ($this->tokens[$firstNonEmptyInValue]['code'] !== \T_VARIABLE && isset(Collections::namespacedNameTokens()[$this->tokens[$firstNonEmptyInValue]['code']]) === \false && isset(Collections::ooHierarchyKeywords()[$this->tokens[$firstNonEmptyInValue]['code']]) === \false && isset($this->openBrackets[$this->tokens[$firstNonEmptyInValue]['code']]) === \false) {
                return self::SHORT_ARRAY;
            }
            /*
             ...
             */
            if (isset(Collections::namespacedNameTokens()[$this->tokens[$firstNonEmptyInValue]['code']]) === \true || isset(Collections::ooHierarchyKeywords()[$this->tokens[$firstNonEmptyInValue]['code']]) === \true) {
                $nextAfter = $this->phpcsFile->findNext($skipNames, $firstNonEmptyInValue + 1, null, \true);
                if ($this->tokens[$nextAfter]['code'] !== \T_DOUBLE_COLON) {
                     ...
                     */
                    $nextNextAfter = $this->phpcsFile->findNext(Tokens::$emptyTokens, $nextAfter + 1, null, \true);
            ...
            if (isset($this->openBrackets[$this->tokens[$firstNonEmptyInValue]['code']]) === \true) {
                /*
                 ...
                 */
                $lastNonEmptyInValue = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $item['end'], $item['start'], \true);
                if (isset($this->tokens[$firstNonEmptyInValue]['bracket_closer']) === \true && $this->tokens[$firstNonEmptyInValue]['bracket_closer'] !== $lastNonEmptyInValue) {
                    return self::SHORT_ARRAY;
                }
                /*
    ...
    private function walkOutside()
    {
        ...
        for ($i = $this->opener - 1; $i >= 0; $i--) {
            ...
            if (isset(Tokens::$emptyTokens[$this->tokens[$i]['code']]) === \true) {
                continue;
            }
            // Stop on an end of statement.
            if (isset($stopPoints[$this->tokens[$i]['code']]) === \true) {
                // End of previous statement or start of document.
                return self::SHORT_ARRAY;
            }
            if (isset($this->tokens[$i]['scope_opener'], $this->tokens[$i]['scope_closer']) === \true) {
                if ($i === $this->tokens[$i]['scope_opener'] && $this->tokens[$i]['scope_closer'] > $this->closer) {
                    // Found a scope wrapping this set of brackets before finding a outer set of brackets.
                    // This will be a short array.
                    return self::SHORT_ARRAY;
                }
                if ($i === $this->tokens[$i]['scope_closer'] && isset($this->tokens[$i]['scope_condition']) === \true) {
            ...
            if (isset($this->tokens[$i]['parenthesis_opener'], $this->tokens[$i]['parenthesis_closer']) === \true) {
                if ($i === $this->tokens[$i]['parenthesis_opener'] && $this->tokens[$i]['parenthesis_closer'] > $this->closer) {
                    $beforeParensOpen = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $i - 1, null, \true);
             ...
             */
            if (isset($this->tokens[$i]['attribute_opener'], $this->tokens[$i]['attribute_closer']) === \true && $i === $this->tokens[$i]['attribute_closer']) {
             ...
             */
            if (isset($this->tokens[$i]['bracket_opener'], $this->tokens[$i]['bracket_closer']) === \true && $i === $this->tokens[$i]['bracket_closer']) {
                /*
                ...
                $prevNonEmpty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $adjOpener - 1, null, \true);
                $nextNonEmpty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
                if ($this->tokens[$prevNonEmpty]['code'] === $this->tokens[$this->beforeOpener]['code'] && $this->tokens[$nextNonEmpty]['code'] === $this->tokens[$this->afterCloser]['code'] && Cache::isCached($this->phpcsFile, IsShortArrayOrListWithCache::CACHE_KEY, $adjOpener) === \true) {
            ...
            // Open bracket.
            if (isset($this->openBrackets[$this->tokens[$i]['code']]) === \true) {
                if (isset($this->tokens[$i]['bracket_closer']) === \false || $this->tokens[$i]['code'] === \T_OPEN_SQUARE_BRACKET) {
                    /*
