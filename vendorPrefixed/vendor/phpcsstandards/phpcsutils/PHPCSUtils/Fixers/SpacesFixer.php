 ...
 */
namespace BitPayVendor\PHPCSUtils\Fixers;
...
final class SpacesFixer
{
     ...
     */
    public static function checkAndFix(File $phpcsFile, $stackPtr, $secondPtr, $expectedSpaces, $errorTemplate, $errorCode = 'Found', $errorType = 'error', $errorSeverity = 0, $metricName = '')
    {
        $tokens = $phpcsFile->getTokens();
        /*
         * Validate the received function input.
         */
        if (isset($tokens[$stackPtr], $tokens[$secondPtr]) === \false || $tokens[$stackPtr]['code'] === \T_WHITESPACE || $tokens[$secondPtr]['code'] === \T_WHITESPACE) {
        ...
        } elseif (\is_int($expectedSpaces) === \true && $expectedSpaces >= 0) {
            $expected = $expectedSpaces;
        } elseif (\is_string($expectedSpaces) === \true && Numbers::isDecimalInt($expectedSpaces) === \true) {
            $expected = (int) $expectedSpaces;
        }
        if ($expected === \false) {
            throw new RuntimeException('The $expectedSpaces setting should be either "newline", 0 or a positive integer');
        }
        $ptrA = $stackPtr;
        $ptrB = $secondPtr;
        ...
        $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $ptrA + 1, null, \true);
        if ($nextNonEmpty !== \false && $nextNonEmpty < $ptrB) {
            throw new RuntimeException('The $stackPtr and the $secondPtr token must be adjacent tokens separated only' . ' by whitespace and/or comments');
        }
        /*
        ...
        $fixable = \true;
        $nextNonWhitespace = $phpcsFile->findNext(\T_WHITESPACE, $ptrA + 1, null, \true);
        if ($nextNonWhitespace !== $ptrB) {
            // Comment found between the tokens and we don't know where it should go, so don't auto-fix.
            $fixable = \false;
        }
        if ($found === 'newline' && $tokens[$ptrA]['code'] === \T_COMMENT && \substr($tokens[$ptrA]['content'], -2) !== '*/') {
            /*
        ...
        $method = 'add';
        $method .= $fixable === \true ? 'Fixable' : '';
        $method .= $errorType === 'error' ? 'Error' : 'Warning';
        $recorded = $phpcsFile->{$method}($errorTemplate, $stackPtr, $errorCode, [$expectedPhrase, $foundPhrase], $errorSeverity);
