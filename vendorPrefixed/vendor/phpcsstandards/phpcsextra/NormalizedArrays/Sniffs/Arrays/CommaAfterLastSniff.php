...
final class CommaAfterLastSniff implements Sniff
{
     ...
     * @var array
     */
    private $validValues = ['enforce' => \true, 'forbid' => \true, 'skip' => \true];
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $lastNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $closer - 1, $opener, \true);
        if ($lastNonEmpty === \false || $lastNonEmpty === $opener) {
            // Bow out: empty array.
            return;
        }
        $isComma = $tokens[$lastNonEmpty]['code'] === \T_COMMA;
        $phpcsFile->recordMetric($stackPtr, \sprintf(self::METRIC_NAME, \ucfirst($phrase)), $isComma === \true ? 'yes' : 'no');
        switch ($action) {
            case 'enforce':
                if ($isComma === \true) {
                    return;
                ...
                if ($fix === \true) {
                    $extraContent = ',';
                    if (($tokens[$lastNonEmpty]['code'] === \T_END_HEREDOC || $tokens[$lastNonEmpty]['code'] === \T_END_NOWDOC) && $tokens[$lastNonEmpty]['content'] === \ltrim($tokens[$lastNonEmpty]['content'])) {
            ...
            case 'forbid':
                ...
                if ($fix === \true) {
                    ...
                    $prevNonWhitespace = $phpcsFile->findPrevious(\T_WHITESPACE, $lastNonEmpty - 1, $opener, \true);
                    $nextNonWhitespace = $phpcsFile->findNext(\T_WHITESPACE, $lastNonEmpty + 1, $closer + 1, \true);
                    if ($prevNonWhitespace !== \false && $tokens[$prevNonWhitespace]['line'] < $tokens[$lastNonEmpty]['line'] && $nextNonWhitespace !== \false && $tokens[$nextNonWhitespace]['line'] > $tokens[$lastNonEmpty]['line']) {
