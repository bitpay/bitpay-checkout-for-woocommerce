...
final class ArrayBraceSpacingSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->keywordSpacing === \false && $this->spacesSingleLine === \false && $this->spacesMultiLine === \false && $this->spacesWhenEmpty === \false) {
        ...
        if ($tokens[$stackPtr]['code'] === \T_ARRAY && $this->keywordSpacing !== \false) {
            ...
            SpacesFixer::checkAndFix($phpcsFile, $stackPtr, $opener, $this->keywordSpacing, $error, $code, 'error', 0, 'Space between array keyword and open brace');
        }
        /*
         * Check for empty arrays.
         */
        $nextNonWhiteSpace = $phpcsFile->findNext(\T_WHITESPACE, $opener + 1, null, \true);
        if ($nextNonWhiteSpace === $closer) {
            ...
            SpacesFixer::checkAndFix($phpcsFile, $opener, $closer, $this->spacesWhenEmpty, $error, $code, 'error', 0, 'Space between open and close brace for an empty array');
            return;
        }
        /*
        ...
        if ($tokens[$opener]['line'] === $tokens[$closer]['line']) {
            ...
            SpacesFixer::checkAndFix($phpcsFile, $opener, $phpcsFile->findNext(\T_WHITESPACE, $opener + 1, null, \true), $this->spacesSingleLine, $error, $code, 'error', 0, 'Space after array opener, single line array');
            $error = 'Expected %s before the array closer in a single line array. Found: %s';
            $code = 'SpaceBeforeArrayCloserSingleLine';
            SpacesFixer::checkAndFix($phpcsFile, $closer, $phpcsFile->findPrevious(\T_WHITESPACE, $closer - 1, null, \true), $this->spacesSingleLine, $error, $code, 'error', 0, 'Space before array closer, single line array');
            return;
        }
        // Multi-line array.
        ...
        if ($this->spacesMultiLine === 'newline') {
            ...
            if (($tokens[$nextNonWhitespace]['code'] === \T_COMMENT || isset(Tokens::$phpcsCommentTokens[$tokens[$nextNonWhitespace]['code']]) === \true) && $tokens[$nextNonWhitespace]['line'] === $tokens[$opener]['line']) {
                // We found a trailing comment after array opener. Treat that as the opener instead.
                $opener = $nextNonWhitespace;
                $nextNonWhitespace = $phpcsFile->findNext(\T_WHITESPACE, $opener + 1, null, \true);
            }
        }
        SpacesFixer::checkAndFix($phpcsFile, $opener, $nextNonWhitespace, $this->spacesMultiLine, $error, $code, 'error', 0, 'Space after array opener, multi-line array');
        $error = 'Expected %s before the array closer in a multi line array. Found: %s';
        $code = 'SpaceBeforeArrayCloserMultiLine';
        SpacesFixer::checkAndFix($phpcsFile, $closer, $phpcsFile->findPrevious(\T_WHITESPACE, $closer - 1, null, \true), $this->spacesMultiLine, $error, $code, 'error', 0, 'Space before array closer, multi-line array');
    }
}
