...
final class OneStatementInShortEchoTagSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        for ($endOfStatement = $stackPtr + 1; $endOfStatement < $phpcsFile->numTokens; $endOfStatement++) {
            if ($tokens[$endOfStatement]['code'] === \T_CLOSE_TAG || $tokens[$endOfStatement]['code'] === \T_SEMICOLON) {
                break;
            }
            // Skip over anything within parenthesis.
            if ($tokens[$endOfStatement]['code'] === \T_OPEN_PARENTHESIS && isset($tokens[$endOfStatement]['parenthesis_closer'])) {
                $endOfStatement = $tokens[$endOfStatement]['parenthesis_closer'];
            }
        }
        if ($endOfStatement === $phpcsFile->numTokens || $tokens[$endOfStatement]['code'] === \T_CLOSE_TAG) {
            return;
        }
        // Semi-colon, so check for any code between it and the close tag.
        $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $endOfStatement + 1, null, \true);
        if ($nextNonEmpty === \false || $tokens[$nextNonEmpty]['code'] === \T_CLOSE_TAG) {
            return;
        }
        $fix = $phpcsFile->addFixableError('Only one statement is allowed when using short open echo PHP tags.' . ' Use the "<?php" open tag for multi-statement PHP.', $nextNonEmpty, 'MultipleFound');
        if ($fix === \true) {
