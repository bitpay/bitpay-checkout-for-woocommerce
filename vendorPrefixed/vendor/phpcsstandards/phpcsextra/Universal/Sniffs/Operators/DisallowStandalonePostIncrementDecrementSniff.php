...
final class DisallowStandalonePostIncrementDecrementSniff implements Sniff
{
     ...
     * @var array
     */
    private $allowedTokens = [\T_VARIABLE => \T_VARIABLE];
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset(Collections::incrementDecrementOperators()[$tokens[$end]['code']])) {
            ...
            $end = $phpcsFile->findNext(Tokens::$emptyTokens, $end + 1, null, \true);
        }
        if ($tokens[$end]['code'] !== \T_SEMICOLON && $tokens[$end]['code'] !== \T_CLOSE_TAG) {
            // Not a stand-alone statement.
            return $end;
        }
        $counter = 0;
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            if (isset($operators[$tokens[$i]['code']]) === \true) {
                $lastCode = $tokens[$i]['code'];
                ++$counter;
                continue;
            }
            if (isset($this->allowedTokens[$tokens[$i]['code']]) === \true) {
                $lastCode = $tokens[$i]['code'];
                continue;
            }
            if ($tokens[$i]['code'] === \T_OPEN_SQUARE_BRACKET && isset($tokens[$i]['bracket_closer']) && ($lastCode === \T_VARIABLE || $lastCode === \T_STRING)) {
                // Array access.
        ...
        if ($counter > 1) {
            $phpcsFile->addWarning('Using multiple increment/decrement operators in a stand-alone statement is strongly discouraged.' . ' Found: %s', $stackPtr, 'MultipleOperatorsFound', [GetTokensAsString::compact($phpcsFile, $start, $end - 1, \true)]);
            return $end;
        }
        $type = 'increment';
        ...
        $lastNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $end - 1, $start, \true);
        ...
        $replacement .= GetTokensAsString::compact($phpcsFile, $start, $lastNonEmpty - 1, \true);
