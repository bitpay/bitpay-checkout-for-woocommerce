...
final class DisallowMixedGroupUseSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($totalCount === 1 || $ooCount !== 0 && $functionCount === 0 && $constantCount === 0 || $ooCount === 0 && $functionCount !== 0 && $constantCount === 0 || $ooCount === 0 && $functionCount === 0 && $constantCount !== 0) {
        ...
        $error = 'Group use statements should import one type of construct.' . ' Mixed group use statement found importing %s.';
        $code = 'Found';
        $data = [$found];
        ...
        if ($tokens[$endOfStatement]['code'] !== \T_SEMICOLON) {
            $endRemoval = $phpcsFile->findPrevious(Tokens::$emptyTokens, $endOfStatement - 1, null, \true);
        }
