...
final class CommaSpacingSniff implements Sniff
{
    ...
    protected function processSpacingBefore(File $phpcsFile, $stackPtr)
    {
        ...
        if ($prevNonWhitespace !== $prevNonEmpty && $tokens[$prevNonEmpty]['code'] !== \T_COMMA && $tokens[$prevNonEmpty]['line'] !== $tokens[$nextNonEmpty]['line']) {
            // Special case: comma after a trailing comment - the comma should be moved to before the comment.
            $fix = $phpcsFile->addFixableError('Comma found after comment, expected the comma after the end of the code', $stackPtr, 'CommaAfterComment');
            if ($fix === \true) {
                ...
                if ($tokens[$stackPtr - 1]['code'] === \T_WHITESPACE && $tokens[$stackPtr - 1]['line'] === $tokens[$stackPtr]['line'] && $tokens[$stackPtr]['line'] !== $tokens[$nextNonWhitespace]['line']) {
        ...
        if (isset(Tokens::$blockOpeners[$tokens[$prevNonWhitespace]['code']]) === \true || $tokens[$prevNonWhitespace]['code'] === \T_OPEN_SHORT_ARRAY || $tokens[$prevNonWhitespace]['code'] === \T_OPEN_USE_GROUP) {
            // Should only realistically be possible for lists. Leave for a block brace spacing sniff to sort out.
            return;
        }
        $expectedSpaces = 0;
        if ($tokens[$prevNonEmpty]['code'] === \T_END_HEREDOC || $tokens[$prevNonEmpty]['code'] === \T_END_NOWDOC) {
            /*
            ...
            if ($this->phpVersion === 0 && \ltrim($tokens[$prevNonEmpty]['content']) === $tokens[$prevNonEmpty]['content']) {
                $expectedSpaces = 'newline';
            }
        }
        $error = 'Expected %1$s between "' . $this->escapePlaceholders($tokens[$prevNonWhitespace]['content']) . '" and the comma. Found: %2$s';
        $codeSuffix = $this->getSuffix($phpcsFile, $stackPtr);
        $metricSuffix = $this->codeSuffixToMetric($codeSuffix);
        SpacesFixer::checkAndFix($phpcsFile, $stackPtr, $prevNonWhitespace, $expectedSpaces, $error, 'SpaceBefore' . $codeSuffix, 'error', 0, self::METRIC_NAME_BEFORE . $metricSuffix);
    }
    /**
    ...
    protected function processSpacingAfter(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$nextNonWhitespace]['code'] === \T_CLOSE_CURLY_BRACKET || $tokens[$nextNonWhitespace]['code'] === \T_CLOSE_SQUARE_BRACKET || $tokens[$nextNonWhitespace]['code'] === \T_CLOSE_PARENTHESIS || $tokens[$nextNonWhitespace]['code'] === \T_CLOSE_SHORT_ARRAY || $tokens[$nextNonWhitespace]['code'] === \T_CLOSE_USE_GROUP) {
            // Ignore. Leave for a block spacing sniff to sort out.
            return;
        }
        $nextToken = $tokens[$stackPtr + 1];
        $error = 'Expected %1$s between the comma and "' . $this->escapePlaceholders($tokens[$nextNonWhitespace]['content']) . '". Found: %2$s';
        $codeSuffix = $this->getSuffix($phpcsFile, $stackPtr);
        $metricSuffix = $this->codeSuffixToMetric($codeSuffix);
        if ($nextToken['code'] === \T_WHITESPACE) {
            ...
            $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
            if (isset(Tokens::$commentTokens[$tokens[$nextNonWhitespace]['code']]) === \true && ($nextNonEmpty === \false || $tokens[$stackPtr]['line'] !== $tokens[$nextNonEmpty]['line'])) {
                // Separate error code to allow for aligning trailing comments.
                $errorCode = 'TooMuchSpaceAfterCommaBeforeTrailingComment';
            }
            SpacesFixer::checkAndFix($phpcsFile, $stackPtr, $nextNonWhitespace, 1, $error, $errorCode, 'error', 0, self::METRIC_NAME_AFTER . $metricSuffix);
            return;
        }
        SpacesFixer::checkAndFix($phpcsFile, $stackPtr, $nextNonWhitespace, 1, $error, 'NoSpaceAfter' . $codeSuffix, 'error', 0, self::METRIC_NAME_AFTER . $metricSuffix);
    }
    /**
    ...
    private function getSuffix($phpcsFile, $stackPtr)
    {
        ...
        $prevNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $opener - 1, null, \true);
        if (isset(Collections::nameTokens()[$tokens[$prevNonEmpty]['code']]) === \true) {
            return 'InFunctionCall';
        }
