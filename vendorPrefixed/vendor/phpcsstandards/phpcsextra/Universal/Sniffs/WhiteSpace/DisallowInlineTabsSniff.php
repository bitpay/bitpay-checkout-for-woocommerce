...
final class DisallowInlineTabsSniff implements Sniff
{
     ...
     * @var array
     */
    private $find = [\T_WHITESPACE => \true, \T_DOC_COMMENT_WHITESPACE => \true, \T_DOC_COMMENT_STRING => \true, \T_COMMENT => \true];
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = 0; $i < $phpcsFile->numTokens; $i++) {
            ...
            if (isset($this->find[$tokens[$i]['code']]) === \false || ($tokens[$i]['code'] === \T_WHITESPACE || $tokens[$i]['code'] === \T_DOC_COMMENT_WHITESPACE) && $tokens[$i]['column'] === 1) {
                continue;
            }
            ...
            if (($tokens[$i]['code'] === \T_COMMENT || isset(Tokens::$phpcsCommentTokens[$tokens[$i]['code']])) && $tokens[$i]['column'] === 1 && ($tokens[$i - 1]['code'] === \T_COMMENT || isset(Tokens::$phpcsCommentTokens[$tokens[$i - 1]['code']]))) {
                $multiLineComment = \true;
            }
            if ($multiLineComment === \true) {
            ...
            $fix = $phpcsFile->addFixableError('Spaces must be used for mid-line alignment; tabs are not allowed', $i, 'NonIndentTabsUsed');
            if ($fix === \false) {
                continue;
            }
            $indent = '';
