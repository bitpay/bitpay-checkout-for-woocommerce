...
final class PrecisionAlignmentSniff implements Sniff
{
     ...
     */
    private $tokensToCheck = [\T_WHITESPACE => \T_WHITESPACE, \T_INLINE_HTML => \T_INLINE_HTML, \T_DOC_COMMENT_WHITESPACE => \T_DOC_COMMENT_WHITESPACE, \T_COMMENT => \T_COMMENT, \T_END_HEREDOC => \T_END_HEREDOC, \T_END_NOWDOC => \T_END_NOWDOC];
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
         ...
         */
        if (isset($this->tabWidth) === \false || \defined('PHP_CODESNIFFER_IN_TESTS') === \true) {
            $this->tabWidth = Helper::getTabWidth($phpcsFile);
        }
        if (isset($this->indent) === \true) {
        ...
        for ($i = 0; $i < $phpcsFile->numTokens; $i++) {
            ...
            if (isset($ignoreTokens[$tokens[$i]['type']]) === \true || isset($tokens[$i + 1]) && isset($ignoreTokens[$tokens[$i + 1]['type']])) {
            ...
            switch ($tokens[$i]['code']) {
                case \T_WHITESPACE:
                    if ($this->ignoreBlankLines === \true && isset($tokens[$i + 1]) && $tokens[$i]['line'] !== $tokens[$i + 1]['line']) {
                ...
                case \T_DOC_COMMENT_WHITESPACE:
                     ...
                     */
                    if ($this->ignoreBlankLines === \true && isset($tokens[$i + 1]) && $tokens[$i + 1]['content'] === $phpcsFile->eolChar && isset($tokens[$i + 2]) && $tokens[$i]['line'] !== $tokens[$i + 2]['line']) {
                        // Skip blank lines which only contain trailing whitespace.
                        continue 2;
                    }
                    $spaces = $tokens[$i]['length'] % $indent;
                    if (isset($tokens[$i + 1]) === \true && ($tokens[$i + 1]['code'] === \T_DOC_COMMENT_STAR || $tokens[$i + 1]['code'] === \T_DOC_COMMENT_CLOSE_TAG) && $spaces !== 0) {
                ...
                case \T_COMMENT:
                case \T_INLINE_HTML:
                    if ($this->ignoreBlankLines === \true && \trim($tokens[$i]['content']) === '' && isset($tokens[$i + 1]) && $tokens[$i]['line'] !== $tokens[$i + 1]['line']) {
                        // Skip blank lines which only contain trailing whitespace.
                        continue 2;
                    }
                ...
                case \T_PHPCS_IGNORE_FILE:
                     ...
                     */
                    if (isset($content[0]) === \true && $content[0] === '*' && $spaces !== 0) {
                        --$spaces;
                    }
                    break;
                case \T_END_HEREDOC:
            ...
            $fix = $phpcsFile->addFixableWarning('Found precision alignment of %s spaces.', $i, 'Found', [$spaces]);
            if ($fix === \true) {
                ...
                switch ($tokens[$i]['code']) {
                    ...
                    case \T_DOC_COMMENT_WHITESPACE:
                        $replaceLength = ((int) ($tokens[$i]['length'] / $indent) + $tabstops) * $indent;
                        $replace = $this->getReplacement($replaceLength, $origContent);
                        if (isset($tokens[$i + 1]) === \true && ($tokens[$i + 1]['code'] === \T_DOC_COMMENT_STAR || $tokens[$i + 1]['code'] === \T_DOC_COMMENT_CLOSE_TAG) && $tabstops === 0) {
                    ...
                    case \T_PHPCS_IGNORE_FILE:
                        ...
                        $replace = $this->getReplacement($replaceLength, $origContent);
                        if (isset($content[0]) === \true && $content[0] === '*' && $tabstops === 0) {
                            // Maintain the extra space before the star.
                            $replace .= ' ';
                        }
                        if ($content === '') {
