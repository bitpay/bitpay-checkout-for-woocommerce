...
final class DisallowLonelyIfSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
         ...
         * Deal with `else if`.
         */
        if (ControlStructures::isElseIf($phpcsFile, $stackPtr) === \true) {
        ...
        $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $outerScopeOpener + 1, $outerScopeCloser, \true);
        ...
        $innerScopes = [$innerIfToken['scope_opener'] => $innerScopeCloser];
        do {
            /*
            ...
            if ($tokens[$innerScopeCloser]['code'] !== \T_CLOSE_CURLY_BRACKET) {
                if ($tokens[$innerScopeCloser]['code'] === \T_ENDIF) {
                    $nextAfter = $phpcsFile->findNext(Tokens::$emptyTokens, $innerScopeCloser + 1, $outerScopeCloser, \true);
            ...
            $innerNextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $innerScopeCloser + 1, $outerScopeCloser, \true);
            if ($innerNextNonEmpty === \false) {
                // This was the last closer.
                break;
            }
            if ($tokens[$innerNextNonEmpty]['code'] !== \T_ELSE && $tokens[$innerNextNonEmpty]['code'] !== \T_ELSEIF) {
            ...
            if (isset($tokens[$innerNextNonEmpty]['scope_closer']) === \false) {
                ...
                $nextAfter = $phpcsFile->findNext(Tokens::$emptyTokens, $innerNextNonEmpty + 1, $outerScopeCloser, \true);
                if ($nextAfter === \false || $tokens[$nextAfter]['code'] !== \T_IF || isset($tokens[$nextAfter]['scope_closer']) === \false) {
         ...
         */
        if ($phpcsFile->findNext(\T_WHITESPACE, $innerScopeCloser + 1, $outerScopeCloser, \true) !== \false) {
            // Comment between the inner and outer closers.
            $autoFixable = \false;
        }
        if ($tokens[$innerScopeCloser]['code'] === \T_SEMICOLON) {
            $hasComment = $phpcsFile->findPrevious(\T_WHITESPACE, $innerScopeCloser - 1, null, \true);
        ...
        if ($tokens[$outerScopeOpener]['line'] !== $innerIfToken['line']) {
            ...
            if ($phpcsFile->findNext(\T_WHITESPACE, $startOfNextLine, $innerIfPtr, \true) !== \false) {
        ...
        if ($tokens[$outerScopeCloser]['code'] === \T_CLOSE_CURLY_BRACKET && $tokens[$innerScopeCloser]['code'] === \T_CLOSE_CURLY_BRACKET || $tokens[$outerScopeCloser]['code'] === \T_ENDIF && $tokens[$innerScopeCloser]['code'] === \T_SEMICOLON) {
            $outerInnerSameType = \true;
        }
        ...
        while ($tokens[$i]['line'] === $innerIfToken['line'] && $tokens[$i]['code'] === \T_WHITESPACE) {
            $phpcsFile->fixer->replaceToken($i, '');
            ++$i;
        }
        // Remove any potential indentation whitespace for the inner if.
        if ($tokens[$outerScopeOpener]['line'] !== $innerIfToken['line'] && $tokens[$i]['line'] !== $innerIfToken['line']) {
            $i = $nextNonEmpty - 1;
            while ($tokens[$i]['line'] === $innerIfToken['line'] && $tokens[$i]['code'] === \T_WHITESPACE) {
                $phpcsFile->fixer->replaceToken($i, '');
                --$i;
        ...
        $i = $stackPtr + 1;
        while ($tokens[$i]['line'] === $tokens[$stackPtr]['line'] && $tokens[$i]['code'] === \T_WHITESPACE) {
            $phpcsFile->fixer->replaceToken($i, '');
            ++$i;
        }
        if ($outerInnerSameType === \false && $innerScopeCount > 1) {
            $loop = 1;
