...
final class NoEchoSprintfSniff implements Sniff
{
     ...
     */
    private $targetFunctions = ['sprintf' => 'printf', 'vsprintf' => 'vprintf'];
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $skip[] = \T_NS_SEPARATOR;
        $next = $phpcsFile->findNext($skip, $stackPtr + 1, null, \true);
        if ($next === \false || $tokens[$next]['code'] !== \T_STRING || isset($this->targetFunctions[\strtolower($tokens[$next]['content'])]) === \false) {
            // Not our target.
            return;
        }
        $detectedFunction = \strtolower($tokens[$next]['content']);
        $openParens = $phpcsFile->findNext(Tokens::$emptyTokens, $next + 1, null, \true);
        if ($next === \false || $tokens[$openParens]['code'] !== \T_OPEN_PARENTHESIS || isset($tokens[$openParens]['parenthesis_closer']) === \false) {
            // Live coding/parse error.
            return;
        }
        $closeParens = $tokens[$openParens]['parenthesis_closer'];
        $afterFunctionCall = $phpcsFile->findNext(Tokens::$emptyTokens, $closeParens + 1, null, \true);
        if ($afterFunctionCall === \false || $tokens[$afterFunctionCall]['code'] !== \T_SEMICOLON && $tokens[$afterFunctionCall]['code'] !== \T_CLOSE_TAG) {
            // Live coding/parse error or compound echo statement.
            return;
        }
        $fix = $phpcsFile->addFixableError('Unnecessary "echo %s(...)" found. Use "%s(...)" instead.', $next, 'Found', [$tokens[$next]['content'], $this->targetFunctions[$detectedFunction]]);
        if ($fix === \true) {
