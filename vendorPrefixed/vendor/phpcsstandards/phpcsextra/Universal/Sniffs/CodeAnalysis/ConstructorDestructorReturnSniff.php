...
final class ConstructorDestructorReturnSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($properties['return_type'] !== '' && $properties['return_type_token'] !== \false) {
            $data = [$functionType, $properties['return_type']];
            $fix = $phpcsFile->addFixableError('%s can not declare a return type. Found: %s', $properties['return_type_token'], 'ReturnTypeFound', $data);
            if ($fix === \true) {
        ...
        do {
            ...
            if (isset(Collections::functionDeclarationTokens()[$tokens[$current]['code']]) && isset($tokens[$current]['scope_closer'])) {
                // Skip over nested function/closure declarations.
                $current = $tokens[$current]['scope_closer'];
                continue;
            }
            $next = $phpcsFile->findNext(Tokens::$emptyTokens, $current + 1, $end, \true);
            ...
            $data = [$functionType, GetTokensAsString::compact($phpcsFile, $current, $endOfStatement, \true)];
            $phpcsFile->addWarning('%s can not return a value. Found: "%s"', $current, 'ReturnValueFound', $data);
        } while ($current < $end);
    }
}
