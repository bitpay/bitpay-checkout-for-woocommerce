...
final class DisallowCurlyBraceSyntaxSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['scope_condition']) === \false || $tokens[$stackPtr]['scope_condition'] !== $stackPtr) {
            $phpcsFile->recordMetric($stackPtr, self::METRIC_NAME, 'no');
            return;
        }
        $phpcsFile->recordMetric($stackPtr, self::METRIC_NAME, 'yes');
        $phpcsFile->addError('Namespace declarations using the curly brace syntax are not allowed.', $stackPtr, 'Forbidden');
    }
}
