...
final class EnforceCurlyBraceSyntaxSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['scope_condition']) === \true && $tokens[$stackPtr]['scope_condition'] === $stackPtr) {
            $phpcsFile->recordMetric($stackPtr, self::METRIC_NAME, 'yes');
            return;
        }
        $phpcsFile->recordMetric($stackPtr, self::METRIC_NAME, 'no');
        $phpcsFile->addError('Namespace declarations without curly braces are not allowed.', $stackPtr, 'Forbidden');
    }
}
