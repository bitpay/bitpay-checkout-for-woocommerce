...
final class RequireFinalMethodsInTraitsSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $data = [$methodProps['scope'], $magic, $methodName, ObjectDeclarations::getName($phpcsFile, $scopePtr)];
        $fix = $phpcsFile->addFixableError('The non-abstract, %s %smethod "%s()" in trait %s should be declared as final.', $stackPtr, $code, $data);
        if ($fix === \true) {
