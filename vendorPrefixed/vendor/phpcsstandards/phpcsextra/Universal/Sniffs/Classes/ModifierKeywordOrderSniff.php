...
final class ModifierKeywordOrderSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($classProp['readonly_token'] === \false || $classProp['final_token'] === \false && $classProp['abstract_token'] === \false) {
            /*
    ...
    private function handleError(File $phpcsFile, $firstKeyword, $secondKeyword)
    {
        ...
        $message = 'Class modifier keywords are not in the correct order. Expected: "%s", found: "%s"';
        $data = [$tokens[$secondKeyword]['content'] . ' ' . $tokens[$firstKeyword]['content'], $tokens[$firstKeyword]['content'] . ' ' . $tokens[$secondKeyword]['content']];
