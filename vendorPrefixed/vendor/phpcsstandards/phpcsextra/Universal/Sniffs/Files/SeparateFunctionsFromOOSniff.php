...
final class SeparateFunctionsFromOOSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = 0; $i < $phpcsFile->numTokens; $i++) {
            ...
            if ($tokens[$i]['code'] !== \T_OPEN_CURLY_BRACKET && isset($tokens[$i]['bracket_opener'], $tokens[$i]['bracket_closer']) && $i === $tokens[$i]['bracket_opener']) {
            ...
            if ($tokens[$i]['code'] === \T_DOC_COMMENT_OPEN_TAG && isset($tokens[$i]['comment_closer'])) {
            ...
            if (($tokens[$i]['code'] === \T_START_HEREDOC || $tokens[$i]['code'] === \T_START_NOWDOC || $tokens[$i]['code'] === \T_ANON_CLASS || $tokens[$i]['code'] === \T_CLOSURE || $tokens[$i]['code'] === \T_FN) && isset($tokens[$i]['scope_condition'], $tokens[$i]['scope_closer']) && $tokens[$i]['scope_condition'] === $i) {
        ...
        if ($functionCount > 0 && $OOCount > 0) {
            ...
            $phpcsFile->addError('A file should either contain function declarations or OO structure declarations, but not both.' . ' Found %d function declaration(s) and %d OO structure declaration(s).' . ' The first function declaration was found on line %d;' . ' the first OO declaration was found on line %d', $reportToken, 'Mixed', [$functionCount, $OOCount, $tokens[$firstFunction]['line'], $tokens[$firstOO]['line']]);
