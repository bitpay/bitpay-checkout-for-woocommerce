...
final class MixedKeyedUnkeyedArraySniff extends AbstractArrayDeclarationSniff
{
    ...
    public function processKey(File $phpcsFile, $startPtr, $endPtr, $itemNr)
    {
        ...
        if (empty($this->itemsWithoutKey) === \false) {
            foreach ($this->itemsWithoutKey as $itemNr => $stackPtr) {
                $phpcsFile->addError('Inconsistent array detected. A mix of keyed and unkeyed array items is not allowed.' . ' The array item in position %d does not have an array key.', $stackPtr, 'Found', [$itemNr]);
            }
    ...
    public function processNoKey(File $phpcsFile, $startPtr, $itemNr)
    {
        ...
        if ($this->hasKeys === \true) {
            $phpcsFile->addError('Inconsistent array detected. A mix of keyed and unkeyed array items is not allowed.' . ' The array item in position %d does not have an array key.', $firstNonEmpty, 'Found', [$itemNr]);
        } else {
