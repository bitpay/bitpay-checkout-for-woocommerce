...
final class DuplicateArrayKeySniff extends AbstractArrayDeclarationSniff
{
    ...
    public function processKey(File $phpcsFile, $startPtr, $endPtr, $itemNr)
    {
        ...
        $errorMsg = 'Duplicate array key found. The value will be overwritten%s.' . ' The %s array key "%s" was first seen on line %d';
        $errorCode = 'Found';
        $errors = [];
        $baseData = [$integerKey === \true ? 'integer' : 'string', $key];
        /*
         * Check if we've seen the key before.
         */
        if (($this->phpVersion === -1 || $this->phpVersion < 80000) && isset($this->keysSeenLt8[$key]) === \true) {
            $errors['phplt8'] = ['data_subset' => $baseData, 'error_suffix' => '', 'code_suffix' => ''];
            if ($integerKey === \true) {
                $errors['phplt8']['error_suffix'] = ' when using PHP < 8.0';
                $errors['phplt8']['code_suffix'] = 'ForPHPlt80';
            }
            $firstSeen = $this->keysSeenLt8[$key];
            $firstNonEmptyFirstSeen = $phpcsFile->findNext(Tokens::$emptyTokens, $firstSeen['ptr'], null, \true);
            $errors['phplt8']['data_subset'][] = $this->tokens[$firstNonEmptyFirstSeen]['line'];
        }
        if (($this->phpVersion === -1 || $this->phpVersion >= 80000) && isset($this->keysSeenGt8[$key]) === \true) {
            $errors['phpgt8'] = ['data_subset' => $baseData, 'error_suffix' => '', 'code_suffix' => ''];
            if ($integerKey === \true) {
        ...
        if ($errors !== []) {
            ...
            if (isset($errors['phplt8'], $errors['phpgt8']) && $errors['phplt8']['data_subset'] === $errors['phpgt8']['data_subset']) {
         ...
         */
        $this->keysSeenLt8[$key] = ['item' => $itemNr, 'ptr' => $startPtr];
        $this->keysSeenGt8[$key] = ['item' => $itemNr, 'ptr' => $startPtr];
        if ($integerKey === \true) {
            if (isset($this->currentMaxIntKeyLt8) === \false && $key > -1 || isset($this->currentMaxIntKeyLt8) === \true && $key > $this->currentMaxIntKeyLt8) {
                $this->currentMaxIntKeyLt8 = $key;
            }
            if (isset($this->currentMaxIntKeyGt8) === \false || $key > $this->currentMaxIntKeyGt8) {
    ...
    public function processNoKey(File $phpcsFile, $startPtr, $itemNr)
    {
        ...
        $this->keysSeenLt8[$this->currentMaxIntKeyLt8] = ['item' => $itemNr, 'ptr' => $startPtr];
        // Track the key for PHP 8.0+.
        if (isset($this->currentMaxIntKeyGt8) === \false) {
            $this->currentMaxIntKeyGt8 = -1;
        }
        ++$this->currentMaxIntKeyGt8;
        $this->keysSeenGt8[$this->currentMaxIntKeyGt8] = ['item' => $itemNr, 'ptr' => $startPtr];
    }
}
