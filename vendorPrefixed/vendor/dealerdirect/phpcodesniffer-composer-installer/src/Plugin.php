 ...
 * @license MIT
 */
namespace BitPayVendor\PHPCSStandards\Composer\Plugin\Installers\PHPCodeSniffer;

use BitPayVendor\Composer\Composer;
use BitPayVendor\Composer\EventDispatcher\EventSubscriberInterface;
use BitPayVendor\Composer\IO\IOInterface;
use BitPayVendor\Composer\Package\AliasPackage;
use BitPayVendor\Composer\Package\PackageInterface;
use BitPayVendor\Composer\Package\RootPackageInterface;
use BitPayVendor\Composer\Plugin\PluginInterface;
use BitPayVendor\Composer\Script\Event;
use BitPayVendor\Composer\Script\ScriptEvents;
use BitPayVendor\Composer\Util\Filesystem;
use BitPayVendor\Composer\Util\ProcessExecutor;
use BitPayVendor\Symfony\Component\Finder\Finder;
use BitPayVendor\Symfony\Component\Process\Exception\LogicException;
use BitPayVendor\Symfony\Component\Process\Exception\ProcessFailedException;
use BitPayVendor\Symfony\Component\Process\Exception\RuntimeException;
use BitPayVendor\Symfony\Component\Process\PhpExecutableFinder;
/**
...
class Plugin implements PluginInterface, EventSubscriberInterface
{
    ...
    {
        return array(ScriptEvents::POST_INSTALL_CMD => array(array('onDependenciesChangedEvent', 0)), ScriptEvents::POST_UPDATE_CMD => array(array('onDependenciesChangedEvent', 0)));
    }
    /**
    ...
    {
        if ($this->isPHPCodeSnifferInstalled() === \true) {
            $this->processExecutor->execute($this->getPhpcsCommand() . ' --config-show', $output, $this->getPHPCodeSnifferInstallPath());
    ...
    private function saveInstalledPaths()
    {
        ...
        // Okay, lets rock!
        $command = \vsprintf('%s %s', array('phpcs command' => $this->getPhpcsCommand(), 'arguments' => \implode(' ', $arguments)));
    ...
    protected function getPhpcsCommand()
    {
        ...
        return \vsprintf('%s %s', array('php executable' => $this->getPhpExecCommand(), 'phpcs executable' => $phpcsExecutable));
    }
    /**
    ...
    private function updateInstalledPaths()
    {
        ...
        if ($this->composer->getPackage() instanceof RootPackageInterface && $this->composer->getPackage()->getType() === self::PACKAGE_TYPE) {
    ...
    {
        $codingStandardPackages = \array_filter($this->composer->getRepositoryManager()->getLocalRepository()->getPackages(), function (PackageInterface $package) {
