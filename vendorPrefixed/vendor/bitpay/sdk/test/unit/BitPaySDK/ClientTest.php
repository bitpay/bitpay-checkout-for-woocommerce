...
namespace BitPayVendor\BitPaySDK\Test;

use BitPayVendor\BitPaySDK\Client;
use BitPayVendor\BitPaySDK\Client\BillClient;
use BitPayVendor\BitPaySDK\Client\InvoiceClient;
use BitPayVendor\BitPaySDK\Client\LedgerClient;
use BitPayVendor\BitPaySDK\Client\PayoutClient;
use BitPayVendor\BitPaySDK\Client\PayoutRecipientsClient;
...
use BitPayVendor\BitPaySDK\Exceptions\BillDeliveryException;
use BitPayVendor\BitPaySDK\Exceptions\BillQueryException;
use BitPayVendor\BitPaySDK\Exceptions\BillUpdateException;
...
use BitPayVendor\BitPaySDK\Exceptions\LedgerQueryException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutCancellationException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutCreationException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutNotificationException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutQueryException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutRecipientCancellationException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutRecipientCreationException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutRecipientNotificationException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutRecipientQueryException;
use BitPayVendor\BitPaySDK\Exceptions\PayoutRecipientUpdateException;
use BitPayVendor\BitPaySDK\Exceptions\RateQueryException;
use BitPayVendor\BitPaySDK\Exceptions\RefundCancellationException;
use BitPayVendor\BitPaySDK\Exceptions\RefundCreationException;
use BitPayVendor\BitPaySDK\Exceptions\RefundNotificationException;
use BitPayVendor\BitPaySDK\Exceptions\RefundQueryException;
use BitPayVendor\BitPaySDK\Exceptions\RefundUpdateException;
...
use BitPayVendor\BitPaySDK\Model\Ledger\Ledger;
use BitPayVendor\BitPaySDK\Model\Ledger\LedgerEntry;
...
use BitPayVendor\BitPaySDK\Model\Rate\Rates;
...
class ClientTest extends TestCase
{
    ...
    public function testWithData()
    {
        ...
        $result = $this->getTestedClassInstance()::createWithData(Env::TEST, __DIR__ . '/bitpay_private_test.key', $tokens, 'YourMasterPassword');
    ...
    public function testWithDataException()
    {
        ...
        $instance::createWithData(Env::TEST, __DIR__ . '/bitpay_private_test.key', $tokens, 'badPassword');
    }
    /**
     * @throws BitPayException
     */
    ...
    public function testGetTokens() : void
    {
        ...
        $restCliMock->expects(self::once())->method('get')->with("tokens")->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/getTokensResponse.json'));
    ...
    public function testCreateBill()
    {
        ...
        $billToArray = ['id' => $expectedId, 'status' => $expectedStatus, 'token' => $expectedToken];
        $billMock->method('toArray')->willReturn($billToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("bills", $billMock->toArray(), \true)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/createBillResponse.json'));
    ...
    public function testCreateBillException()
    {
        $billMock = $this->createMock(Bill::class);
        $restCliMock = $this->getRestCliMock();
        $billToArray = ['id' => 'testId', 'status' => 'status', 'token' => 'token'];
        $billMock->method('toArray')->willReturn($billToArray);
        $restCliMock->expects(self::once())->method('post')->with("bills", $billMock->toArray(), \true)->willThrowException(new BitPayException());
        $billClient = $this->getClient($restCliMock);
    ...
    public function testCreateBillShouldCatchRestCliException()
    {
        ...
        $billMock->method('toArray')->willReturn($billToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("bills", $billMock->toArray(), \true)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(BillCreationException::class);
        $client->createBill($billMock, Facade::MERCHANT, \true);
    }
    public function testCreateBillShouldCatchJsonMapperException()
    {
        ...
        $billToArray = ['id' => $exampleBillId, 'status' => 'status', 'token' => 'token'];
        $billMock->method('toArray')->willReturn($billToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("bills", $billMock->toArray(), \true)->willReturn($exampleBadResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(BillCreationException::class);
        $client->createBill($billMock, Facade::MERCHANT, \true);
    }
    public function testGetBill()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("bills/" . $exampleBillId, $params, \true)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/getBill.json'));
        $client = $this->getClient($restCliMock);
        $result = $client->getBill($exampleBillId);
        self::assertEquals('6EBQR37MgDJPfEiLY3jtRq7eTP2aodR5V5wmXyyZhru5FM5yF4RCGKYQtnT7nhwHjA', $result->getToken());
    ...
    public function testGetBillShouldCatchRestCliBitPayException()
    {
        ...
        $exampleBillId = 'X6KJbe9RxAGWNReCwd1xRw';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("bills/" . $exampleBillId, $params, \true)->willThrowException(new BillQueryException());
        $client = $this->getClient($restCliMock);
    ...
    public function testGetBillShouldCatchRestCliException()
    {
        ...
        $exampleBillId = 'X6KJbe9RxAGWNReCwd1xRw';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("bills/" . $exampleBillId, $params, \true)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
    ...
    public function testGetBillShouldCatchJsonMapperException()
    {
        ...
        $exampleBadResponse = \file_get_contents(__DIR__ . '/jsonResponse/badResponse.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("bills/" . $exampleBillId, $params, \true)->willReturn($exampleBadResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(BillQueryException::class);
        $client->getBill($exampleBillId);
    }
    public function testGetBills()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("bills", $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
        $result = $client->getBills($status);
        self::assertIsArray($result);
        self::assertEquals('6EBQR37MgDJPfEiLY3jtRqBMYLg8XSDqhp2kp7VSDqCMHGHnsw4bqnnwQmtehzCvSo', $result[0]->getToken());
        self::assertEquals('6EBQR37MgDJPfEiLY3jtRq7eTP2aodR5V5wmXyyZhru5FM5yF4RCGKYQtnT7nhwHjA', $result[1]->getToken());
    ...
    public function testGetBillsShouldCatchRestCliBitPayException()
    {
        ...
        $params['status'] = $status;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("bills", $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(BillQueryException::class);
        $client->getBills($status);
    }
    ...
    public function testGetBillsShouldCatchRestCliException()
    {
        ...
        $params['status'] = $status;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("bills", $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(BillQueryException::class);
        $client->getBills($status);
    }
    public function testUpdateBill()
    {
        ...
        $billMock->method('toArray')->willReturn($billToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("bills/" . $exampleBillId, $billMock->toArray())->willReturn($exampleResponse);
        $restCliMock->expects(self::once())->method('get')->with("bills/" . $exampleBillId, $params, \true)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testUpdateBillShouldCatchRestCliBitPayException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $exampleBillId = 'X6KJbe9RxAGWNReCwd1xRw';
        $exampleResponse = \file_get_contents(__DIR__ . '/jsonResponse/getBill.json', \true);
        $billMock->method('getId')->willReturn($exampleBillId);
        $billToArray = ['id' => $exampleBillId, 'status' => 'status', 'token' => 'token'];
        $billMock->method('toArray')->willReturn($billToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("bills/" . $exampleBillId, $billMock->toArray())->willThrowException(new BitPayException());
        $restCliMock->expects(self::once())->method('get')->with("bills/" . $exampleBillId, $params, \true)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testUpdateBillShouldCatchRestCliException()
    {
        ...
        $billMock->method('toArray')->willReturn($billToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("bills/" . $exampleBillId, $billMock->toArray())->willThrowException(new Exception());
        $restCliMock->expects(self::once())->method('get')->with("bills/" . $exampleBillId, $params, \true)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(BillUpdateException::class);
        $client->updateBill($billMock, $exampleBillId);
    ...
    public function testUpdateBillShouldCatchJsonMapperException()
    {
        ...
        $billMock->method('toArray')->willReturn($billToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("bills/" . $exampleBillId, $billMock->toArray())->willReturn($badResponse);
        $restCliMock->expects(self::once())->method('get')->with("bills/" . $exampleBillId, $params, \true)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(BillUpdateException::class);
        $client->updateBill($billMock, $exampleBillId);
    ...
    public function testDeliverBill()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("bills/" . $exampleBillId . '/deliveries', ['token' => $exampleBillToken])->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testDeliverBillShouldCatchRestCliBitPayException()
    {
        ...
        $exampleBillToken = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("bills/" . $exampleBillId . '/deliveries', ['token' => $exampleBillToken])->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testDeliverBillShouldCatchRestCliException()
    {
        ...
        $exampleBillToken = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("bills/" . $exampleBillId . '/deliveries', ['token' => $exampleBillToken])->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(BillDeliveryException::class);
        $client->deliverBill($exampleBillId, $exampleBillToken);
    }
    public function testGetLedger()
    {
        ...
        $restCliMock->expects(self::once())->method('get')->with("ledgers/" . $exampleCurrency, $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetLedgers()
    {
        ...
        $restCliMock->expects(self::once())->method('get')->with("ledgers", $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testSubmitPayoutRecipients()
    {
        ...
        $payoutRecipientToArray = ['token' => self::MERCHANT_TOKEN, 'id' => $exampleRecipientId];
        $payoutRecipientsMock->method('toArray')->willReturn($payoutRecipientToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("recipients", $payoutRecipientsMock->toArray())->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testSubmitPayoutRecipientsShouldCatchRestCliBitPayException()
    {
        ...
        $payoutRecipientsMock->method('toArray')->willReturn($payoutRecipientToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("recipients", $payoutRecipientsMock->toArray())->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testSubmitPayoutRecipientsShouldCatchRestCliException()
    {
        ...
        $payoutRecipientsMock->method('toArray')->willReturn($payoutRecipientToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("recipients", $payoutRecipientsMock->toArray())->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(Exception::class);
        $client->submitPayoutRecipients($payoutRecipientsMock);
    }
    ...
    public function testSubmitPayoutRecipientsShouldCatchJsonMapperException()
    {
        ...
        $payoutRecipientsMock->method('toArray')->willReturn($payoutRecipientToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("recipients", $payoutRecipientsMock->toArray())->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientCreationException::class);
        $client->submitPayoutRecipients($payoutRecipientsMock);
    }
    public function testGetPayoutRecipient()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("recipients/" . $recipientId, $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetPayoutRecipientShouldHandleRestCliBitPayException()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $recipientId = 'test';
        $restCliMock->expects(self::once())->method('get')->with("recipients/" . $recipientId, $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testGetPayoutRecipientShouldHandleRestCliException()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $recipientId = 'test';
        $restCliMock->expects(self::once())->method('get')->with("recipients/" . $recipientId, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientQueryException::class);
        $client->getPayoutRecipient($recipientId);
    }
    public function testGetPayoutRecipientShouldHandleJsonMapperException()
    {
        ...
        $badResponse = \file_get_contents(__DIR__ . '/jsonResponse/badResponse.json');
        $recipientId = 'test';
        $restCliMock->expects(self::once())->method('get')->with("recipients/" . $recipientId, $params)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetPayoutRecipients()
    {
        ...
        $restCliMock->expects(self::once())->method('get')->with("recipients", $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetPayoutRecipientsShouldHandleRestCliBitPayException()
    {
        ...
        $params['limit'] = $limit;
        $params['offset'] = $offset;
        $restCliMock->expects(self::once())->method('get')->with("recipients", $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientQueryException::class);
    ...
    public function testGetPayoutRecipientsShouldHandleRestCliException()
    {
        ...
        $params['limit'] = $limit;
        $params['offset'] = $offset;
        $restCliMock->expects(self::once())->method('get')->with("recipients", $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientQueryException::class);
        $client->getPayoutRecipients($status, $limit, $offset);
    }
    public function testGetPayoutRecipientsShouldHandleJsonMapperException()
    {
        ...
        $params['limit'] = $limit;
        $params['offset'] = $offset;
        $restCliMock->expects(self::once())->method('get')->with("recipients", $params)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientQueryException::class);
        $client->getPayoutRecipients($status, $limit, $offset);
    }
    ...
    public function testUpdatePayoutRecipient()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("recipients/" . $exampleRecipientId, $payoutRecipientMock->toArray())->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testUpdatePayoutRecipientShouldCatchRestCliBitPayException()
    {
        ...
        $payoutRecipientMock->method('toArray')->willReturn($payoutRecipientToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("recipients/" . $exampleRecipientId, $payoutRecipientMock->toArray())->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testUpdatePayoutRecipientShouldCatchRestCliException()
    {
        ...
        $payoutRecipientMock->method('toArray')->willReturn($payoutRecipientToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("recipients/" . $exampleRecipientId, $payoutRecipientMock->toArray())->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientUpdateException::class);
        $client->updatePayoutRecipient($exampleRecipientId, $payoutRecipientMock);
    }
    public function testUpdatePayoutRecipientShouldCatchJsonMapperException()
    {
        ...
        $payoutRecipientMock->method('toArray')->willReturn($payoutRecipientToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("recipients/" . $exampleRecipientId, $payoutRecipientMock->toArray())->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientUpdateException::class);
        $client->updatePayoutRecipient($exampleRecipientId, $payoutRecipientMock);
    }
    public function testDeletePayoutRecipient()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("recipients/" . $exampleRecipientId, $params)->willReturn($successResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testDeletePayoutRecipientShouldCatchRestCliBitPayException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("recipients/" . $exampleRecipientId, $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testDeletePayoutRecipientShouldCatchRestCliException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("recipients/" . $exampleRecipientId, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientCancellationException::class);
        $client->deletePayoutRecipient($exampleRecipientId);
    }
    public function testDeletePayoutRecipientShouldCatchJsonDecodeException()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("recipients/" . $exampleRecipientId, $params)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testRequestPayoutRecipientNotification()
    {
        ...
        $successResponse = \file_get_contents(__DIR__ . '/jsonResponse/success.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("recipients/" . $exampleRecipientId . '/notifications', $content)->willReturn($successResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testRequestPayoutRecipientNotificationShouldCatchRestCliBitPayException()
    {
        ...
        $exampleRecipientId = 'X3icwc4tE8KJ5hEPNPpDXW';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("recipients/" . $exampleRecipientId . '/notifications', $content)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testRequestPayoutRecipientNotificationShouldCatchRestCliException()
    {
        ...
        $exampleRecipientId = 'X3icwc4tE8KJ5hEPNPpDXW';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("recipients/" . $exampleRecipientId . '/notifications', $content)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutRecipientNotificationException::class);
        $client->requestPayoutRecipientNotification($exampleRecipientId);
    }
    public function testRequestPayoutRecipientNotificationShouldCatchJsonDecodeException()
    {
        ...
        $badResponse = \file_get_contents(__DIR__ . '/jsonResponse/false.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("recipients/" . $exampleRecipientId . '/notifications', $content)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetRates()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with('rates', null, \false)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    {
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with('rates', null, \false)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    {
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with('rates', null, \false)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RateQueryException::class);
        $client->getRates();
    }
    public function testGetRatesShouldHandleCorruptJson()
    {
        $badResponse = \file_get_contents(__DIR__ . '/jsonResponse/badResponse.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with('rates', null, \false)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(RateQueryException::class);
        $client->getRates();
    }
    public function testGetCurrencyRates()
    {
        ...
        $restCliMock->expects(self::once())->method('get')->with('rates/' . $exampleCurrency, null, \false)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetCurrencyRatesShouldHandleRestCliBitPayException()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with('rates/' . $exampleCurrency, null, \false)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(RateQueryException::class);
    ...
    public function testGetCurrencyRatesShouldHandleRestCliException()
    {
        $exampleCurrency = Currency::USD;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with('rates/' . $exampleCurrency, null, \false)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RateQueryException::class);
        $client->getCurrencyRates(Currency::USD);
    }
    public function testGetCurrencyRatesShouldFailWhenDataInvalid()
    {
        ...
        $badResponse = \file_get_contents(__DIR__ . '/jsonResponse/badResponse.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with('rates/' . $exampleCurrency, null, \false)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(RateQueryException::class);
        $client->getCurrencyRates(Currency::USD);
    }
    public function testGetCurrencyPairRate()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with('rates/' . $baseCurrency . '/' . $currency, null, \false)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetSettlements()
    {
        ...
        $restCliMock->expects(self::once())->method('get')->with("settlements", $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetSettlementsShouldCatchRestCliBitPayException()
    {
        ...
        $params['limit'] = $limit;
        $params['offset'] = $offset;
        $restCliMock->expects(self::once())->method('get')->with("settlements", $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testGetSettlementsShouldCatchRestCliException()
    {
        ...
        $params['limit'] = $limit;
        $params['offset'] = $offset;
        $restCliMock->expects(self::once())->method('get')->with("settlements", $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(SettlementQueryException::class);
        $client->getSettlements($currency, $dateStart, $dateEnd, $status, $limit, $offset);
    }
    public function testGetSettlementsShouldCatchJsonMapperException()
    {
        ...
        $params['offset'] = $offset;
        $badResponse = \file_get_contents(__DIR__ . '/jsonResponse/badResponse.json');
        $restCliMock->expects(self::once())->method('get')->with("settlements", $params)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(SettlementQueryException::class);
        $client->getSettlements($currency, $dateStart, $dateEnd, $status, $limit, $offset);
    }
    public function testGetSettlement()
    {
        ...
        $restCliMock->expects(self::once())->method('get')->with("settlements/" . $settlementId, $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
        ...
        self::assertEquals('2GrR6GDeYxUFYM9sDKViy6nFFTy4Rjvm1SYdLBjK46jkeJdgUTRccRfhtwkhNcuZky', $result->getToken());
    ...
    public function testGetSettlementShouldHandleRestCliException()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $settlementId = 'RPWTabW8urd3xWv2To989v';
        $restCliMock->expects(self::once())->method('get')->with("settlements/" . $settlementId, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(SettlementQueryException::class);
        $client->getSettlement($settlementId);
    }
    public function testGetSettlementShouldCatchJsonMapperException()
    {
        ...
        $settlementId = 'RPWTabW8urd3xWv2To989v';
        $badResponse = \file_get_contents(__DIR__ . '/jsonResponse/badResponse.json');
        $restCliMock->expects(self::once())->method('get')->with("settlements/" . $settlementId, $params)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(SettlementQueryException::class);
        $client->getSettlement($settlementId);
    }
    public function testGetSettlementReconciliationReport()
    {
        ...
        $exampleResponse = \file_get_contents(__DIR__ . '/jsonResponse/getSettlementReconciliationReportResponse.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("settlements/" . $exampleId . '/reconciliationReport', $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetSettlementReconciliationReportShouldCatchRestCliBitPayException()
    {
        ...
        $params['token'] = $exampleToken;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("settlements/" . $exampleId . '/reconciliationReport', $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(SettlementQueryException::class);
    ...
    public function testGetSettlementReconciliationReportShouldCatchRestCliException()
    {
        ...
        $params['token'] = $exampleToken;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("settlements/" . $exampleId . '/reconciliationReport', $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(SettlementQueryException::class);
        $client->getSettlementReconciliationReport($settlement);
    }
    public function testGetSettlementReconciliationReportShouldCatchJsonMapperException()
    {
        ...
        $badResponse = \file_get_contents(__DIR__ . '/jsonResponse/badResponse.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("settlements/" . $exampleId . '/reconciliationReport', $params)->willReturn($badResponse);
        $client = $this->getClient($restCliMock);
        $this->expectException(SettlementQueryException::class);
        $client->getSettlementReconciliationReport($settlement);
    }
    ...
    public function testGetPayout()
    {
        ...
        $exampleResponseArray = \json_decode($exampleResponse, \true);
        $payoutId = $exampleResponseArray['id'];
        $restCliMock->expects(self::once())->method('get')->with("payouts/" . $payoutId, $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetPayoutShouldHandleRestCliBitPayException()
    {
        ...
        $exampleResponseArray = \json_decode($exampleResponse, \true);
        $payoutId = $exampleResponseArray['id'];
        $restCliMock->expects(self::once())->method('get')->with("payouts/" . $payoutId, $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutQueryException::class);
    ...
    public function testGetPayoutShouldHandleJsonMapperException()
    {
        ...
        $exampleResponseArray = \json_decode($exampleResponse, \true);
        $payoutId = $exampleResponseArray['id'];
        $restCliMock->expects(self::once())->method('get')->with("payouts/" . $payoutId, $params)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutQueryException::class);
        $client->getPayout($payoutId);
    }
    ...
    public function testGetPayoutShouldHandleRestCliException()
    {
        ...
        $exampleResponseArray = \json_decode($exampleResponse, \true);
        $payoutId = $exampleResponseArray['id'];
        $restCliMock->expects(self::once())->method('get')->with("payouts/" . $payoutId, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutQueryException::class);
        $client->getPayout($payoutId);
    }
    public function testGetPayouts()
    {
        ...
        $restCliMock->expects(self::once())->method('get')->with("payouts", $params)->willReturn($exampleResponse);
        $client = $this->getClient($restCliMock);
        $result = $client->getPayouts($params['startDate'], $params['endDate'], $params['status'], $params['reference'], $params['limit'], $params['offset']);
        self::assertIsArray($result);
    ...
    public function testGetPayoutsShouldHandleRestCliBitPayException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("payouts", $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutQueryException::class);
        $client->getPayouts($params['startDate'], $params['endDate'], $params['status'], $params['reference'], $params['limit'], $params['offset']);
    }
    public function testGetPayoutsShouldHandleRestCliException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("payouts", $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutQueryException::class);
        $client->getPayouts($params['startDate'], $params['endDate'], $params['status'], $params['reference'], $params['limit'], $params['offset']);
    }
    public function testGetPayoutsShouldHandleJsonMapperException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("payouts", $params)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutQueryException::class);
        $client->getPayouts($params['startDate'], $params['endDate'], $params['status'], $params['reference'], $params['limit'], $params['offset']);
    }
    public function testCancelPayout()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("payouts/" . $examplePayoutId, $params)->willReturn('{"status":"success"}');
        $client = $this->getClient($restCliMock);
    ...
    public function testCancelPayoutShouldCatchRestCliBitPayException()
    {
        ...
        $examplePayoutId = 'test';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("payouts/" . $examplePayoutId, $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testCancelPayoutShouldCatchRestCliException()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("payouts/" . $examplePayoutId, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutCancellationException::class);
        $client->cancelPayout($examplePayoutId);
    }
    public function testCancelPayoutShouldCatchUnexistentPropertyError()
    {
        ...
        $examplePayoutId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("payouts/" . $examplePayoutId, $params)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/false.json', \true));
        $client = $this->getClient($restCliMock);
    ...
    public function testSubmitPayout()
    {
        ...
        $exampleCurrency = Currency::USD;
        $payoutBatchToArray = ['token' => self::MERCHANT_TOKEN, 'currency' => $exampleCurrency];
        $payoutMock->method('getCurrency')->willReturn($exampleCurrency);
        $payoutMock->method('toArray')->willReturn($payoutBatchToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts", $payoutMock->toArray())->willReturn('{ "currency": "EUR", "balance": 0 }');
        $client = $this->getClient($restCliMock);
    ...
    public function testSubmitPayoutShouldCatchRestCliBitPayException()
    {
        ...
        $payoutMock->method('toArray')->willReturn($payoutBatchToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts", $payoutMock->toArray())->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testSubmitPayoutShouldCatchRestCliException()
    {
        ...
        $payoutMock->method('toArray')->willReturn($payoutBatchToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts", $payoutMock->toArray())->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutCreationException::class);
        $client->submitPayout($payoutMock);
    }
    public function testSubmitPayoutShouldCatchJsonMapperException()
    {
        ...
        $payoutMock->method('toArray')->willReturn($payoutBatchToArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts", $payoutMock->toArray())->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutCreationException::class);
        $client->submitPayout($payoutMock);
    }
    public function testRequestNotification()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts/{$payoutId}/notifications", $content)->willReturn('{ "status": "success", "data": {}, "message": null }');
        $client = $this->getClient($restCliMock);
    ...
    public function testRequestNotificationShouldCatchRestCliBitPayException()
    {
        ...
        $payoutId = 'JMwv8wQCXANoU2ZZQ9a9GH';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts/{$payoutId}/notifications", $content)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testRequestNotificationShouldCatchRestCliException()
    {
        ...
        $payoutId = 'JMwv8wQCXANoU2ZZQ9a9GH';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts/{$payoutId}/notifications", $content)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(PayoutNotificationException::class);
        $client->requestPayoutNotification($payoutId);
    }
    public function testRequestNotificationShouldCatchJsonException()
    {
        ...
        $payoutId = 'JMwv8wQCXANoU2ZZQ9a9GH';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts/{$payoutId}/notifications", $content)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/false.json', \true));
        $client = $this->getClient($restCliMock);
        self::assertFalse($client->requestPayoutNotification($payoutId));
    }
    public function testCreatePayoutGroup() : void
    {
        $expectedRequest = \json_decode(\file_get_contents(__DIR__ . '/jsonResponse/createPayoutGroupRequest.json', \true), \true, 512, \JSON_THROW_ON_ERROR);
        $notificationURL = 'https://yournotiticationURL.com/wed3sa0wx1rz5bg0bv97851eqx';
        $shopperId = '7qohDf2zZnQK5Qanj8oyC2';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("payouts/group", $expectedRequest)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/createPayoutGroupResponse.json', \true));
        $client = $this->getClient($restCliMock);
        $payout = new Payout();
        $payout->setAmount(10);
    ...
    public function testCancelPayoutGroup() : void
    {
        $groupId = '12345';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("payouts/group/" . $groupId, ['token' => self::PAYOUT_TOKEN])->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/cancelPayoutGroupResponse.json', \true));
        $client = $this->getClient($restCliMock);
        $result = $client->cancelPayoutGroup($groupId);
        $firstPayout = $result->getPayouts()[0];
        $firstFailed = $result->getFailed()[0];
        self::assertCount(2, $result->getPayouts());
        self::assertEquals('https://yournotiticationURL.com/wed3sa0wx1rz5bg0bv97851eqx', $firstPayout->getNotificationURL());
    ...
    public function testCreateRefund()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("refunds/", $params, \true)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/createRefundResponse.json', \true));
        $client = $this->getClient($restCliMock);
        /** @var Refund $result */
        $result = $client->createRefund($params['invoiceId'], $params['amount'], $params['currency'], $params['preview'], $params['immediate'], $params['buyerPaysRefundFee'], $params['guid']);
    ...
    public function testCreateRefundShouldCatchRestCliBitPayException()
    {
        $params = $this->getInvoiceRefundParams();
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("refunds/", $params, \true)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundCreationException::class);
        $client->createRefund($params['invoiceId'], $params['amount'], $params['currency'], $params['preview'], $params['immediate'], $params['buyerPaysRefundFee'], $params['guid']);
    }
    public function testCreateRefundShouldCatchRestCliException()
    {
        $params = $this->getInvoiceRefundParams();
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("refunds/", $params, \true)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundCreationException::class);
        $client->createRefund($params['invoiceId'], $params['amount'], $params['currency'], $params['preview'], $params['immediate'], $params['buyerPaysRefundFee'], $params['guid']);
    }
    public function testCreateRefundShouldCatchJsonMapperException()
    {
        $params = $this->getInvoiceRefundParams();
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("refunds/", $params, \true)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundCreationException::class);
        $client->createRefund($params['invoiceId'], $params['amount'], $params['currency'], $params['preview'], $params['immediate'], $params['buyerPaysRefundFee'], $params['guid']);
    }
    public function testCancelRefund()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("refunds/" . $exampleRefundId, $params)->willReturn(self::CANCEL_REFUND_JSON_STRING);
        $client = $this->getClient($restCliMock);
    ...
    public function testCancelRefundShouldCatchRestCliBitPayException()
    {
        ...
        $exampleRefundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("refunds/" . $exampleRefundId, $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testCancelRefundShouldCatchRestCliException()
    {
        ...
        $exampleRefundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("refunds/" . $exampleRefundId, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundCancellationException::class);
        $client->cancelRefund($exampleRefundId);
    }
    public function testCancelRefundShouldCatchJsonMapperException()
    {
        ...
        $exampleRefundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("refunds/" . $exampleRefundId, $params)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundCancellationException::class);
        $client->cancelRefund($exampleRefundId);
    }
    public function testCancelRefundByGuid()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("refunds/guid/" . $guid, $params)->willReturn(self::CANCEL_REFUND_JSON_STRING);
        $client = $this->getClient($restCliMock);
    ...
    public function testCancelRefundByGuidShouldCatchRestCliBitPayException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("refunds/guid/" . $guid, $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundCancellationException::class);
    ...
    public function testCancelRefundByGuidShouldCatchRestCliException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("refunds/guid/" . $guid, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundCancellationException::class);
        $client->cancelRefundByGuid($guid);
    }
    public function testCancelRefundByGuidShouldCatchJsonMapperException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('delete')->with("refunds/guid/" . $guid, $params)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundCancellationException::class);
        $client->cancelRefundByGuid($guid);
    }
    public function testUpdateRefund()
    {
        ...
        $refundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("refunds/" . $refundId, $params)->willReturn(self::UPDATE_REFUND_JSON_STRING);
        $client = $this->getClient($restCliMock);
    ...
    public function testUpdateRefundShouldCatchRestCliBitPayException()
    {
        ...
        $refundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("refunds/" . $refundId, $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testUpdateRefundShouldCatchRestCliException()
    {
        ...
        $refundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("refunds/" . $refundId, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundUpdateException::class);
        $client->updateRefund($refundId, $params['status']);
    }
    public function testUpdateRefundShouldCatchJsonMapperException()
    {
        ...
        $refundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("refunds/" . $refundId, $params)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundUpdateException::class);
        $client->updateRefund($refundId, $params['status']);
    }
    public function testUpdateRefundByGuid()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("refunds/guid/" . $guid, $params)->willReturn(self::UPDATE_REFUND_JSON_STRING);
        $client = $this->getClient($restCliMock);
    ...
    public function testUpdateRefundBuGuidShouldCatchRestCliBitPayException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("refunds/guid/" . $guid, $params)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundUpdateException::class);
    ...
    public function testUpdateRefundBuGuidShouldCatchRestCliException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("refunds/guid/" . $guid, $params)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundUpdateException::class);
        $client->updateRefundByGuid($guid, $params['status']);
    }
    public function testUpdateRefundByGuidShouldCatchJsonMapperException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("refunds/guid/" . $guid, $params)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundUpdateException::class);
        $client->updateRefundByGuid($guid, $params['status']);
    }
    public function testGetRefunds()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/", $params, \true)->willReturn(self::CORRECT_JSON_STRING);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetRefundsShouldCatchRestCliBitPayException()
    {
        ...
        $params['invoiceId'] = $exampleInvoiceId;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/", $params, \true)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testGetRefundsShouldCatchRestCliException()
    {
        ...
        $params['invoiceId'] = $exampleInvoiceId;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/", $params, \true)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundQueryException::class);
        $client->getRefunds($exampleInvoiceId);
    }
    ...
    public function testGetRefundsShouldCatchJsonMapperException()
    {
        ...
        $params['invoiceId'] = $exampleInvoiceId;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/", $params, \true)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundQueryException::class);
        $client->getRefunds($exampleInvoiceId);
    }
    ...
    public function testGetRefund()
    {
        ...
        $exampleRefundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/" . $exampleRefundId, $params, \true)->willReturn(self::UPDATE_REFUND_JSON_STRING);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetRefundShouldCatchRestCliBitPayException()
    {
        ...
        $exampleRefundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/" . $exampleRefundId, $params, \true)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundQueryException::class);
    ...
    public function testGetRefundShouldCatchRestCliException()
    {
        ...
        $exampleRefundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/" . $exampleRefundId, $params, \true)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundQueryException::class);
        $client->getRefund($exampleRefundId);
    }
    ...
    public function testGetRefundShouldCatchRestCliJsonMapperException()
    {
        ...
        $exampleRefundId = 'testId';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/" . $exampleRefundId, $params, \true)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundQueryException::class);
        $client->getRefund($exampleRefundId);
    }
    public function testGetRefundByGuid()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/guid/" . $guid, $params, \true)->willReturn(self::UPDATE_REFUND_JSON_STRING);
        $client = $this->getClient($restCliMock);
    ...
    public function testGetRefundByGuidShouldCatchRestCliBitPayException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/guid/" . $guid, $params, \true)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundQueryException::class);
    ...
    public function testGetRefundByGuidShouldCatchRestCliException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/guid/" . $guid, $params, \true)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundQueryException::class);
        $client->getRefundByGuid($guid);
    }
    ...
    public function testGetRefundByGuidShouldCatchRestCliJsonMapperException()
    {
        ...
        $guid = 'testGuid';
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("refunds/guid/" . $guid, $params, \true)->willReturn(self::CORRUPT_JSON_STRING);
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundQueryException::class);
        $client->getRefundByGuid($guid);
    }
    ...
    public function testSendRefundNotification()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("refunds/" . $exampleRefundId . "/notifications", $params, \true)->willReturn('{"status":"success"}');
        $client = $this->getClient($restCliMock);
    ...
    public function testSendRefundNotificationShouldCatchRestCliBitPayException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("refunds/" . $exampleRefundId . "/notifications", $params, \true)->willThrowException(new BitPayException());
        $client = $this->getClient($restCliMock);
    ...
    public function testSendRefundNotificationShouldCatchRestCliException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("refunds/" . $exampleRefundId . "/notifications", $params, \true)->willThrowException(new Exception());
        $client = $this->getClient($restCliMock);
        $this->expectException(RefundNotificationException::class);
        $client->sendRefundNotification($exampleRefundId);
    }
    ...
    public function testSendRefundNotificationShouldCatchJsonMapperException()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("refunds/" . $exampleRefundId . "/notifications", $params, \true)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/false.json', \true));
        $client = $this->getClient($restCliMock);
    ...
    public function testGetInvoice()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method("get")->with('invoices/' . $exampleInvoiceObject->id, ['token' => self::MERCHANT_TOKEN], \true)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/getInvoice.json'));
        $testedObject = $this->getClient($restCliMock);
        $result = $testedObject->getInvoice($exampleInvoiceObject->id, Facade::MERCHANT, \true);
    ...
    public function testGetInvoiceByGuid()
    {
        $exampleInvoiceObject = \json_decode(\file_get_contents(__DIR__ . '/jsonResponse/getInvoice.json'));
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method("get")->with('invoices/guid/' . $exampleInvoiceObject->guid, ['token' => self::MERCHANT_TOKEN], \true)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/getInvoice.json'));
        $testedObject = $this->getClient($restCliMock);
        $result = $testedObject->getInvoiceByGuid($exampleInvoiceObject->guid, Facade::MERCHANT, \true);
        self::assertEquals($exampleInvoiceObject->id, $result->getId());
        self::assertEquals($exampleInvoiceObject->amountPaid, $result->getAmountPaid());
        self::assertEquals($exampleInvoiceObject->currency, $result->getCurrency());
    }
    ...
    public function testGetInvoiceShouldCatchRestCliExceptions(string $exceptionClass)
    {
        $exampleInvoiceObject = \json_decode(\file_get_contents(__DIR__ . '/jsonResponse/getInvoice.json'));
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method("get")->with('invoices/' . $exampleInvoiceObject->id, ['token' => $exampleInvoiceObject->token], \true)->willThrowException(new $exceptionClass());
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(InvoiceQueryException::class);
        $testedObject->getInvoice($exampleInvoiceObject->id, Facade::MERCHANT, \true);
    }
    public function testGetInvoiceShouldCatchJsonMapperException()
    {
        $exampleInvoiceObject = \json_decode(\file_get_contents(__DIR__ . '/jsonResponse/getInvoice.json'));
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method("get")->with('invoices/' . $exampleInvoiceObject->id, ['token' => $exampleInvoiceObject->token], \true)->willReturn('badString');
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(InvoiceQueryException::class);
        $testedObject->getInvoice($exampleInvoiceObject->id, Facade::MERCHANT, \true);
    }
    ...
    {
        $params = ['status' => 'status', 'dateStart' => 'dateStart', 'dateEnd' => 'dateEnd', 'limit' => 1, 'offset' => 1, 'orderId' => 'orderId', 'token' => self::MERCHANT_TOKEN];
        $successResponse = \file_get_contents(__DIR__ . '/jsonResponse/getInvoices.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method("get")->with('invoices', $params)->willReturn($successResponse);
        $testedObject = $this->getClient($restCliMock);
        $result = $testedObject->getInvoices($params['dateStart'], $params['dateEnd'], $params['status'], $params['orderId'], $params['limit'], $params['offset']);
        $responseToCompare = \json_decode($successResponse, \true);
    ...
    public function testGetInvoicesShouldCatchRestCliExceptions(string $exceptionClass)
    {
        $params = ['status' => 'status', 'dateStart' => 'dateStart', 'dateEnd' => 'dateEnd', 'limit' => 1, 'offset' => 1, 'orderId' => 'orderId', 'token' => self::MERCHANT_TOKEN];
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method("get")->with('invoices', $params)->willThrowException(new $exceptionClass());
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(InvoiceQueryException::class);
        $testedObject->getInvoices($params['dateStart'], $params['dateEnd'], $params['status'], $params['orderId'], $params['limit'], $params['offset']);
    }
    public function testGetInvoicesShouldCatchJsonMapperException()
    {
        $params = ['status' => 'status', 'dateStart' => 'dateStart', 'dateEnd' => 'dateEnd', 'limit' => 1, 'offset' => 1, 'orderId' => 'orderId', 'token' => self::MERCHANT_TOKEN];
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method("get")->with('invoices', $params)->willReturn('badString');
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(InvoiceQueryException::class);
        $testedObject->getInvoices($params['dateStart'], $params['dateEnd'], $params['status'], $params['orderId'], $params['limit'], $params['offset']);
    }
    public function testRequestInvoiceNotificationShouldReturnTrueOnSuccess()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with('invoices/' . $invoiceId . '/notifications', $params)->willReturn($expectedSuccessResponse);
        $testedObject = $this->getClient($restCliMock);
    ...
    public function testRequestInvoiceNotificationShouldReturnFalseOnFailure()
    {
        ...
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with('invoices/' . self::TEST_INVOICE_ID . '/notifications', $params)->willReturn($expectedFailResponse);
        $testedObject = $this->getClient($restCliMock);
    ...
    public function testRequestInvoiceNotificationShouldCatchJsonMapperException()
    {
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with("invoices/" . self::TEST_INVOICE_ID . '/notifications', $params, \true)->willThrowException(new InvoiceQueryException());
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(InvoiceQueryException::class);
    ...
    public function testCancelInvoice()
    {
        ...
        self::assertEquals(\true, $result->getIsCancelled());
    }
    /**
     * @dataProvider exceptionClassProvider
     */
    ...
    public function testCancelInvoiceByGuid()
    {
        ...
        $invoice = \json_decode(\file_get_contents(__DIR__ . '/jsonResponse/getInvoice.json'));
        $invoice->isCancelled = \true;
        $restCliMock->expects(self::once())->method('delete')->with("invoices/guid/" . self::TEST_INVOICE_GUID, $params)->willReturn(\json_encode($invoice));
        $testedObject = $this->getClient($restCliMock);
    ...
    public function testCancelInvoiceByGuidShouldCatchRestCliExceptions(string $exceptionClass)
    {
        $restCliMock = $this->getRestCliMock();
        $params = ['token' => self::MERCHANT_TOKEN, 'forceCancel' => \true];
        $restCliMock->expects(self::once())->method('delete')->with("invoices/guid/" . self::TEST_INVOICE_GUID, $params)->willThrowException(new $exceptionClass());
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(InvoiceCancellationException::class);
    ...
    public function testCancelInvoiceByGuidShouldCatchJsonMapperException()
    {
        $restCliMock = $this->getRestCliMock();
        $params = ['token' => self::MERCHANT_TOKEN, 'forceCancel' => \true];
        $restCliMock->expects(self::once())->method('delete')->with("invoices/guid/" . self::TEST_INVOICE_GUID, $params)->willReturn('corruptJson');
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(InvoiceCancellationException::class);
        $testedObject->cancelInvoiceByGuid(self::TEST_INVOICE_GUID, $params['forceCancel']);
    }
    public function testPayInvoice()
    {
        ...
        $params['token'] = self::MERCHANT_TOKEN;
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with("invoices/pay/" . self::TEST_INVOICE_ID, $params)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/payInvoiceResponse.json'));
        $testedObject = $this->getClient($restCliMock);
    ...
    {
        $invoiceArray = \json_decode(\file_get_contents(__DIR__ . '/jsonResponse/createInvoiceResponse.json'), \true);
        $invoiceMock = $this->createMock(Invoice::class);
        $invoiceMock->expects(self::once())->method('toArray')->willReturn($invoiceArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with('invoices', $invoiceArray, \true)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/createInvoiceResponse.json'));
        $testedObject = $this->getClient($restCliMock);
    ...
    {
        $invoiceArray = \json_decode(\file_get_contents(__DIR__ . '/jsonResponse/getInvoice.json'), \true);
        $invoiceMock = $this->createMock(Invoice::class);
        $invoiceMock->expects(self::once())->method('toArray')->willReturn($invoiceArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with('invoices', $invoiceArray, \true)->willThrowException(new $exceptionClass());
        $testedObject = $this->getClient($restCliMock);
    ...
    public function testCreateInvoiceShouldCatchJsonMapperException()
    {
        ...
        $invoiceMock->expects(self::once())->method('toArray')->willReturn($invoiceArray);
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('post')->with('invoices', $invoiceArray, \true)->willReturn('corruptJson');
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(InvoiceCreationException::class);
        $testedObject->createInvoice($invoiceMock);
    }
    ...
    {
        $params = ['token' => self::MERCHANT_TOKEN, 'buyerEmail' => '', 'buyerSms' => 'buyerSms', 'smsCode' => 'smsCode', 'autoVerify' => \false];
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with('invoices/' . self::TEST_INVOICE_ID, $params)->willThrowException(new $exceptionClass());
        $this->expectException(InvoiceUpdateException::class);
        $testedObject = $this->getClient($restCliMock);
        $testedObject->updateInvoice(self::TEST_INVOICE_ID, $params['buyerSms'], $params['smsCode'], $params['buyerEmail'], $params['autoVerify']);
    }
    public function testUpdateInvoiceShouldCatchJsonMapperException()
    {
        $params = ['token' => self::MERCHANT_TOKEN, 'buyerEmail' => '', 'buyerSms' => 'buyerSms', 'smsCode' => 'smsCode', 'autoVerify' => \false];
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with('invoices/' . self::TEST_INVOICE_ID, $params)->willReturn('corruptJson');
        $this->expectException(InvoiceUpdateException::class);
        $testedObject = $this->getClient($restCliMock);
        $testedObject->updateInvoice(self::TEST_INVOICE_ID, $params['buyerSms'], $params['smsCode'], $params['buyerEmail'], $params['autoVerify']);
    }
    ...
    {
        $params = ['buyerEmail' => 'test', 'buyerSms' => 'buyerSms', 'smsCode' => 'smsCode', 'autoVerify' => \false];
    ...
    {
        $params = ['buyerEmail' => '', 'buyerSms' => 'buyerSms', 'smsCode' => '', 'autoVerify' => \false];
    ...
    {
        $params = ['token' => self::MERCHANT_TOKEN, 'buyerEmail' => null, 'buyerSms' => 'buyerSms', 'smsCode' => 'smsCode', 'autoVerify' => \false];
        $successResponse = \file_get_contents(__DIR__ . '/jsonResponse/getInvoice.json');
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('update')->with('invoices/' . self::TEST_INVOICE_ID, $params)->willReturn($successResponse);
        $testedObject = $this->getClient($restCliMock);
        $result = $testedObject->updateInvoice(self::TEST_INVOICE_ID, $params['buyerSms'], $params['smsCode'], $params['buyerEmail'], $params['autoVerify']);
        $invoiceArray = \json_decode($successResponse, \true);
        self::assertEquals($invoiceArray['id'], $result->getId());
        self::assertEquals($invoiceArray['status'], $result->getStatus());
        self::assertEquals($invoiceArray['guid'], $result->getGuid());
    }
    ...
    {
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("supportedWallets/", null, \false)->willReturn(\file_get_contents(__DIR__ . '/jsonResponse/getSupportedWalletsResponse.json'));
        $client = $this->getClient($restCliMock);
    ...
    {
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("supportedWallets/", null, \false)->willThrowException(new BitPayException());
        $testedObject = $this->getClient($restCliMock);
    ...
    {
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("supportedWallets/", null, \false)->willThrowException(new Exception());
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(WalletQueryException::class);
        $testedObject->getSupportedWallets();
    }
    ...
    {
        $restCliMock = $this->getRestCliMock();
        $restCliMock->expects(self::once())->method('get')->with("supportedWallets/", null, \false)->willReturn(self::CORRUPT_JSON_STRING);
        $testedObject = $this->getClient($restCliMock);
        $this->expectException(WalletQueryException::class);
        $testedObject->getSupportedWallets();
    }
    ...
    {
        return ['status' => 'status', 'startDate' => '2021-07-08 14:21:05', 'endDate' => '2021-07-08 14:24:05', 'limit' => 1, 'offset' => 1, 'reference' => 'reference'];
    }
    private function getInvoiceRefundParams() : array
    {
        return ['token' => self::MERCHANT_TOKEN, 'invoiceId' => 'UZjwcYkWAKfTMn9J1yyfs4', 'amount' => 10.1, 'currency' => Currency::BTC, 'preview' => \true, 'immediate' => \false, 'buyerPaysRefundFee' => \false, 'guid' => '3df73895-2531-e26a-3caa-098a746389b7'];
    }
    /**
     * @throws \ReflectionException
     */
