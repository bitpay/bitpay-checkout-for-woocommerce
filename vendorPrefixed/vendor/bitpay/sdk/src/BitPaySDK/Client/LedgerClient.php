...
class LedgerClient
{
    ...
    public function get(string $currency, string $startDate, string $endDate) : array
    {
        ...
        try {
            ...
            $ledger = $mapper->mapArray(\json_decode($responseJson, \true, 512, \JSON_THROW_ON_ERROR), [], LedgerEntry::class);
        } catch (Exception $e) {
    ...
    public function getLedgers() : array
    {
        ...
        try {
            $mapper = JsonMapperFactory::create();
            $ledgers = $mapper->mapArray(\json_decode($responseJson, \true, 512, \JSON_THROW_ON_ERROR), [], Ledger::class);
        } catch (Exception $e) {
            throw new LedgerQueryException("failed to deserialize BitPay server response (Ledger) : " . $e->getMessage());
        }
        return $ledgers;
