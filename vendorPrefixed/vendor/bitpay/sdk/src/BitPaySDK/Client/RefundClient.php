...
class RefundClient
{
     ...
     * @since 7.2.0)
     */
    public function create(string $invoiceId, float $amount, string $currency, bool $preview = \false, bool $immediate = \false, bool $buyerPaysRefundFee = \false, string $guid = null) : Refund
    {
        $params = [];
        ...
        try {
            $responseJson = $this->restCli->post("refunds/", $params, \true);
        ...
        try {
            ...
            return $mapper->map(\json_decode($responseJson, \true, 512, \JSON_THROW_ON_ERROR), new Refund());
        } catch (Exception $e) {
     ...
     * @throws Exception
     */
    public function update(string $refundId, string $status) : Refund
    {
        $params = [];
        $params["token"] = $this->tokenCache->getTokenByFacade(Facade::MERCHANT);
     ...
     * @since 7.2.0
     */
    public function updateByGuid(string $guid, string $status) : Refund
    {
        $params = [];
        $params["token"] = $this->tokenCache->getTokenByFacade(Facade::MERCHANT);
        $params["status"] = $status;
    ...
    public function getRefunds(string $invoiceId) : array
    {
        ...
        $params["invoiceId"] = $invoiceId;
        try {
            $responseJson = $this->restCli->get("refunds/", $params, \true);
        } catch (BitPayException $e) {
        ...
        try {
            $mapper = JsonMapperFactory::create();
            return $mapper->mapArray(\json_decode($responseJson, \true, 512, \JSON_THROW_ON_ERROR), [], Refund::class);
        } catch (Exception $e) {
    ...
    public function get(string $refundId) : Refund
    {
        ...
        $params["token"] = $this->tokenCache->getTokenByFacade(Facade::MERCHANT);
        try {
            $responseJson = $this->restCli->get("refunds/" . $refundId, $params, \true);
        } catch (BitPayException $e) {
            throw new RefundQueryException("failed to serialize refund object : " . $e->getMessage(), null, null, $e->getApiCode());
        } catch (Exception $e) {
            throw new RefundQueryException("failed to serialize refund object : " . $e->getMessage());
    ...
    public function getByGuid(string $guid) : Refund
    {
        ...
        $params["token"] = $this->tokenCache->getTokenByFacade(Facade::MERCHANT);
        try {
            $responseJson = $this->restCli->get("refunds/guid/" . $guid, $params, \true);
        } catch (BitPayException $e) {
            throw new RefundQueryException("failed to serialize refund object : " . $e->getMessage(), null, null, $e->getApiCode());
        } catch (Exception $e) {
            throw new RefundQueryException("failed to serialize refund object : " . $e->getMessage());
    ...
    public function sendNotification(string $refundId) : bool
    {
        ...
        $params["token"] = $this->tokenCache->getTokenByFacade(Facade::MERCHANT);
        try {
            $responseJson = $this->restCli->post("refunds/" . $refundId . "/notifications", $params, \true);
        } catch (BitPayException $e) {
        ...
        try {
            $result = \json_decode($responseJson, \true, 512, \JSON_THROW_ON_ERROR);
            return $result['status'] === "success";
        } catch (Exception $e) {
            throw new RefundNotificationException("failed to deserialize BitPay server response (Refund) : " . $e->getMessage());
        }
    }
    /**
