...
class BillClient
{
     ...
     */
    public function create(Bill $bill, string $facade = Facade::MERCHANT, bool $signRequest = \true) : Bill
    {
        try {
     ...
     * @throws BitPayException
     */
    public function get(string $billId, string $facade = Facade::MERCHANT, bool $signRequest = \true) : Bill
    {
        try {
            $params = [];
    ...
    public function getBills(string $status = null) : array
    {
        ...
        try {
            ...
            $bills = $mapper->mapArray(\json_decode($responseJson, \true, 512, \JSON_THROW_ON_ERROR), [], Bill::class);
        } catch (Exception $e) {
            throw new BillQueryException("failed to deserialize BitPay server response (Bill) : " . $e->getMessage());
        }
        return $bills;
    ...
    public function update(Bill $bill, string $billId) : Bill
    {
        ...
        try {
            $mapper = JsonMapperFactory::create();
            return $mapper->map(\json_decode($responseJson, \true, 512, \JSON_THROW_ON_ERROR), $bill);
        } catch (Exception $e) {
     ...
     * @throws BitPayException
     */
    public function deliver(string $billId, string $billToken, bool $signRequest = \true) : bool
    {
        try {
            $responseJson = $this->restCli->post("bills/" . $billId . "/deliveries", ['token' => $billToken], $signRequest);
        } catch (BitPayException $e) {
    ...
    {
        $mapper = JsonMapperFactory::create();
        return $mapper->map(\json_decode($responseJson, \true, 512, \JSON_THROW_ON_ERROR), new Bill());
    }
}
