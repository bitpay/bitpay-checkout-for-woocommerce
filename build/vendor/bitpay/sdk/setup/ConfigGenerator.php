...
$privateKeyname = 'PrivateKeyName.key';
// Add here the name for your Private key
$generateMerchantToken = \true;
// Set to true to generate a token for the Merchant facade
$generatePayoutToken = \true;
// Set to true to generate a token for the Payout facade (Request to Support if you need it)
$yourMasterPassword = 'YourMasterPassword';
//Will be used to encrypt your PrivateKey
$generateJSONfile = \true;
// Set to true to generate the Configuration File in Json format
$generateYMLfile = \true;
// Set to true to generate the Configuration File in Yml format
$proxy = null;
...
try {
    if ($generateMerchantToken) {
        $facade = 'merchant';
        $postData = \json_encode(['id' => $sin, 'facade' => $facade]);
        $curlCli = \curl_init($baseUrl . "/tokens");
        \curl_setopt($curlCli, \CURLOPT_HTTPHEADER, ['x-accept-version: 2.0.0', 'Content-Type: application/json', 'x-identity' => $publicKey->__toString(), 'x-signature' => $privateKey->sign($baseUrl . "/tokens" . $postData)]);
        \curl_setopt($curlCli, \CURLOPT_CUSTOMREQUEST, 'POST');
        \curl_setopt($curlCli, \CURLOPT_POSTFIELDS, \stripslashes($postData));
        \curl_setopt($curlCli, \CURLOPT_RETURNTRANSFER, \true);
        $result = \curl_exec($curlCli);
        $resultData = \json_decode($result, \true);
        \curl_close($curlCli);
 ...
 */
$config = ["BitPayConfiguration" => ["Environment" => $env, "EnvConfig" => ['Test' => ["PrivateKeyPath" => $isProd ? null : __DIR__ . "/" . $privateKeyname, "PrivateKeySecret" => $isProd ? null : $yourMasterPassword, "ApiTokens" => ["merchant" => $isProd ? null : $merchantToken, "payout" => $isProd ? null : $payoutToken], "Proxy" => $proxy], 'Prod' => ["PrivateKeyPath" => $isProd ? __DIR__ . "/" . $privateKeyname : null, "PrivateKeySecret" => $isProd ? $yourMasterPassword : null, "ApiTokens" => ["merchant" => $isProd ? $merchantToken : null, "payout" => $isProd ? $payoutToken : null], "Proxy" => $proxy]]]];
try {
    if ($generateJSONfile) {
