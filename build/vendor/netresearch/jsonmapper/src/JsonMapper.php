...
class JsonMapper
{
    ...
    public function map($json, $object)
    {
        ...
        if (!\is_object($object) && (!\is_string($object) || !\class_exists($object))) {
            throw new \InvalidArgumentException('JsonMapper::map() requires second argument to ' . 'be an object or existing class name' . ', ' . \gettype($object) . ' given.');
        }
        if (\is_string($object)) {
        ...
        foreach ($json as $key => $jvalue) {
            ...
            if (!$hasProperty) {
                if ($this->bExceptionOnUndefinedProperty) {
                    ...
                    if ($this->undefinedPropertyHandler !== null) {
                        $undefinedPropertyKey = \call_user_func($this->undefinedPropertyHandler, $object, $key, $jvalue);
                        if (\is_string($undefinedPropertyKey)) {
                            list($hasProperty, $accessor, $type, $isNullable) = $this->inspectProperty($rc, $undefinedPropertyKey);
                        }
                    } else {
            ...
            if ($type === null || $type === 'mixed') {
                ...
                if ($this->isObjectOfSameType($type, $jvalue)) {
                    ...
                    if ($this->isSimpleType($type) && !(\is_array($jvalue) && $this->hasVariadicArrayType($accessor))) {
            ...
            if ($this->isArrayOfType($type)) {
                ...
                if (\substr($type, -1) == ']') {
                    ...
                    if (\is_array($jvalue) && $this->hasVariadicArrayType($accessor)) {
                        $array = array();
                        $subtype = $type;
                    } else {
                        if (\is_a($type, 'ArrayAccess', \true)) {
    ...
    protected function setProperty($object, $accessor, $value)
    {
        ...
        if ($accessor instanceof \ReflectionProperty) {
            ...
            if (\is_array($value) && $this->hasVariadicArrayType($accessor)) {
 ...
 * @link     http://cweiske.de/
 */
\class_alias('BitPayVendor\\JsonMapper', 'JsonMapper', \false);
