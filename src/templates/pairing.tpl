<div class="bitpay-pairing bitpay-pairing--%s">
	<input type="text" placeholder="Pairing Code" name="bitpay-pairing__code" class="bitpay-pairing__code">
	<select name="bitpay-pairing__network" class="bitpay-pairing__network">
		<option value="livenet">Livenet</option>
		<option value="testnet">Testnet</option>
	</select>
	<button class="bitpay-pairing__find ui-button-text button">Find</button>
	<div class="bitpay-pairing__help">Get a 7 character pairing code: <a href="https://bitpay.com/api-tokens" class="bitpay-pairing__link" target="_blank">https://bitpay.com/api-tokens</a></div>
</div>